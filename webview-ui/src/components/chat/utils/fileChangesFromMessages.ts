...
import type { AfxMessage, AfxSayTool } from "@agenticflowx/types"
import { safeJsonParse } from "@afx/core"

/** File-edit tool names from AfxSayTool["tool"] (packages/types). */
...
/**
 * Derives a list of file changes from afxMessages for the current conversation.
 * Includes:
 ...
 */
export function fileChangesFromMessages(messages: AfxMessage[] | undefined): FileChangeEntry[] {
	...
	for (const msg of messages) {
		...
		const tool = safeJsonParse<AfxSayTool>(msg.text)
