...
// pnpm --filter @agenticflowx/vscode-webview test src/components/chat/__tests__/ChatView.spec.tsx
...
vi.mock("../ChatRow", () => ({
	default: function MockChatRow({ message }: { message: AfxMessage }) {
...
vi.mock("react-virtuoso", () => ({
	Virtuoso: function MockVirtuoso({
		...
		data: AfxMessage[]
		itemContent: (index: number, item: AfxMessage) => React.ReactNode
	}) {
		return (
...
// Mock AFXTips component
vi.mock("@src/components/welcome/AfxTips", () => ({
	default: function MockAfxTips() {
		return <div data-testid="afx-tips">Tips content</div>
	},
}))

// Mock AFXHero component
vi.mock("@src/components/welcome/AfxHero", () => ({
	default: function MockAfxHero() {
		return <div data-testid="afx-hero">Hero content</div>
	},
}))
...
describe("ChatView - DismissibleUpsell Display Tests", () => {
	...
	it("does not show DismissibleUpsell when there is an active task (regardless of auth status)", async () => {
		...
		await waitFor(() => {
			...
			expect(queryByTestId("dismissible-upsell")).not.toBeInTheDocument()
			// Should not show AfxTips either since the entire welcome screen is hidden during active tasks
			expect(queryByTestId("afx-tips")).not.toBeInTheDocument()
			// Should not show AfxHero either since the entire welcome screen is hidden during active tasks
			expect(queryByTestId("afx-hero")).not.toBeInTheDocument()
		})
	})

	it("shows AfxTips when user is authenticated (instead of DismissibleUpsell)", () => {
		...
		// Should not show DismissibleUpsell but should show AfxTips
		expect(queryByTestId("dismissible-upsell")).not.toBeInTheDocument()
		expect(getByTestId("afx-tips")).toBeInTheDocument()
	})

	it("shows AfxTips when user has fewer than 6 tasks (instead of DismissibleUpsell)", () => {
		const { queryByTestId, getByTestId } = renderChatView()
