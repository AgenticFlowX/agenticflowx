...
import { defaultModeSlug } from "@afx/modes"
...
const getEnhancePromptButton = () => {
	return screen.getByRole("button", {
		name: (_, element) => {
			// Find the button with the wand sparkles codicon
			return element.querySelector(".codicon-wand") !== null
		},
	})
}
...
describe("ChatTextArea", () => {
	...
	describe("enhance prompt button", () => {
		it("should be enabled even when sendingDisabled is true (for message queueing)", () => {
			...
			render(<ChatTextArea {...defaultProps} inputValue="Test prompt" sendingDisabled={true} />)
	...
	describe("handleEnhancePrompt", () => {
		...
		it("should not render enhance button when input is empty", () => {
			;(useExtensionState as ReturnType<typeof vi.fn>).mockReturnValue({
				filePaths: [],
				openedTabs: [],
				apiConfiguration: {
			...
			const { container } = render(<ChatTextArea {...defaultProps} inputValue="" />)

			// Enhance button should not be present when input is empty
			const wandIcon = container.querySelector(".codicon-wand")
			expect(wandIcon).not.toBeInTheDocument()
		})
		...
		it("should show loading state while enhancing", () => {
			...
			// Check if the wand icon has the spinning modifier class
			const animatingIcon = enhanceButton.querySelector(".codicon-modifier-spin")
	...
	describe("effect dependencies", () => {
		it("should update when apiConfiguration changes", () => {
			...
			rerender(<ChatTextArea {...defaultProps} inputValue="Test prompt" />)
		...
		describe("prompt history navigation", () => {
			...
			it("should ignore empty or whitespace-only messages", () => {
				...
				;(useExtensionState as ReturnType<typeof vi.fn>).mockReturnValue({
					...
					taskHistory: [],
					afxMessages: afxMessagesWithEmpty,
					cwd: "/test/workspace",
				})
			...
			it("should use task history (oldest first) when no conversation messages exist", () => {
				...
				;(useExtensionState as ReturnType<typeof vi.fn>).mockReturnValue({
					...
					afxMessages: [], // No conversation messages
					cwd: "/test/workspace",
				})
	...
	describe("send button visibility", () => {
		it("should show send button when there are images but no text", () => {
			...
			const sendButton = Array.from(buttons).find((button) => button.querySelector(".codicon-send") !== null)
