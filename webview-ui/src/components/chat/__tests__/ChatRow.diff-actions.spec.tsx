	...
	useTranslation: () => ({
		t: (key: string) => {
			const map: Record<string, string> = {
				"chat:fileOperations.wantsToEdit": "AFX wants to edit this file",
				"chat:fileOperations.wantsToEditProtected": "AFX wants to edit a protected file",
				"chat:fileOperations.wantsToEditOutsideWorkspace": "AFX wants to edit outside workspace",
				"chat:fileOperations.wantsToApplyBatchChanges": "AFX wants to apply batch changes",
			}
			return map[key] || key
		},
	}),
...
function createToolAskMessage(toolPayload: Record<string, unknown>): AfxMessage {
	return {
		type: "ask",
		ask: "tool",
		ts: Date.now(),
...
function renderChatRow(message: AfxMessage, isExpanded = false) {
	return render(
...
describe("ChatRow - inline diff stats and actions", () => {
	...
	it("uses appliedDiff edit treatment (header/icon/diff stats)", () => {
		...
		expect(screen.getByText("AFX wants to edit this file")).toBeInTheDocument()
	...
	it("preserves protected and outside-workspace messaging in unified branch", () => {
		...
		expect(screen.getByText("AFX wants to edit outside workspace")).toBeInTheDocument()
		...
		expect(screen.getByText("AFX wants to edit a protected file")).toBeInTheDocument()
	...
	it("keeps batch diff handling for unified edit tools", () => {
		...
		expect(screen.getByText("AFX wants to apply batch changes")).toBeInTheDocument()
