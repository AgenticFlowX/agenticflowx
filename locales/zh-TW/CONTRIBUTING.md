...
AgenticFlowX 是一個由社群驅動的專案，我們非常重視每一份貢獻。為了簡化協作流程，我們採用 [「Issue 優先」的方法](#issue-優先方法)，這意味著所有的 [Pull Request (PR)](#提交-pull-request) 都必須先連結到一個 GitHub Issue。請仔細閱讀本指南。
...
### 3. 加入 AgenticFlowX 社群

- **主要方式：** 加入我們的 [Discord](https://github.com/AgenticFlowX/agenticflowx/discussions) 並私訊 **the maintainers**。
- **替代方式：** 經驗豐富的貢獻者可以透過 [GitHub Project](https://github.com/orgs/AgenticFlowX/projects/1) 直接參與。
...
- **檢查現有 Issue**：在 [GitHub Issues](https://github.com/AgenticFlowX/agenticflowx/issues) 中搜尋。
- **使用以下範本建立 Issue**：
    - **增強功能：** 「Enhancement Request」範本（著重於使用者利益的淺顯描述）。
    - **錯誤** 「Bug Report」範本（最少的重現步驟 + 預期與實際結果 + 版本）。
- **想參與其中嗎？** 在 Issue 上留言「Claiming」，並在 [Discord](https://github.com/AgenticFlowX/agenticflowx/discussions) 上私訊 **the maintainers** 以獲得分配。分配結果將在討論串中確認。
- **PR 必須連結到 Issue。** 未連結的 PR 可能會被關閉。

### 決定做什麼

- 查看 [GitHub 專案](https://github.com/orgs/AgenticFlowX/projects/1) 中的「Issue [Unassigned]」。
- 如需文件，請造訪 [AgenticFlowX Docs](https://github.com/AgenticFlowX/agenticflowx-Docs)。

### 回報錯誤

- 首先檢查現有的報告。
- 使用 [「Bug Report」範本](https://github.com/AgenticFlowX/agenticflowx/issues/new/choose) 建立一個新的錯誤回報，並提供：
    - 清晰、編號的重現步驟
    - 預期與實際結果
    - AgenticFlowX 版本（必填）；如果相關，還需提供 API 供應商/模型
- **安全問題**：透過 [安全公告 (Security Advisories)](https://github.com/AgenticFlowX/agenticflowx/security/advisories/new) 私下回報。
...
- 必須引用一個已指派的 GitHub Issue。如要被指派：請在 Issue 上留言「Claiming」，並在 [Discord](https://github.com/AgenticFlowX/agenticflowx/discussions) 上私訊 **the maintainers**。指派結果將可在討論串中確認。
...
透過貢獻，您同意您的貢獻將根據 Apache 2.0 授權條款進行授權，這與 AgenticFlowX 的授權一致。
