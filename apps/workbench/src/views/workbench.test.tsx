...
describe("Workbench", () => {
  ...
  it("renders spec and design as clean paper readers", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText("PRD Studio")).toHaveLength(2);
    });
    ...
    expect(screen.getAllByLabelText("PRD section highlights").length).toBeGreaterThan(0);
  });

  it("keeps columns readable with an internal horizontal rail", () => {
    renderWorkbench();

    const region = screen.getByTestId("workbench-column-region");
    const rail = region.firstElementChild as HTMLElement | null;

    expect(rail).not.toBeNull();
    expect(rail?.style.gridTemplateColumns).toBe("repeat(3, minmax(420px, 1fr))");
  });
