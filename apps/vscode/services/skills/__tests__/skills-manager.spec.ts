...
const GLOBAL_AFX_DIR = p(HOME_DIR, ".afx")
const GLOBAL_AGENTS_DIR = p(HOME_DIR, ".agents")

// Mock afx-config
vi.mock("../../afx-config", () => ({
	getGlobalAfxDirectory: () => GLOBAL_AFX_DIR,
...
describe("SkillsManager", () => {
	...
	let mockProvider: Partial<ITaskProvider>

	// Pre-computed paths for tests
	const globalSkillsDir = p(GLOBAL_AFX_DIR, "skills")
	const globalSkillsCodeDir = p(GLOBAL_AFX_DIR, "skills-code")
	const globalSkillsArchitectDir = p(GLOBAL_AFX_DIR, "skills-architect")
	const projectAfxDir = p(PROJECT_DIR, ".afx")
	const projectSkillsDir = p(projectAfxDir, "skills")
	...
	beforeEach(() => {
		...
		skillsManager = new SkillsManager(mockProvider as ITaskProvider)
	})
...
# Too Long Description`)
		...
		it("should handle symlinked skills directory", async () => {
			...
			// Simulate .afx/skills being a symlink to /shared/skills
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should handle symlinked skill subdirectory", async () => {
			...
			// Simulate .afx/skills/my-alias being a symlink to /external/actual-skill
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should prioritize .afx skills over .agents skills with same name", async () => {
...
# Agent Common Skill`
				...
				if (file === rooSkillMd) {
					return `---
name: common-skill
description: AFX version (should take priority)
---
...
# AFX Common Skill`
			...
			// .afx should override .agents
			expect(commonSkill?.description).toBe("AFX version (should take priority)")
		})
...
Instructions`)
	...
	describe("createSkill", () => {
		it("should create a new global skill", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_AFX_DIR, "skills", "new-skill", "SKILL.md"))
			expect(mockMkdir).toHaveBeenCalledWith(p(GLOBAL_AFX_DIR, "skills", "new-skill"), { recursive: true })
			expect(mockWriteFile).toHaveBeenCalled()

			// Verify the content written
			const writeCall = mockWriteFile.mock.calls[0]
			expect(writeCall[0]).toBe(p(GLOBAL_AFX_DIR, "skills", "new-skill", "SKILL.md"))
		...
		it("should create a mode-specific skill with modeSlugs array", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_AFX_DIR, "skills", "code-skill", "SKILL.md"))
		...
		it("should create a project skill", async () => {
			...
			expect(createdPath).toBe(p(PROJECT_DIR, ".afx", "skills", "project-skill", "SKILL.md"))
		})
...
Instructions`)
	...
	describe("moveSkill", () => {
		it("should move a skill from generic to mode-specific directory", async () => {
			...
			const destDir = p(GLOBAL_AFX_DIR, "skills-code", "test-skill")
			const destSkillsDir = p(GLOBAL_AFX_DIR, "skills-code")
...
Instructions`)
		...
		it("should move a skill from one mode to another", async () => {
			const sourceSkillsDir = p(GLOBAL_AFX_DIR, "skills-code")
			const sourceDir = p(sourceSkillsDir, "test-skill")
			const testSkillMd = p(sourceDir, "SKILL.md")
			const destDir = p(GLOBAL_AFX_DIR, "skills-architect", "test-skill")
			const destSkillsDir = p(GLOBAL_AFX_DIR, "skills-architect")
