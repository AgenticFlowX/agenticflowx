...
import { RepoPerTaskCheckpointService } from "../repo-per-task-checkpoint-service"

const tmpDir = path.join(os.tmpdir(), "CheckpointService")

const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "AgenticFlowX",
	userEmail = "support@agenticflowx.com",
	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "AgenticFlowX")
				await mainGit.addConfig("user.email", "support@agenticflowx.com")
				...
				await nestedGit.addConfig("user.name", "AgenticFlowX")
				await nestedGit.addConfig("user.email", "support@agenticflowx.com")
		...
		describe(`${klass.name}#saveCheckpoint with allowEmpty option`, () => {
			...
			it("isolates checkpoint operations from GIT_DIR environment variable", async () => {
				...
				// In the real-world Dev Container scenario, GIT_DIR is set BEFORE AFX starts,
