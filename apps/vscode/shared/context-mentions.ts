...
*/
// [AFX-START] AFX-specific mention patterns, injected into the main mentionRegex.
// Keeps AFX additions isolated so upstream syncs only touch this block.
// - afx-specs: references AFX spec files, with optional #fragment drill-down (e.g. @afx-specs#auth-flow)
const _mentionRegexAfx = `afx-specs(?:#[a-z0-9-]*)*`

const _mentionRegexBase = `(?:^|(?<=\\s))(?<!\\\\)@((?:\\/|\\w+:\\/\\/)(?:[^\\s\\\\]|\\\\ )+?|[a-f0-9]{7,40}\\b|problems\\b|git-changes\\b|terminal\\b|${_mentionRegexAfx})(?=[.,;:!?]?(?=[\\s\\r\\n]|$))`

export const mentionRegex = new RegExp(_mentionRegexBase)
// [AFX-END]
