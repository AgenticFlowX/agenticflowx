...
import type { AfxMessage, AfxSay } from "@agenticflowx/types"

import { combineApiRequests } from "../combine-api-requests"

describe("combineApiRequests", () => {
	// Helper function to create a basic api_req_started message
	const createStartMessage = (text: string = '{"request":"GET /api/data"}', ts: number = 1000): AfxMessage => ({
		type: "say",
		say: "api_req_started",
		text,
		ts,
	...
	const createFinishMessage = (text: string = '{"cost":0.005}', ts: number = 1001): AfxMessage => ({
		type: "say",
	...
	const createOtherMessage = (say: AfxSay = "text", text: string = "Hello world", ts: number = 999): AfxMessage => ({
		type: "say",
		say,
		text,
		ts,
	...
	describe("Basic functionality", () => {
		it("should combine a pair of api_req_started and api_req_finished messages", () => {
			const messages: AfxMessage[] = [createStartMessage(), createFinishMessage()]
		...
		it("should preserve non-API messages", () => {
			...
			const messages: AfxMessage[] = [otherMessage, createStartMessage(), createFinishMessage()]
	...
	describe("Edge cases", () => {
		...
		it("should keep api_req_started message if no matching api_req_finished is found", () => {
			...
			const messages: AfxMessage[] = [startMessage]
		...
		it("should handle missing text field in api_req_started", () => {
			...
			const messages: AfxMessage[] = [startMessage, finishMessage]
		...
		it("should handle missing text field in api_req_finished", () => {
			const startMessage = createStartMessage()
			const finishMessage: AfxMessage = {
				type: "say",
				say: "api_req_finished",
				ts: 1001,
				// text field is missing
	...
	describe("Bug verification", () => {
		it("should correctly replace api_req_started messages with combined ones", () => {
			...
			const messages: AfxMessage[] = [otherMessage, startMessage1, finishMessage1, startMessage2]
