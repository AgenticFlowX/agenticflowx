...
import { IpcServer } from "@agenticflowx/ipc"

import { Package } from "../shared/package"
import { AfxProvider } from "../core/webview/afx-provider"
import { openClineInNewTab } from "../activate/register-commands"
import { getCommands } from "../services/command/commands"

export class API extends EventEmitter<AfxEvents> implements AfxAPI {
	private readonly outputChannel: vscode.OutputChannel
	private readonly sidebarProvider: AfxProvider
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(os.tmpdir(), "agenticflowx-messages.log")
		} else {
			this.log = () => {}
		}
		...
		if (socketPath) {
			...
			ipc.on(IpcMessageType.TaskCommand, async (clientId, command) => {
				const sendResponse = (eventName: AfxEventName, payload: unknown[]) => {
					ipc.send(clientId, {
				...
				switch (command.commandName) {
					...
					case TaskCommandName.GetCommands:
						try {
							...
							sendResponse(AfxEventName.CommandsResponse, [
								commands.map((cmd) => ({
									name: cmd.name,
									source: cmd.source,
					...
					case TaskCommandName.GetModes:
						try {
							...
							sendResponse(AfxEventName.ModesResponse, [modes])
						} catch (error) {
							sendResponse(AfxEventName.ModesResponse, [[]])
						}

						break
					case TaskCommandName.GetModels:
						// TODO: Implement model listing for configured provider
						sendResponse(AfxEventName.ModelsResponse, [{}])
						break
	...
	public override emit<K extends keyof AfxEvents>(
		eventName: K,
		...args: K extends keyof AfxEvents ? AfxEvents[K] : never
	) {
		const data = { eventName: eventName as AfxEventName, payload: args } as TaskEvent
	...
	private registerListeners(provider: AfxProvider) {
		provider.on(AfxEventName.TaskCreated, (task) => {
			// Task Lifecycle

			task.on(AfxEventName.TaskStarted, async () => {
				this.emit(AfxEventName.TaskStarted, task.taskId)
				await this.fileLog(`[${new Date().toISOString()}] taskStarted -> ${task.taskId}\n`)
			})

			task.on(AfxEventName.TaskCompleted, async (_, tokenUsage, toolUsage) => {
				this.emit(AfxEventName.TaskCompleted, task.taskId, tokenUsage, toolUsage, {
			...
			task.on(AfxEventName.TaskAborted, () => {
				this.emit(AfxEventName.TaskAborted, task.taskId)
			})

			task.on(AfxEventName.TaskFocused, () => {
				this.emit(AfxEventName.TaskFocused, task.taskId)
			})

			task.on(AfxEventName.TaskUnfocused, () => {
				this.emit(AfxEventName.TaskUnfocused, task.taskId)
			})

			task.on(AfxEventName.TaskActive, () => {
				this.emit(AfxEventName.TaskActive, task.taskId)
			})

			task.on(AfxEventName.TaskInteractive, () => {
				this.emit(AfxEventName.TaskInteractive, task.taskId)
			})

			task.on(AfxEventName.TaskResumable, () => {
				this.emit(AfxEventName.TaskResumable, task.taskId)
			})

			task.on(AfxEventName.TaskIdle, () => {
				this.emit(AfxEventName.TaskIdle, task.taskId)
			})

			// Subtask Lifecycle

			task.on(AfxEventName.TaskPaused, () => {
				this.emit(AfxEventName.TaskPaused, task.taskId)
			})

			task.on(AfxEventName.TaskUnpaused, () => {
				this.emit(AfxEventName.TaskUnpaused, task.taskId)
			})

			task.on(AfxEventName.TaskSpawned, (childTaskId) => {
				this.emit(AfxEventName.TaskSpawned, task.taskId, childTaskId)
			})

			task.on(AfxEventName.TaskDelegated as any, (childTaskId: string) => {
				;(this.emit as any)(AfxEventName.TaskDelegated, task.taskId, childTaskId)
			})

			task.on(AfxEventName.TaskDelegationCompleted as any, (childTaskId: string, summary: string) => {
				;(this.emit as any)(AfxEventName.TaskDelegationCompleted, task.taskId, childTaskId, summary)
			})

			task.on(AfxEventName.TaskDelegationResumed as any, (childTaskId: string) => {
				;(this.emit as any)(AfxEventName.TaskDelegationResumed, task.taskId, childTaskId)
			})

			// Task Execution

			task.on(AfxEventName.Message, async (message) => {
				this.emit(AfxEventName.Message, { taskId: task.taskId, ...message })
			...
			task.on(AfxEventName.TaskModeSwitched, (taskId, mode) => {
				this.emit(AfxEventName.TaskModeSwitched, taskId, mode)
			})

			task.on(AfxEventName.TaskAskResponded, () => {
				this.emit(AfxEventName.TaskAskResponded, task.taskId)
			})

			task.on(AfxEventName.QueuedMessagesUpdated, (taskId, messages) => {
				this.emit(AfxEventName.QueuedMessagesUpdated, taskId, messages)
			})

			// Task Analytics

			task.on(AfxEventName.TaskToolFailed, (taskId, tool, error) => {
				this.emit(AfxEventName.TaskToolFailed, taskId, tool, error)
			})

			task.on(AfxEventName.TaskTokenUsageUpdated, (_, tokenUsage, toolUsage) => {
				this.emit(AfxEventName.TaskTokenUsageUpdated, task.taskId, tokenUsage, toolUsage)
			})

			// Let's go!

			this.emit(AfxEventName.TaskCreated, task.taskId)
		})
	}
	...
	public getConfiguration(): AfxSettings {
		return Object.fromEntries(
			Object.entries(this.sidebarProvider.getValues()).filter(([key]) => !isSecretStateKey(key)),
		)
	}

	public async setConfiguration(values: AfxSettings) {
