...
import { AfxEventName, ProviderSettings, TokenUsage, ToolUsage } from "@agenticflowx/types"

import { Task } from "../task"
import { AfxProvider } from "../../webview/afx-provider"
import { hasToolUsageChanged, hasTokenUsageChanged } from "../../../shared/get-api-metrics"

// Mock dependencies
vi.mock("../../webview/afx-provider")
vi.mock("../../../integrations/terminal/terminal-registry", () => ({
	TerminalRegistry: {
		releaseTerminalsForTask: vi.fn(),
	},
}))
vi.mock("../../ignore/afx-ignore-controller")
vi.mock("../../protect/afx-protected-controller")
vi.mock("../../context-tracking/file-context-tracker")
vi.mock("../../../integrations/editor/diff-view-provider")
vi.mock("../../tools/tool-repetition-detector")
vi.mock("../../../api", () => ({
...
vi.mock("@agenticflowx/telemetry", () => ({
	TelemetryService: {
		instance: {
...
describe("Task token usage throttling", () => {
	...
	beforeEach(() => {
		...
		task = new Task({
			provider: mockProvider as AfxProvider,
	...
	test("should emit TaskTokenUsageUpdated immediately on first change", async () => {
		...
		// Add a message to trigger saveAfxMessages
		await (task as any).addToAfxMessages({
			ts: Date.now(),
			type: "say",
			say: "text",
			text: "Test message",
		...
		expect(emitSpy).toHaveBeenCalledWith(
			AfxEventName.TaskTokenUsageUpdated,
			task.taskId,
			expect.any(Object),
			expect.any(Object),
		)
	...
	test("should throttle subsequent emissions within 2 seconds", async () => {
		...
		const firstEmitCount = emitSpy.mock.calls.filter(
			(call) => call[0] === AfxEventName.TaskTokenUsageUpdated,
		).length
	...
	test("should force final emission on task abort", async () => {
		...
		const tokenUsageUpdateIndex = calls.findIndex((call) => call[0] === AfxEventName.TaskTokenUsageUpdated)
		const taskAbortedIndex = calls.findIndex((call) => call[0] === AfxEventName.TaskAborted)
