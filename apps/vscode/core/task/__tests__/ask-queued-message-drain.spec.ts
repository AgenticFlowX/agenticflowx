...
describe("Task.ask queued message drain", () => {
	it("consumes queued message while blocked on followup ask", async () => {
		...
		const { MessageQueueService } = await import("../../message-queue/message-queue-service")
		;(task as any).messageQueueService = new MessageQueueService()

		// Minimal stubs used by ask()
		;(task as any).addToAfxMessages = vi.fn(async () => {})
		;(task as any).saveAfxMessages = vi.fn(async () => {})
		;(task as any).updateAfxMessage = vi.fn(async () => {})
