...
import type { GlobalState, ProviderSettings } from "@agenticflowx/types"
...
vi.mock("../../environment/get-environment-details", () => ({
...
describe("Task persistence", () => {
	let mockProvider: AfxProvider & Record<string, any>
	...
	beforeEach(() => {
		...
		mockProvider = new AfxProvider(
			mockExtensionContext,
			mockOutputChannel,
			"sidebar",
			new ContextProxy(mockExtensionContext),
		) as AfxProvider & Record<string, any>
	...
	describe("saveAfxMessages", () => {
		it("returns true on success", async () => {
			...
			const result = await (task as Record<string, any>).saveAfxMessages()
			expect(result).toBe(true)
		})
		...
		it("snapshots the array before passing to saveTaskMessages", async () => {
			...
			await (task as Record<string, any>).saveAfxMessages()

			expect(mockSaveTaskMessages).toHaveBeenCalledTimes(1)

			const callArgs = mockSaveTaskMessages.mock.calls[0][0]
			// The messages passed should be a COPY, not the live reference
			expect(callArgs.messages).not.toBe(task.afxMessages)
			// But the content should be the same
			expect(callArgs.messages).toEqual(task.afxMessages)
		})
	})
