...
import type { ProviderSettings, ModeConfig, ModelInfo } from "@agenticflowx/types"
import { customToolRegistry, formatNative } from "@agenticflowx/core"
...
export async function buildNativeToolsArrayWithRestrictions(options: BuildToolsOptions): Promise<BuildToolsResult> {
	...
	if (experiments?.customTools) {
		const toolDirs = getAfxDirectoriesForCwd(cwd).map((dir) => path.join(dir, "tools"))
