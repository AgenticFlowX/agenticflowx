...
import { AskIgnoredError } from "./ask-ignored-error"
...
import { formatToolInvocation } from "../tools/helpers/tool-result-formatting"
...
import { t } from "../../i18n"
import { getApiMetrics, hasTokenUsageChanged, hasToolUsageChanged } from "../../shared/get-api-metrics"
import { AfxAskResponse } from "../../shared/webview-message"
...
import { AfxTerminalProcess } from "../../integrations/terminal/types"
import { TerminalRegistry } from "../../integrations/terminal/terminal-registry"
import { OutputInterceptor } from "../../integrations/terminal/output-interceptor"
...
// core modules
import { ToolRepetitionDetector } from "../tools/tool-repetition-detector"
import { restoreTodoListForTask } from "../tools/update-todo-list-tool"
import { FileContextTracker } from "../context-tracking/file-context-tracker"
import { AfxIgnoreController, createVscodeFileWatcher } from "../ignore/afx-ignore-controller"
import { AfxProtectedController } from "../protect/afx-protected-controller"
import { type AssistantMessageContent, presentAssistantMessage } from "../assistant-message"
import { NativeToolCallParser } from "../assistant-message/native-tool-call-parser"
import { manageContext, willManageContext } from "../context-management"
import { AfxProvider } from "../webview/afx-provider"
...
import { getEnvironmentDetails } from "../environment/get-environment-details"
...
import { processUserContentMentions } from "../mentions/process-user-content-mentions"
import { getMessagesSinceLastSummary, summarizeConversation, getEffectiveApiHistory } from "../condense"
import { MessageQueueService } from "../message-queue/message-queue-service"
import { AutoApprovalHandler, checkAutoApproval } from "../auto-approval"
import { MessageManager } from "../message-manager"
import { validateAndFixToolResultIds } from "./validate-tool-result-ids"
import { mergeConsecutiveApiMessages } from "./merge-consecutive-api-messages"
...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	providerRef: WeakRef<AfxProvider>
	...
	// TaskStatus
	idleAsk?: AfxMessage
	resumableAsk?: AfxMessage
	interactiveAsk?: AfxMessage

	didFinishAbortingStream = false
	abandoned = false
	abortReason?: AfxApiReqCancelReason
	isInitialized = false
	...
	afxIgnoreController?: AfxIgnoreController
	afxProtectedController?: AfxProtectedController
	fileContextTracker: FileContextTracker
	terminalProcess?: AfxTerminalProcess
	...
	// Ask
	private askResponse?: AfxAskResponse
	...
	constructor({
		...
		this.afxIgnoreController = new AfxIgnoreController(this.cwd, createVscodeFileWatcher)
		this.afxProtectedController = new AfxProtectedController(this.cwd)
		this.fileContextTracker = new FileContextTracker(provider, this.taskId)

		this.afxIgnoreController.initialize().catch((error) => {
			console.error("Failed to initialize AfxIgnoreController:", error)
		})
		...
		this.messageQueueStateChangedHandler = () => {
			this.emit(AfxEventName.TaskUserMessage, this.taskId)
			this.emit(AfxEventName.QueuedMessagesUpdated, this.taskId, this.messageQueueService.messages)
		...
		this.debouncedEmitTokenUsage = debounce(
			(tokenUsage: TokenUsage, toolUsage: ToolUsage) => {
				...
				if (tokenChanged || toolChanged) {
					this.emit(AfxEventName.TaskTokenUsageUpdated, this.taskId, tokenUsage, toolUsage)
					this.tokenUsageSnapshot = tokenUsage
					this.tokenUsageSnapshotAt = this.afxMessages.at(-1)?.ts
	 ...
	 *
	 * @private
	 * @param provider - The AfxProvider instance to fetch state from
	 * @returns Promise that resolves when initialization is complete
	 */
	private async initializeTaskMode(provider: AfxProvider): Promise<void> {
		try {
	 ...
	 * @returns Promise that resolves when initialization is complete
	 */
	private async initializeTaskApiConfigName(provider: AfxProvider): Promise<void> {
		try {
			const state = await provider.getState()
	 ...
	 *
	 * @private
	 * @param provider - The AfxProvider instance to listen to
	 */
	private setupProviderProfileChangeListener(provider: AfxProvider): void {
		...
		provider.on(AfxEventName.ProviderProfileChanged, this.providerProfileChangeListener)
	}
	...
	private async getSavedAfxMessages(): Promise<AfxMessage[]> {
		return readTaskMessages({ taskId: this.taskId, globalStoragePath: this.globalStoragePath })
	}

	private async addToAfxMessages(message: AfxMessage) {
		this.afxMessages.push(message)
		const provider = this.providerRef.deref()
		// Avoid resending large, mostly-static fields (notably taskHistory) on every chat message update.
		// taskHistory is maintained in-memory in the webview and updated via taskHistoryItemUpdated.
		await provider?.postStateToWebviewWithoutTaskHistory()
		this.emit(AfxEventName.Message, { action: "created", message })
		await this.saveAfxMessages()
	}

	public async overwriteAfxMessages(newMessages: AfxMessage[]) {
		this.afxMessages = newMessages
		restoreTodoListForTask(this)
		await this.saveAfxMessages()
	}

	private async updateAfxMessage(message: AfxMessage) {
		const provider = this.providerRef.deref()
		await provider?.postMessageToWebview({ type: "messageUpdated", clineMessage: message })
		this.emit(AfxEventName.Message, { action: "updated", message })
	}

	private async saveAfxMessages(): Promise<boolean> {
		try {
			await saveTaskMessages({
				messages: structuredClone(this.afxMessages),
				taskId: this.taskId,
				globalStoragePath: this.globalStoragePath,
			})
			...
			console.error("Failed to save AFX messages:", error)
			return false
		}
	}

	private findMessageByTimestamp(ts: number): AfxMessage | undefined {
		for (let i = this.afxMessages.length - 1; i >= 0; i--) {
			if (this.afxMessages[i].ts === ts) {
				return this.afxMessages[i]
			}
		}
	...
	async ask(
		...
		isProtected?: boolean,
	): Promise<{ response: AfxAskResponse; text?: string; images?: string[] }> {
		...
		if (this.abort) {
			throw new Error(`[AgenticFlowX#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}

		let askTs: number

		if (partial !== undefined) {
			const lastMessage = this.afxMessages.at(-1)
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					this.updateAfxMessage(lastMessage)
					...
					await this.addToAfxMessages({ ts: askTs, type: "ask", ask: type, text, partial, isProtected })
				...
				if (isUpdatingPreviousPartial) {
					...
					askTs = Date.now()
					this.lastMessageTs = askTs
					await this.addToAfxMessages({ ts: askTs, type: "ask", ask: type, text, isProtected })
				}
			}
		} else {
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(AfxEventName.TaskInteractive, this.taskId)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(AfxEventName.TaskResumable, this.taskId)
						}
					}, statusMutationTimeout),
				)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(AfxEventName.TaskIdle, this.taskId)
						}
					}, statusMutationTimeout),
				)
			}
		...
		if (this.idleAsk || this.resumableAsk || this.interactiveAsk) {
			...
			this.emit(AfxEventName.TaskActive, this.taskId)
		}

		this.emit(AfxEventName.TaskAskResponded)
		return result
	}

	handleWebviewAskResponse(askResponse: AfxAskResponse, text?: string, images?: string[]) {
		...
		if (askResponse === "messageResponse" || askResponse === "yesButtonClicked") {
			...
			if (lastFollowUpIndex !== -1) {
				...
				this.afxMessages[lastFollowUpIndex].isAnswered = true
				// Save the updated messages
				this.saveAfxMessages().catch((error) => {
		...
		if (askResponse === "yesButtonClicked") {
			...
			if (lastToolAskIndex !== -1) {
				this.afxMessages[lastToolAskIndex].isAnswered = true
				void this.updateAfxMessage(this.afxMessages[lastToolAskIndex])
				this.saveAfxMessages().catch((error) => {
	...
	private async getFilesReadByAgentSafely(context: string): Promise<string[] | undefined> {
		try {
			...
			console.error(`[Task#${context}] Failed to get files read by AFX:`, error)
			return undefined
		}
	}
	...
	public async condenseContext(): Promise<void> {
		...
		const {
			...
			filesReadByAgent,
			cwd: this.cwd,
			afxIgnoreController: this.afxIgnoreController,
		})
		if (error) {
			await this.say(
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[AgenticFlowX#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	private async startTask(task?: string, images?: string[]): Promise<void> {
		try {
			// `conversationHistory` (for API) and `afxMessages` (for webview)
			// need to be in sync.
			// If the extension process were killed, then on restart the
			// `afxMessages` might not be empty, so we need to set it to [] when
	...
	private async resumeTaskFromHistory() {
		try {
			const modifiedAfxMessages = await this.getSavedAfxMessages()
			...
			if (lastRelevantMessageIndex !== -1) {
				modifiedAfxMessages.splice(lastRelevantMessageIndex + 1)
			}

			// Remove any trailing reasoning-only UI messages that were not part of the persisted API conversation
			while (modifiedAfxMessages.length > 0) {
				const last = modifiedAfxMessages[modifiedAfxMessages.length - 1]
				if (last.type === "say" && last.say === "reasoning") {
					modifiedAfxMessages.pop()
				} else {
					break
				}
			}
			...
			if (lastApiReqStartedIndex !== -1) {
				const lastApiReqStarted = modifiedAfxMessages[lastApiReqStartedIndex]
				const { cost, cancelReason }: AfxApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")

				if (cost === undefined && cancelReason === undefined) {
					modifiedAfxMessages.splice(lastApiReqStartedIndex, 1)
				}
			}

			await this.overwriteAfxMessages(modifiedAfxMessages)
			this.afxMessages = await this.getSavedAfxMessages()
			...
			const lastAfxMessage = this.afxMessages
				.slice()
				.reverse()
				.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // Could be multiple resume tasks.

			let askType: AfxAsk
			if (lastAfxMessage?.ask === "completion_result") {
			...
			if (existingApiConversationHistory.length > 0) {
				...
				if (lastMessage.isSummary) {
					...
					// See: https://github.com/agenticflowx/agenticflowx/issues/11487
			...
			const agoText = ((): string => {
				const timestamp = lastAfxMessage?.ts ?? Date.now()
				const now = Date.now()
	...
	public async abortTask(isAbandoned = false) {
		...
		this.emit(AfxEventName.TaskAborted)
	...
	public dispose(): void {
		...
		try {
			if (this.providerProfileChangeListener) {
				const provider = this.providerRef.deref()
				if (provider) {
					provider.off(AfxEventName.ProviderProfileChanged, this.providerProfileChangeListener)
				}
		...
		try {
			if (this.afxIgnoreController) {
				this.afxIgnoreController.dispose()
				this.afxIgnoreController = undefined
			}
		} catch (error) {
			console.error("Error disposing AfxIgnoreController:", error)
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		this.emit(AfxEventName.TaskStarted)
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			if (this.abort) {
				throw new Error(
					`[AgenticFlowX#recursivelyMakeRooRequests] task ${this.taskId}.${this.instanceId} aborted`,
				)
			}
			...
			const showAfxIgnoredFiles = state?.showAfxIgnoredFiles ?? false
			...
			// message.
			const lastApiReqIndex = findLastIndex(this.afxMessages, (m) => m.say === "api_req_started")

			this.afxMessages[lastApiReqIndex].text = JSON.stringify({
				apiProtocol,
			} satisfies AfxApiReqInfo)
			...
			try {
				...
				// from now).
				const updateApiReqMsg = (cancelReason?: AfxApiReqCancelReason, streamingFailedMessage?: string) => {
					if (lastApiReqIndex < 0 || !this.afxMessages[lastApiReqIndex]) {
						return
					}

					const existingData = JSON.parse(this.afxMessages[lastApiReqIndex].text || "{}")
				...
				const abortStream = async (cancelReason: AfxApiReqCancelReason, streamingFailedMessage?: string) => {
				...
				try {
					...
					const drainStreamInBackgroundToFindAllUsage = async (apiReqIndex: number) => {
						...
						const captureUsageData = async (
							...
							if (
								...
								const apiReqMessage = this.afxMessages[messageIndex]
								if (apiReqMessage) {
									await this.updateAfxMessage(apiReqMessage)
								}
					...
					if (!this.abandoned) {
						...
						const cancelReason: AfxApiReqCancelReason = this.abort ? "user_cancelled" : "streaming_failed"
				...
				if (reasoningMessage) {
					...
					if (lastReasoningIndex !== -1 && this.afxMessages[lastReasoningIndex].partial) {
						this.afxMessages[lastReasoningIndex].partial = false
						await this.updateAfxMessage(this.afxMessages[lastReasoningIndex])
					}
				}
	...
	private async getSystemPrompt(): Promise<string> {
		...
		const afxIgnoreInstructions = this.afxIgnoreController?.getInstructions()
	...
	public async *attemptApiRequest(
		...
		if (contextTokens) {
			...
			// Get files read by AFX for code folding - only when context management will run
		...
		const approvalResult = await this.autoApprovalHandler.checkAutoApprovalLimits(
			state,
			this.combineMessages(this.afxMessages.slice(1)),
	...
	public combineMessages(messages: AfxMessage[]) {
		return combineApiRequests(combineCommandSequences(messages))
	}

	public getTokenUsage(): TokenUsage {
		return getApiMetrics(this.combineMessages(this.afxMessages.slice(1)))
	}
	...
	public recordToolError(toolName: ToolName, error?: string) {
		...
		if (error) {
			this.emit(AfxEventName.TaskToolFailed, this.taskId, toolName, error)
		}
	}
	...
	public get taskAsk(): AfxMessage | undefined {
