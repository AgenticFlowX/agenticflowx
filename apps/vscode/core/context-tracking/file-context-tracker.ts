...
import type { FileMetadataEntry, RecordSource, TaskMetadata } from "./file-context-tracker-types"
import { AfxProvider } from "../webview/afx-provider"

// This class is responsible for tracking file operations that may result in stale context.
// If a user modifies a file outside of AFX, the context may become stale and need to be updated.
// We do not want AFX to reload the context every time a file is modified, so we use this class merely
// to inform AFX that the change has occurred, and tell AFX to reload the file before making
// any changes to it. This fixes an issue with diff editing, where AFX was unable to complete a diff edit.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are passed to AFX via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of AFX, we detect and track this change to prevent stale context.
export class FileContextTracker {
	readonly taskId: string
	private providerRef: WeakRef<AfxProvider>
	...
	constructor(provider: AfxProvider, taskId: string) {
	...
	async setupFileWatcher(filePath: string) {
		...
		watcher.onDidChange(() => {
			if (this.recentlyEditedByAgent.has(filePath)) {
				this.recentlyEditedByAgent.delete(filePath) // This was an edit by AFX, no need to inform AFX
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform AFX
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to AFX via a tool, mention, or edit.
	...
	public getContextProxy(): ContextProxy | undefined {
		...
		if (!provider) {
			console.error("AfxProvider reference is no longer valid")
			return undefined
		}
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: RecordSource) {
		try {
			...
			let newEntry: FileMetadataEntry = {
				...
				afx_read_date: getLatestDateForField(filePath, "afx_read_date"),
				afx_edit_date: getLatestDateForField(filePath, "afx_edit_date"),
			...
			switch (source) {
				...
				// afx_edited: AFX has edited the file
				case "afx_edited":
					newEntry.afx_read_date = now
					newEntry.afx_edit_date = now
					this.checkpointPossibleFiles.add(filePath)
					this.markFileAsEditedByAgent(filePath)
					break

				// read_tool/file_mentioned: AFX has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.afx_read_date = now
					break
	...
	/**
	 * Gets a list of unique file paths that AFX has read during this task.
	...
	async getFilesReadByAgent(sinceTimestamp?: number): Promise<string[]> {
		try {
			...
			const readEntries = metadata.files_in_context.filter((entry) => {
				// Only include files that were read by AFX (not user edits)
				...
				if (sinceTimestamp && entry.afx_read_date) {
					return entry.afx_read_date >= sinceTimestamp
				}

				return true
			})

			// Sort by afx_read_date descending (most recent first)
			// Entries without a date go to the end
			readEntries.sort((a, b) => {
				const dateA = a.afx_read_date ?? 0
				const dateB = b.afx_read_date ?? 0
				return dateB - dateA
			})
			...
			console.error("Failed to get files read by AFX:", error)
			return []
		}
	}
