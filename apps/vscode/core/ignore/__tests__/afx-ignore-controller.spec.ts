...
// npx vitest core/ignore/__tests__/AfxIgnoreController.spec.ts

import type { Mock } from "vitest"

import { AfxIgnoreController, LOCK_TEXT_SYMBOL } from "../afx-ignore-controller"
...
describe("AfxIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: AfxIgnoreController
	let mockFileExists: Mock<typeof fileExistsAtPath>
	let mockAccess: Mock<typeof fs.access>
	let mockReadFile: Mock<typeof fs.readFile>
	let mockWatcher: any

	/**
	 * Helper: simulate .afxignore file existing (for the package's internal fileExistsAtPath).
	 * Sets both the extension's fileExistsAtPath mock AND fs.access to match.
	 */
	function simulateFileExists(exists: boolean) {
		mockFileExists.mockResolvedValue(exists)
		if (exists) {
	...
	beforeEach(() => {
		...
		mockAccess = fs.access as Mock<typeof fs.access>
		mockReadFile = fs.readFile as Mock<typeof fs.readFile>

		// Default: file does not exist
		simulateFileExists(false)
		...
		// Create controller with mock file watcher factory
		controller = new AfxIgnoreController(TEST_CWD, () => mockWatcher)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .afxignore exists
		 */
		it("should load .afxignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .afxignore file
			simulateFileExists(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets.json")

			// Initialize controller
			await controller.initialize()

			// Verify file was checked and read (package uses fs.access internally)
			expect(mockAccess).toHaveBeenCalledWith(path.join(TEST_CWD, ".afxignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".afxignore"), "utf8")

			// Verify content was stored
			expect(controller.afxIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .afxignore doesn't exist
		 */
		it("should allow all access when .afxignore doesn't exist", async () => {
			// Setup mocks to simulate missing .afxignore file
			simulateFileExists(false)
			...
			expect(controller.afxIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .afxignore changes", async () => {
			// With the FileWatcherFactory pattern, the factory returns our mockWatcher.
			// Verify event handlers were registered on the mock watcher.
		 ...
		 */
		it("should handle errors when loading .afxignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .afxignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .afxignore exists
		 */
		it("should allow all access when no .afxignore content", async () => {
			// Create a new controller with no .afxignore
			simulateFileExists(false)
			const emptyController = new AfxIgnoreController(TEST_CWD, () => mockWatcher)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .afxignore exists
		 */
		it("should allow all commands when no .afxignore exists", async () => {
			// Create a new controller with no .afxignore
			simulateFileExists(false)
			const emptyController = new AfxIgnoreController(TEST_CWD, () => mockWatcher)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .afxignore
		 */
		it("should generate formatted instructions when .afxignore exists", async () => {
			// Setup .afxignore content
			simulateFileExists(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .afxignore exists
		 */
		it("should return undefined when no .afxignore exists", async () => {
			// Setup no .afxignore
			simulateFileExists(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .afxignore is created
		 */
		it("should reload .afxignore when file is created", async () => {
			// Setup initial state without .afxignore
			simulateFileExists(false)
			await controller.initialize()
			...
			// Setup for the test
			simulateFileExists(false) // Initially no file exists

			// Create and initialize controller with no .afxignore
			controller = new AfxIgnoreController(TEST_CWD, () => mockWatcher)
			await controller.initialize()
			...
			// Force reload of .afxignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .afxignore is changed
		 */
		it("should reload .afxignore when file is changed", async () => {
			// Setup initial state with .afxignore
			simulateFileExists(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .afxignore is deleted
		 */
		it("should reset when .afxignore is deleted", async () => {
			// Setup initial state with .afxignore
			simulateFileExists(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
			...
			// Simulate file deletion and re-initialize (watcher callback is fire-and-forget)
			simulateFileExists(false)
			await controller.initialize()
