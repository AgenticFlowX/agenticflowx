...
import { ApiMessage } from "../task-persistence/api-messages"
...
export class MessageManager {
	 ...
	 * @throws Error if timestamp not found in afxMessages
	 */
	async rewindToTimestamp(ts: number, options: RewindOptions = {}): Promise<void> {
		...
		// Find the index in afxMessages
		const clineIndex = this.task.afxMessages.findIndex((m) => m.ts === ts)
		if (clineIndex === -1) {
			throw new Error(`Message with timestamp ${ts} not found in afxMessages`)
		}
	...
	/**
	 * Rewind conversation to a specific index in afxMessages.
	...
	async rewindToIndex(toIndex: number, options: RewindOptions = {}): Promise<void> {
		const cutoffTs = this.task.afxMessages[toIndex]?.ts ?? Date.now()
	...
	private async performRewind(toIndex: number, cutoffTs: number, options: RewindOptions): Promise<void> {
		...
		// Step 2: Truncate afxMessages
		await this.truncateAfxMessages(toIndex)
	...
	private collectRemovedContextEventIds(fromIndex: number): ContextEventIds {
		...
		for (let i = fromIndex; i < this.task.afxMessages.length; i++) {
			const msg = this.task.afxMessages[i]
	...
	/**
	 * Truncate afxMessages to the specified index.
	 */
	private async truncateAfxMessages(toIndex: number): Promise<void> {
		await this.task.overwriteAfxMessages(this.task.afxMessages.slice(0, toIndex))
	}
	...
	private async truncateApiHistoryWithCleanup(
		...
		if (!skipCleanup) {
			...
			// Collect timestamps from remaining afxMessages
			for (const msg of this.task.afxMessages) {
				if (msg.ts) {
