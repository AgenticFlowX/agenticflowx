...
describe("Context Management", () => {
	...
	describe("filesReadByAgent parameters", () => {
		...
		it("should pass filesReadByAgent, cwd, and afxIgnoreController to summarizeConversation when provided", async () => {
			// Mock the summarizeConversation function
			...
			const mockAfxIgnoreController = {
				filterPaths: vi.fn(),
			} as unknown as import("../../ignore/afx-ignore-controller").AfxIgnoreController
			...
			const result = await manageContext({
				...
				filesReadByAgent,
				cwd,
				afxIgnoreController: mockAfxIgnoreController,
			})

			// Verify summarizeConversation was called with filesReadByAgent, cwd, and afxIgnoreController
			expect(summarizeSpy).toHaveBeenCalledWith({
				messages: messagesWithSmallContent,
				apiHandler: mockApiHandler,
				systemPrompt: "System prompt",
		...
		it("should pass undefined filesReadByAgent parameters when not provided", async () => {
			...
			const result = await manageContext({
				...
				profileThresholds: {},
				currentProfileId: "default",
				// filesReadByAgent, cwd, afxIgnoreController are NOT provided
			})
