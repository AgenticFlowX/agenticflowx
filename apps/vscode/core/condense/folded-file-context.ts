...
export interface FoldedFileContextOptions {
	...
	cwd: string
	/** Optional AfxIgnoreController for file access validation */
...
export async function generateFoldedFileContext(
	...
	const { maxCharacters = 50000, cwd, afxIgnoreController } = options
	...
	for (let i = 0; i < filePaths.length; i++) {
		...
		try {
			...
			const definitions = await parseSourceCodeDefinitionsForFile(absolutePath, afxIgnoreController)
