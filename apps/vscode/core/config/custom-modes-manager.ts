...
import { type ModeConfig, type PromptComponent, customModesSettingsSchema, modeConfigSchema } from "@agenticflowx/types"

import { fileExistsAtPath } from "../../utils/fs"
import { getWorkspacePath } from "../../utils/path"
import { getGlobalAfxDirectory } from "../../services/afx-config"
import { logger } from "../../utils/logging"
import { GlobalFileNames } from "../../shared/global-file-names"
import { ensureSettingsDirectoryExists } from "../../utils/global-context"
import { t } from "../../i18n"

const AFXMODES_FILENAME = ".afxmodes"
...
export class CustomModesManager {
	...
	private async getWorkspaceAfxmodes(): Promise<string | undefined> {
		...
		const afxmodesPath = path.join(workspaceRoot, AFXMODES_FILENAME)
		const exists = await fileExistsAtPath(afxmodesPath)
		return exists ? afxmodesPath : undefined
	}
	...
	private parseYamlSafely(content: string, filePath: string): any {
		...
		try {
			...
			// For .afxmodes files, try JSON as fallback
			if (filePath.endsWith(AFXMODES_FILENAME)) {
				try {
			...
			// For non-.afxmodes files, just log and return empty object
			const errorMsg = yamlError instanceof Error ? yamlError.message : String(yamlError)
			console.error(`[CustomModesManager] Failed to parse YAML from ${filePath}:`, errorMsg)
			return {}
		}
	...
	private async loadModesFromFile(filePath: string): Promise<ModeConfig[]> {
		try {
			...
			if (!result.success) {
				...
				// Show user-friendly error for .afxmodes files
				if (filePath.endsWith(AFXMODES_FILENAME)) {
			...
			const isAfxmodes = filePath.endsWith(AFXMODES_FILENAME)
			const source = isAfxmodes ? ("project" as const) : ("global" as const)
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		const handleSettingsChange = async () => {
			try {
				...
				// Get modes from .afxmodes if it exists (takes precedence)
				const afxmodesPath = await this.getWorkspaceAfxmodes()
				const afxmodesModes = afxmodesPath ? await this.loadModesFromFile(afxmodesPath) : []

				// Merge modes from both sources (.afxmodes takes precedence)
				const mergedModes = await this.mergeCustomModes(afxmodesModes, result.data.customModes)
		...
		// Watch .afxmodes file - watch the path even if it doesn't exist yet
		const workspaceFolders = vscode.workspace.workspaceFolders
		if (workspaceFolders && workspaceFolders.length > 0) {
			const workspaceRoot = getWorkspacePath()
			const afxmodesPath = path.join(workspaceRoot, AFXMODES_FILENAME)
			const afxmodesWatcher = vscode.workspace.createFileSystemWatcher(afxmodesPath)

			const handleAfxmodesChange = async () => {
				try {
					const settingsModes = await this.loadModesFromFile(settingsPath)
					const afxmodesModes = await this.loadModesFromFile(afxmodesPath)
					// .afxmodes takes precedence
					const mergedModes = await this.mergeCustomModes(afxmodesModes, settingsModes)
					await this.context.globalState.update("customModes", mergedModes)
					this.clearCache()
					await this.onUpdate()
				} catch (error) {
					console.error(`[CustomModesManager] Error handling .afxmodes file change:`, error)
				}
			}

			this.disposables.push(afxmodesWatcher.onDidChange(handleAfxmodesChange))
			this.disposables.push(afxmodesWatcher.onDidCreate(handleAfxmodesChange))
			this.disposables.push(
				afxmodesWatcher.onDidDelete(async () => {
					// When .afxmodes is deleted, refresh with only settings modes
					try {
						const settingsModes = await this.loadModesFromFile(settingsPath)
						...
						console.error(`[CustomModesManager] Error handling .afxmodes file deletion:`, error)
					}
				}),
			)
			this.disposables.push(afxmodesWatcher)
		}
	}
	...
	public async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .afxmodes if it exists.
		const afxmodesPath = await this.getWorkspaceAfxmodes()
		const afxmodesModes = afxmodesPath ? await this.loadModesFromFile(afxmodesPath) : []
		...
		for (const mode of afxmodesModes) {
		...
		const mergedModes = [
			...afxmodesModes.map((mode) => ({ ...mode, source: "project" as const })),
			...settingsModes
	...
	public async updateCustomMode(slug: string, config: ModeConfig): Promise<void> {
		try {
			...
			if (isProjectMode) {
				...
				const workspaceRoot = getWorkspacePath()
				targetPath = path.join(workspaceRoot, AFXMODES_FILENAME)
				const exists = await fileExistsAtPath(targetPath)

				logger.info(`${exists ? "Updating" : "Creating"} project mode in ${AFXMODES_FILENAME}`, {
					slug,
	...
	public async deleteCustomMode(slug: string, fromMarketplace = false): Promise<void> {
		try {
			...
			const projectMode = afxmodesModes.find((m) => m.slug === slug)
			...
			await this.queueWrite(async () => {
				...
				if (projectMode && afxmodesPath) {
					await this.updateModesInFile(afxmodesPath, (modes) => modes.filter((m) => m.slug !== slug))
				}
	...
	private async deleteRulesFolder(slug: string, mode: ModeConfig, fromMarketplace = false): Promise<void> {
		try {
			...
			if (scope === "project") {
				...
				if (workspacePath) {
					rulesFolderPath = path.join(workspacePath, ".afx", `rules-${slug}`)
				} else {
				...
				rulesFolderPath = path.join(homeDir, ".afx", `rules-${slug}`)
			}
	...
	/**
	 * Checks if a mode has associated rules files in the .afx/rules-{slug}/ directory
	...
	public async checkRulesDirectoryHasContent(slug: string): Promise<boolean> {
		try {
			...
			if (!mode) {
				// If not in custom modes, check if it's in .afxmodes (project-specific)
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}

				const afxmodesPath = path.join(workspacePath, AFXMODES_FILENAME)
				try {
					const afxmodesExists = await fileExistsAtPath(afxmodesPath)
					if (afxmodesExists) {
						const afxmodesContent = await fs.readFile(afxmodesPath, "utf-8")
						const afxmodesData = yaml.parse(afxmodesContent)
						const afxmodesModes = afxmodesData?.customModes || []

						// Check if this specific mode exists in .afxmodes
						const modeInAfxmodes = afxmodesModes.find((m: any) => m.slug === slug)
						if (!modeInAfxmodes) {
							return false // Mode not found anywhere
						}
					} else {
						return false // No .afxmodes file and not in custom modes
					}
				} catch (error) {
					return false // Cannot read .afxmodes and not in custom modes
				}
			}
			...
			if (isGlobalMode) {
				// For global modes, check in global .afx directory
				const globalAfxDir = getGlobalAfxDirectory()
				modeRulesDir = path.join(globalAfxDir, `rules-${slug}`)
			} else {
				// For project modes, check in workspace .afx directory
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}
				modeRulesDir = path.join(workspacePath, ".afx", `rules-${slug}`)
			}
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
				...
				if (workspacePath) {
					...
					try {
						...
						if (afxmodesExists) {
							...
							// Find the mode in .afxmodes
							mode = afxmodesModes.find((m: any) => m.slug === slug)
						}
					} catch (error) {
			...
			let baseDir: string
			if (isGlobalMode) {
				// For global modes, use the global .afx directory
				baseDir = getGlobalAfxDirectory()
			} else {
			...
			// Check for .afx/rules-{slug}/ directory (or rules-{slug}/ for global)
			const modeRulesDir = isGlobalMode
				? path.join(baseDir, `rules-${slug}`)
				: path.join(baseDir, ".afx", `rules-${slug}`)
	...
	private async importRulesFiles(
		...
		if (source === "global") {
			...
			const workspacePath = getWorkspacePath()
			baseDir = path.join(workspacePath, ".afx")
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
