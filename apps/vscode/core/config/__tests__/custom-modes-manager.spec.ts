...
import { CustomModesManager } from "../custom-modes-manager"
...
describe("CustomModesManager", () => {
	...
	const mockAfxmodes = path.join(mockWorkspacePath, ".afxmodes")
	...
	beforeEach(() => {
		...
		;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
			return path === mockSettingsPath || path === mockAfxmodes
		})
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .afxmodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const afxmodesModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: settingsModes })
				}
				if (path === mockAfxmodes) {
					return yaml.stringify({ customModes: afxmodesModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .afxmodes taking precedence", async () => {
			const settingsModes = [
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from afxmodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .afxmodes since it takes precedence
		...
		it("should handle missing .afxmodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .afxmodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .afxmodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .afxmodes precedence", async () => {
			...
			let afxmodesContent = { customModes: afxmodesModes }

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockAfxmodes) {
					return yaml.stringify(afxmodesContent)
				}
				if (path === mockSettingsPath) {
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockAfxmodes) {
					afxmodesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .afxmodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Afxmodes Mode 1", // .afxmodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .afxmodes file when adding project-specific mode", async () => {
			...
			// Mock .afxmodes to not exist initially
			let afxmodesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .afxmodes was created with the project mode
			...
			const writeCall = (fs.writeFile as Mock).mock.calls[0]
			expect(path.normalize(writeCall[0])).toBe(path.normalize(mockAfxmodes))

			// Verify the content written to .afxmodes
			expect(afxmodesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				;(fs.readFile as Mock).mockImplementation(async (path: string) => {
					...
					if (path === mockAfxmodes && afxmodesContent) {
						return yaml.stringify(afxmodesContent)
					}
					throw new Error("File not found")
				})
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".afxmodes"),
			...
			it("should import multiple modes at once", async () => {
				...
				expect(result.success).toBe(true)
				expect(afxmodesContent.customModes).toHaveLength(2)
				expect(afxmodesContent.customModes[0].slug).toBe("mode1")
				expect(afxmodesContent.customModes[1].slug).toBe("mode2")
			})
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .afxmodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .afx directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".afxmodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".afx"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".afx", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .afxmodes file", async () => {
			const afxmodesContent = { customModes: [{ slug: "other-mode", name: "Other Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockAfxmodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockAfxmodes) {
					return yaml.stringify(afxmodesContent)
		...
		it("should return false when .afxmodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when rules directory doesn't exist", async () => {
			const afxmodesContent = { customModes: [{ slug: "test-mode", name: "Test Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockAfxmodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.afx/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .afxmodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .afxmodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .afxmodes", async () => {
			const afxmodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .afxmodes", async () => {
			const afxmodesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.afx/rules-code" },
			])
		...
		it("should successfully export global mode with rules from global .afx directory", async () => {
			// Mock a global mode
			const globalMode = {
