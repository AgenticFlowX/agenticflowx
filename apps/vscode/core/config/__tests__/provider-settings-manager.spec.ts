...
import { ProviderSettingsManager, ProviderProfiles, SyncCloudProfilesResult } from "../provider-settings-manager"
...
describe("ProviderSettingsManager", () => {
	...
	describe("initialize", () => {
		...
		// Model migration tests removed: MODEL_MIGRATIONS was emptied as part of
		// the roo -> afx transition. No model ID migrations are currently active.
	...
	describe("SaveConfig", () => {
		it("should save new config", async () => {
			...
			expect(mockSecrets.store.mock.calls[0][0]).toEqual("agenticflowx_api_config")
	...
	describe("ResetAllConfigs", () => {
		it("should delete all stored configs", async () => {
			...
			expect(mockSecrets.delete).toHaveBeenCalledWith("agenticflowx_api_config")
		})
	})
