...
describe("CustomModesManager - Export/Import with Slug Changes", () => {
	...
	describe("Import with Slug Changes", () => {
		it("should place files in rules-{new-slug} folder when slug is changed", async () => {
			...
			const rule1Path = Object.keys(writtenFiles).find((p) => p.includes("rule1.md") && !p.includes(".afxmodes"))
			const rule2Path = Object.keys(writtenFiles).find((p) => p.includes("rule2.md") && !p.includes(".afxmodes"))

			expect(rule1Path).toBeDefined()
			expect(rule2Path).toBeDefined()

			// Check that files are in rules-new-slug-name folder
			expect(rule1Path).toContain(path.join(".afx", "rules-new-slug-name", "rule1.md"))
			expect(rule2Path).toContain(path.join(".afx", "rules-new-slug-name", "subfolder", "rule2.md"))
		...
		it("should handle mixed format paths correctly", async () => {
			...
			expect(oldFormatPath).toContain(path.join(".afx", "rules-mixed-mode", "old-format.md"))
			expect(newFormatPath).toContain(path.join(".afx", "rules-mixed-mode", "new-format.md"))
			expect(nestedPath).toContain(path.join(".afx", "rules-mixed-mode", "nested", "file.md"))
		})
	})
	...
	describe("End-to-End Export/Import with Slug Change", () => {
		it("should successfully export and re-import with a different slug", async () => {
			...
			const ruleFilePath = Object.keys(writtenFiles).find(
				(p) => p.includes("rule.md") && !p.includes(".afxmodes"),
			)
			expect(ruleFilePath).toBeDefined()
			expect(ruleFilePath).toContain(path.join(".afx", "rules-renamed-mode", "rule.md"))
