...
import type { ProviderName } from "@agenticflowx/types"
import { TelemetryService } from "@agenticflowx/telemetry"

import { importSettings, importSettingsFromFile, importSettingsWithFeedback, exportSettings } from "../import-export"
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/agenticflowx-settings.json",
			})
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/agenticflowx-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(
				path.join("/mock/home", "Downloads", "agenticflowx-settings.json"),
			)
		})
