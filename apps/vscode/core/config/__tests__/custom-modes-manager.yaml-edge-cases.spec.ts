...
describe("CustomModesManager - YAML Edge Cases", () => {
	...
	const mockAfxmodes = `${path.sep}mock${path.sep}workspace${path.sep}.afxmodes`
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithNonBreakingSpaces,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithZeroWidth,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should normalize various quote characters", async () => {
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithFancyQuotes,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Complex fileRegex handling", () => {
		it("should handle complex fileRegex syntax gracefully", async () => {
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithComplexFileRegex,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("UTF-8 encoding", () => {
		...
		it("should handle various international characters", async () => {
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithInternational,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Additional edge cases", () => {
		it("should handle mixed line endings (CRLF vs LF)", async () => {
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithMixedLineEndings,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle multiple BOMs in sequence", async () => {
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithMultipleBOMs,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle deeply nested structures with edge case characters", async () => {
			...
			mockFsReadFile({
				[mockAfxmodes]: yamlWithComplexNesting,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
