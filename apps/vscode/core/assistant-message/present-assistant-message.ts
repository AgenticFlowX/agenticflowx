...
import type { ToolName, AfxAsk, ToolProgressStatus } from "@agenticflowx/types"
import { ConsecutiveMistakeError, TelemetryEventName } from "@agenticflowx/types"
...
import { AskIgnoredError } from "../task/ask-ignored-error"
import { Task } from "../task/task"

import { listFilesTool } from "../tools/list-files-tool"
import { readFileTool } from "../tools/read-file-tool"
import { readCommandOutputTool } from "../tools/read-command-output-tool"
import { writeToFileTool } from "../tools/write-to-file-tool"
...
import { useMcpToolTool } from "../tools/use-mcp-tool-tool"
import { accessMcpResourceTool } from "../tools/access-mcp-resource-tool"
import { askFollowupQuestionTool } from "../tools/ask-followup-question-tool"
import { switchModeTool } from "../tools/switch-mode-tool"
import { attemptCompletionTool, AttemptCompletionCallbacks } from "../tools/attempt-completion-tool"
import { newTaskTool } from "../tools/new-task-tool"
import { updateTodoListTool } from "../tools/update-todo-list-tool"
import { runSlashCommandTool } from "../tools/run-slash-command-tool"
import { skillTool } from "../tools/skill-tool"
import { applyDiffTool as applyDiffToolClass } from "../tools/apply-diff-tool"
import { isValidToolName, validateToolUse } from "../tools/validate-tool-use"
		...
		case "tool_use": {
			...
			if (!block.partial) {
				...
				if (!repetitionCheck.allowExecution && repetitionCheck.askUser) {
					...
					const { response, text, images } = await cline.ask(
						repetitionCheck.askUser.messageKey as AfxAsk,
				...
				default: {
					...
					if (customTool) {
						try {
							...
							// [AFX-START] Hook: notify on tool execution — @see docs/research/res-afx-plugin-architecture-plan.md
							try {
								const { getGlobalAfxManager } = await import("../../agenticflowx")
								getGlobalAfxManager()?.hookEngine?.onToolExecuted(
									block.name,
