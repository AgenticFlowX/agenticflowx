...
import type { ReadFileParams, ReadFileMode, ReadFileToolParams, FileEntry, LineRange } from "@agenticflowx/types"
import { isLegacyReadFileParams, type AfxSayTool } from "@agenticflowx/types"
...
export class ReadFileTool extends BaseTool<"read_file"> {
	...
	private async requestApproval(
		...
		if (filesToApprove.length > 1) {
			...
			const completeMessage = JSON.stringify({ tool: "readFile", batchFiles } satisfies AfxSayTool)
