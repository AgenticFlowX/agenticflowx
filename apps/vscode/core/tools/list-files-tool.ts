...
export class ListFilesTool extends BaseTool<"list_files"> {
	...
	async execute(params: ListFilesParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const { showAfxIgnoredFiles = false } = (await task.providerRef.deref()?.getState()) ?? {}

			const result = formatResponse.formatFilesList(
				absolutePath,
				files,
				didHitLimit,
				task.afxIgnoreController,
				showAfxIgnoredFiles,
				task.afxProtectedController,
			)
			...
			const completeMessage = JSON.stringify({ ...sharedMessageProps, content: result } satisfies AfxSayTool)
	...
	override async handlePartial(task: Task, block: ToolUse<"list_files">): Promise<void> {
		...
		const partialMessage = JSON.stringify({ ...sharedMessageProps, content: "" } satisfies AfxSayTool)
