...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("agenticflowx")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (agenticflowx-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "agenticflowx-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("agenticflowx-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
