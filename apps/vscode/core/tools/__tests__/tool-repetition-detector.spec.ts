...
import type { ToolName } from "@agenticflowx/types"

import type { ToolUse } from "../../../shared/tools"

import { ToolRepetitionDetector } from "../tool-repetition-detector"
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key, options) => {
		...
		if (key === "tools:toolRepetitionLimitReached" && options?.toolName) {
			return `AFX appears to be stuck in a loop, attempting the same action (${options.toolName}) repeatedly. This might indicate a problem with its current strategy.`
		}
		return key
	}),
}))
