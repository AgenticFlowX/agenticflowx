...
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		...
		afxIgnoreError: vi.fn((path) => `Access denied: ${path}`),
...
describe("writeToFileTool", () => {
	...
	async function executeWriteFileTool(
		...
		mockCline.afxIgnoreController.validateAccess.mockReturnValue(accessAllowed)
	...
	describe("access control", () => {
		it("validates and allows access when afxIgnoreController permits", async () => {
			await executeWriteFileTool({}, { accessAllowed: true })

			expect(mockCline.afxIgnoreController.validateAccess).toHaveBeenCalledWith(testFilePath)
	...
	describe("file operations", () => {
		it("successfully creates new files with full workflow", async () => {
			...
			expect(mockCline.fileContextTracker.trackFileContext).toHaveBeenCalledWith(testFilePath, "afx_edited")
