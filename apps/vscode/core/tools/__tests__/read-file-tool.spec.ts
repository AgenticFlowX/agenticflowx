...
import { readFileTool, ReadFileTool } from "../read-file-tool"
...
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		...
		afxIgnoreError: vi.fn(
			(filePath: string) =>
				`Access to ${filePath} is blocked by the .afxignore file settings. You must try to continue in the task without using this file, or ask the user to update the .afxignore file.`,
		),
...
interface MockTaskOptions {
	...
	afxIgnoreAllowed?: boolean
	maxImageFileSize?: number
	maxTotalImageSize?: number
}

function createMockTask(options: MockTaskOptions = {}) {
	const { supportsImages = false, afxIgnoreAllowed = true, maxImageFileSize = 5, maxTotalImageSize = 20 } = options
	...
	return {
		...
		afxIgnoreController: {
			validateAccess: vi.fn().mockReturnValue(afxIgnoreAllowed),
		},
		fileContextTracker: {
...
describe("ReadFileTool", () => {
	...
	describe("AfxIgnore handling", () => {
		it("should block access to afxignore-protected files", async () => {
			const mockTask = createMockTask({ afxIgnoreAllowed: false })
			const callbacks = createMockCallbacks()

			await readFileTool.execute({ path: "secret.env" }, mockTask as any, callbacks)

			expect(mockTask.say).toHaveBeenCalledWith("afxignore_error", "secret.env")
			expect(formatResponse.afxIgnoreError).toHaveBeenCalledWith("secret.env")
			expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("blocked by the .afxignore"))
		})
	})
