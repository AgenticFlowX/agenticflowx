...
import type { ToolUsage } from "@agenticflowx/types"
...
vitest.mock("../../../integrations/terminal/terminal-registry", () => ({
	TerminalRegistry: {
...
// Import the module
import * as executeCommandModule from "../execute-command-tool"
...
describe("executeCommandTool", () => {
	...
	const originalCliRuntime = process.env.AFX_CLI_RUNTIME
	...
	afterEach(() => {
		process.env.AFX_CLI_RUNTIME = originalCliRuntime
	})
	...
	describe("Error handling", () => {
		...
		it("should handle afxignore validation failures", async () => {
			// Setup
			...
			mockCline.afxIgnoreController = {
				validateCommand: validateCommandMock,
			}

			const mockAfxIgnoreError = "AfxIgnore error"
			;(formatResponse.afxIgnoreError as any).mockReturnValue(mockAfxIgnoreError)
			...
			expect(mockCline.say).toHaveBeenCalledWith("afxignore_error", ".env")
			expect(formatResponse.afxIgnoreError).toHaveBeenCalledWith(".env")
			expect(mockPushToolResult).toHaveBeenCalledWith(mockAfxIgnoreError)
			expect(mockAskApproval).not.toHaveBeenCalled()
			// executeCommandInTerminal should not be called since afxignore blocked it
		})
	})
