...
import { ExecuteCommandOptions } from "../execute-command-tool"
import { TerminalRegistry } from "../../../integrations/terminal/terminal-registry"
import { Terminal } from "../../../integrations/terminal/terminal"
import { ExecaTerminal } from "../../../integrations/terminal/execa-terminal"
import type { AfxTerminalCallbacks } from "../../../integrations/terminal/types"
...
vitest.mock("../../../integrations/terminal/execa-terminal")

// Import the actual executeCommand function (not mocked)
import { executeCommandInTerminal } from "../execute-command-tool"
...
describe("executeCommand", () => {
	...
	describe("Working Directory Behavior", () => {
		it("should use terminal.getCurrentWorkingDirectory() in the output message for completed commands", async () => {
			...
			mockTerminal.runCommand.mockImplementation((command: string, callbacks: AfxTerminalCallbacks) => {
		...
		it("should use terminal.getCurrentWorkingDirectory() for VSCode Terminal with shell integration", async () => {
			...
			mockVSCodeTerminal.runCommand = vitest
				.fn()
				.mockImplementation((command: string, callbacks: AfxTerminalCallbacks) => {
					setTimeout(() => {
						callbacks.onCompleted("Command output", mockProcess)
						callbacks.onShellExecutionComplete({ exitCode: 0 }, mockProcess)
					}, 0)
	...
	describe("Terminal Working Directory Updates", () => {
		it("should update working directory when terminal returns different cwd", async () => {
			...
			const mockTerminalInstance = {
				...
				runCommand: vitest.fn().mockImplementation((command: string, callbacks: AfxTerminalCallbacks) => {
					setTimeout(() => {
