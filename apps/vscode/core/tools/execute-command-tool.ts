...
import { ExitCodeDetails, AfxTerminalCallbacks, AfxTerminalProcess } from "../../integrations/terminal/types"
...
export function resolveAgentTimeoutMs(timeoutSeconds: number | null | undefined): number {
	...
	// background timeouts.
	return process.env.AFX_CLI_RUNTIME === "1" ? 0 : requestedAgentTimeout
}
...
export class ExecuteCommandTool extends BaseTool<"execute_command"> {
	...
	async execute(params: ExecuteCommandParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const ignoredFileAttemptedToAccess = task.afxIgnoreController?.validateCommand(canonicalCommand)

			if (ignoredFileAttemptedToAccess) {
				await task.say("afxignore_error", ignoredFileAttemptedToAccess)
				pushToolResult(formatResponse.afxIgnoreError(ignoredFileAttemptedToAccess))
				return
			}
...
export async function executeCommandInTerminal(
	...
	const callbacks: AfxTerminalCallbacks = {
		onLine: async (lines: string, process: AfxTerminalProcess) => {
