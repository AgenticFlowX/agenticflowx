...
import { AfxEventName, type HistoryItem } from "@agenticflowx/types"
...
export class AttemptCompletionTool extends BaseTool<"attempt_completion"> {
	...
	override async handlePartial(task: Task, block: ToolUse<"attempt_completion">): Promise<void> {
		...
		const lastMessage = task.afxMessages.at(-1)
	...
	private emitTaskCompleted(task: Task): void {
		...
		task.emit(AfxEventName.TaskCompleted, task.taskId, task.getTokenUsage(), task.toolUsage)

		// [AFX-START] Hook: notify on task completion — @see docs/research/res-afx-plugin-architecture-plan.md
		import("../../agenticflowx")
			.then(({ getGlobalAfxManager }) => {
				const filesModified = task.fileContextTracker?.getAndClearCheckpointPossibleFile() ?? []
				getGlobalAfxManager()?.hookEngine?.onTaskCompleted(task.taskId, filesModified)
			})
			.catch(() => {})
		// [AFX-END]
	}
