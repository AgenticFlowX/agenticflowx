...
import { AfxIgnoreController } from "../ignore/afx-ignore-controller"
import { getCommand, type Command } from "../../services/command/commands"
import { buildSkillResult, resolveSkillContentForMode, type SkillLookup } from "../../services/skills/skill-invocation"
...
// [AFX-START] Extract AFX mention context for spec actions — @see docs/research/res-afx-plugin-architecture-plan.md
/**
 * Extract the full @afx-specs mention with any action keywords that follow it.
 * E.g., "@afx-specs review auth" → "afx-specs review auth"
 */
function extractAfxMentionContext(originalText: string, mention: string): string {
	const idx = originalText.indexOf(`@${mention}`)
	if (idx === -1) return mention
	const afterMention = originalText.substring(idx + mention.length + 1).trim()
	const actionMatch = afterMention.match(/^(discuss|review|create|validate|diff)\s*(.*?)(?:\n|$)/i)
	if (actionMatch) {
		return `${mention} ${actionMatch[1]} ${actionMatch[2]}`.trim()
	}
	return mention
}
// [AFX-END]
...
export async function parseMentions(
	...
	showAfxIgnoredFiles: boolean = false,
	...
	parsedText = parsedText.replace(mentionRegexGlobal, (match, mention) => {
		...
		} else if (mention === "afx-specs" || mention.startsWith("afx-specs#")) {
			// [AFX] @afx-specs mention text replacement (with optional #drill-down)
			const drillPath = mention.includes("#") ? mention.substring("afx-specs#".length) : ""
			return drillPath
				? `AFX Spec: ${drillPath.replace(/#/g, " → ")} (see below for content)`
				: `AFX Spec Context (see below for spec overview)`
		}
		return match
	})
	...
	for (const mention of mentions) {
		if (mention.startsWith("/")) {
			...
			// [AFX-START] Handle @afx-specs mention (with optional #drill-down) — @see docs/research/res-afx-plugin-architecture-plan.md
		} else if (mention === "afx-specs" || mention.startsWith("afx-specs#")) {
			try {
				const { getAfxMentionContent, getAfxDrillDownContent } = await import(
					"../../agenticflowx/mentions/spec-mention"
				)

				let specContent: string
				if (mention.startsWith("afx-specs#")) {
					// Drill-down: resolve @afx-specs#auth#design#jwt-tokens
					specContent = await getAfxDrillDownContent(cwd, mention.substring("afx-specs#".length))
				} else {
					// Plain or action mention
					const mentionWithContext = extractAfxMentionContext(text, mention)
					specContent = await getAfxMentionContent(cwd, mentionWithContext)
				}
				parsedText += `\n\n<afx_spec_context>\n${specContent}\n</afx_spec_context>`
			} catch (error: any) {
				parsedText += `\n\n<afx_spec_context>\nError fetching spec context: ${error?.message ?? String(error)}\n</afx_spec_context>`
			}
		}
		// [AFX-END]
	}
...
async function getFileOrFolderContentWithMetadata(
	...
	try {
		...
		if (stats.isFile()) {
			...
			if (afxIgnoreController && !afxIgnoreController.validateAccess(unescapedPath)) {
				return {
					type: "file",
					path: mentionPath,
					content: `[read_file for '${mentionPath}']\nNote: File is ignored by .afxignore.`,
				}
			}
			try {
			...
			for (let index = 0; index < entries.length; index++) {
				...
				if (afxIgnoreController) {
					isIgnored = !afxIgnoreController.validateAccess(entryPath)
				}

				if (isIgnored && !showAfxIgnoredFiles) {
					continue
				}
...
export { processUserContentMentions } from "./process-user-content-mentions"
export type { ProcessUserContentMentionsResult } from "./process-user-content-mentions"
