...
} from "../tools/helpers/image-helpers"
...
export interface ResolveImageMentionsOptions {
	...
	images?: string[]
	cwd: string
	afxIgnoreController?: { validateAccess: (filePath: string) => boolean }
 ...
 * - Respects `.afxignore` via `afxIgnoreController.validateAccess` when provided
 */
export async function resolveImageMentions({
	...
	for (const mention of imageMentions) {
		...
		if (afxIgnoreController && !afxIgnoreController.validateAccess(relPath)) {
			continue
		}
