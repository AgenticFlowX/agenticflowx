...
import { getEnvironmentDetails } from "../get-environment-details"
import { getFullModeDetails } from "../../../shared/modes"
import { isToolAllowedForMode } from "../../tools/validate-tool-use"
...
vi.mock("../../../shared/get-api-metrics")
...
vi.mock("../../tools/validate-tool-use")
...
describe("getEnvironmentDetails", () => {
	...
	beforeEach(() => {
		...
		mockCline = {
			...
			afxIgnoreController: {
				...
				dispose: vi.fn(),
			} as unknown as AfxIgnoreController,
			afxMessages: [],
			api: {
			...
			} as unknown as WeakRef<AfxProvider>,
		}
	...
	it("should return basic environment details", async () => {
		...
		expect(getApiMetrics).toHaveBeenCalledWith(mockCline.afxMessages)
	})
