...
export async function getEnvironmentDetails(cline: Task, includeFileDetails: boolean = false) {
	...
	// Filter paths through afxIgnoreController
	const allowedVisibleFiles = cline.afxIgnoreController
		? cline.afxIgnoreController.filterPaths(visibleFilePaths)
	...
	// Filter paths through afxIgnoreController
	const allowedOpenTabs = cline.afxIgnoreController
		? cline.afxIgnoreController.filterPaths(openTabPaths)
	...
	if (includeCurrentCost) {
		const { totalCost } = getApiMetrics(cline.afxMessages)
	...
	if (includeFileDetails) {
		...
		if (isDesktop) {
			...
			if (maxFiles === 0) {
				...
				const { showAfxIgnoredFiles = false } = state ?? {}

				const result = formatResponse.formatFilesList(
					cline.cwd,
					files,
					didHitLimit,
					cline.afxIgnoreController,
					showAfxIgnoredFiles,
				)
