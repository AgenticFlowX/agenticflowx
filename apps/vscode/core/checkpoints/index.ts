...
import type { AfxApiReqInfo } from "@agenticflowx/types"
...
export async function checkpointRestore(
	...
	const index = task.afxMessages.findIndex((m) => m.ts === ts)
	...
	try {
		...
		if (mode === "restore") {
			...
			const deletedMessages = task.afxMessages.slice(index + 1)
...
export async function checkpointDiff(task: Task, { ts, previousCommitHash, commitHash, mode }: CheckpointDiffOptions) {
	...
	const checkpoints = task.afxMessages.filter(({ say }) => say === "checkpoint_saved").map(({ text }) => text!)
