 ...
 */
export async function handleRequestSkills(provider: AfxProvider): Promise<SkillMetadata[]> {
	try {
 ...
 */
export async function handleOpenSkillFile(provider: AfxProvider, message: WebviewMessage): Promise<void> {
	try {
		const skillName = message.skillName
		const source = message.source as SkillSource
