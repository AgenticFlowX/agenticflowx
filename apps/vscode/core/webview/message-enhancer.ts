...
import { ProviderSettings, AfxMessage, GlobalState, TelemetryEventName } from "@agenticflowx/types"
...
export class MessageEnhancer {
	...
	static async enhanceMessage(options: MessageEnhancerOptions): Promise<MessageEnhancerResult> {
		try {
			...
			if (includeTaskHistoryInEnhance && currentAfxMessages && currentAfxMessages.length > 0) {
				const taskHistory = this.extractTaskHistory(currentAfxMessages)
				if (taskHistory) {
	 ...
	 */
	private static extractTaskHistory(messages: AfxMessage[]): string {
		try {
