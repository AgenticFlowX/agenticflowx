...
// npx vitest run core/webview/__tests__/AfxProvider.sticky-profile.spec.ts
...
describe("AfxProvider - Sticky Provider Profile", () => {
	let provider: AfxProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	let mockPostMessage: any
	let originalAfxCliRuntimeEnv: string | undefined

	beforeEach(async () => {
		vi.clearAllMocks()
		taskIdCounter = 0
		originalAfxCliRuntimeEnv = process.env.AFX_CLI_RUNTIME
	...
	afterEach(() => {
		if (originalAfxCliRuntimeEnv === undefined) {
			delete process.env.AFX_CLI_RUNTIME
		} else {
			process.env.AFX_CLI_RUNTIME = originalAfxCliRuntimeEnv
		}
	})
