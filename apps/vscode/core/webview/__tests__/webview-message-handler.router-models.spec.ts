...
describe("webviewMessageHandler - requestRouterModels provider filter", () => {
	...
	it("fetches only requested provider when values.provider is present ('openrouter')", async () => {
		...
		// Only "openrouter" key should be present
		...
		// getModels should have been called exactly once for openrouter
	...
	it("defaults to aggregate fetching when no provider filter is sent", async () => {
		...
		expect(routerModels).toHaveProperty("unbound")
	})
