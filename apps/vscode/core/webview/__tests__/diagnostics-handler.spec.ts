...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("afx-diagnostics-")
		...
		expect(String(writtenPath)).toContain("afx-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with AgenticFlowX Support to diagnose the issue faster",
		)
