...
import { ProviderSettings, AfxMessage } from "@agenticflowx/types"
...
describe("MessageEnhancer", () => {
	...
	describe("enhanceMessage", () => {
		...
		it("should include task history when enabled", async () => {
			const mockAfxMessages: AfxMessage[] = [
			...
			const result = await MessageEnhancer.enhanceMessage({
				...
				currentAfxMessages: mockAfxMessages,
				providerSettingsManager: mockProviderSettingsManager,
			})
		...
		it("should limit task history to last 10 messages", async () => {
			// Create 15 messages
			const mockAfxMessages: AfxMessage[] = Array.from({ length: 15 }, (_, i) => ({
				type: i % 2 === 0 ? "ask" : "say",
		...
		it("should truncate long messages in task history", async () => {
			...
			const mockAfxMessages: AfxMessage[] = [{ type: "ask", text: longText, ts: 1000 }]
	...
	describe("extractTaskHistory", () => {
		...
		it("should handle messages with circular references", () => {
			...
			const messages = [circularMessage] as AfxMessage[]
