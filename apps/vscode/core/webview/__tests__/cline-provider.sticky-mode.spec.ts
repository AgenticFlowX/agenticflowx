...
// npx vitest core/webview/__tests__/AfxProvider.sticky-mode.spec.ts
...
import type { HistoryItem, ProviderName } from "@agenticflowx/types"
...
describe("AfxProvider - Sticky Mode", () => {
	...
	describe("Error handling", () => {
		it("should handle errors gracefully when saving mode fails", async () => {
			...
			vi.spyOn(mockTask as any, "saveAfxMessages").mockRejectedValue(new Error("Save failed"))
	...
	describe("Concurrent mode switches", () => {
		...
		it("should handle mode switches during task save operations", async () => {
			...
			const mockTask = {
				...
				_taskMode: "code",
				emit: vi.fn(),
				saveAfxMessages: vi.fn().mockImplementation(async () => {
					// Simulate slow save
			...
			const savePromise = mockTask.saveAfxMessages()
