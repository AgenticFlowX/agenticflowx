...
import { TelemetryEventName, type TelemetrySetting } from "@agenticflowx/types"
...
describe("Telemetry Settings Tracking", () => {
	...
	describe("TelemetryService.captureTelemetrySettingsChanged", () => {
		it("should accept parameters without error (stubbed no-op)", () => {
			// TelemetryService is fully stubbed - captureTelemetrySettingsChanged is a no-op.
			// Verify it can be called without throwing.
			const service = new (TelemetryService as any)([])
			expect(() => service.captureTelemetrySettingsChanged("enabled", "disabled")).not.toThrow()
		})
	})
})
