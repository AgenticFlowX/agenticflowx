...
// pnpm --filter agenticflowx test core/webview/__tests__/AfxProvider.spec.ts
...
vi.mock("../../../integrations/workspace/workspace-tracker", () => {
	return {
...
describe("AfxProvider", () => {
	...
	beforeEach(() => {
		...
		provider = new AfxProvider(mockContext, mockOutputChannel, "sidebar", new ContextProxy(mockContext))
	...
	test("constructor initializes correctly", () => {
		expect(provider).toBeInstanceOf(AfxProvider)
		// Since getVisibleInstance returns the last instance where view.visible is true
		// @ts-ignore - accessing private property for testing
		provider.view = mockWebviewView
		expect(AfxProvider.getVisibleInstance()).toBe(provider)
	})
	...
	test("dispose is idempotent — second call is a no-op", async () => {
		...
		// dispose body runs only once: log "Disposing AfxProvider..." appears once
		const disposeCalls = (mockOutputChannel.appendLine as ReturnType<typeof vi.fn>).mock.calls.filter(
			([msg]) => typeof msg === "string" && msg.includes("Disposing AfxProvider..."),
		)
	...
	test("handles showAfxIgnoredFiles setting", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as any).mock.calls[0][0]

		// Default value should be false
		expect((await provider.getState()).showAfxIgnoredFiles).toBe(false)

		// Test showAfxIgnoredFiles with true
		await messageHandler({ type: "updateSettings", updatedSettings: { showAfxIgnoredFiles: true } })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showAfxIgnoredFiles", true)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showAfxIgnoredFiles).toBe(true)

		// Test showAfxIgnoredFiles with false
		await messageHandler({ type: "updateSettings", updatedSettings: { showAfxIgnoredFiles: false } })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showAfxIgnoredFiles", false)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showAfxIgnoredFiles).toBe(false)
	})
	...
	describe("deleteMessage", () => {
		...
		test("handles deletion with confirmation dialog", async () => {
			...
			const mockCline = new Task(defaultTaskOptions) // Create a new mocked instance
			mockCline.afxMessages = mockMessages // Set test-specific messages
			...
			expect(mockCline.overwriteAfxMessages).toHaveBeenCalledWith([
				mockMessages[0],
				mockMessages[1],
				mockMessages[2],
			])
	...
	describe("editMessage", () => {
		...
		test("handles edit with confirmation dialog", async () => {
			...
			mockCline.overwriteAfxMessages = vi.fn()
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.afx", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.afx/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.afx/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			expect.stringContaining("Failed to create or open .afx/mcp.json"),
		)
	})
})
...
describe.skip("ContextProxy integration", () => {
	...
	beforeEach(() => {
		...
		provider = new AfxProvider(mockContext, mockOutputChannel, "sidebar", mockContextProxy)
	})
...
describe("AfxProvider - Router Models", () => {
	let provider: AfxProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	...
	test("handles requestRouterModels with successful responses", async () => {
		...
		const { getModels } = await import("../../../api/providers/fetchers/model-cache")
...
describe("AfxProvider - Comprehensive Edit/Delete Edge Cases", () => {
	let provider: AfxProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	...
	describe("Edit Messages with Images and Attachments", () => {
		...
		test("handles editing messages containing images", async () => {
			...
			const mockCline = new Task(defaultTaskOptions)
			mockCline.afxMessages = mockMessages
			...
			expect(mockCline.overwriteAfxMessages).toHaveBeenCalledWith([mockMessages[0]])
		...
		test("handles editing messages with file attachments", async () => {
			...
			expect(mockCline.overwriteAfxMessages).toHaveBeenCalled()
	...
	describe("Network Failure Scenarios", () => {
		...
		test("handles connection drops during edit operation", async () => {
			...
			mockCline.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockCline.overwriteAfxMessages = vi.fn().mockRejectedValue(new Error("Connection lost"))
			mockCline.overwriteApiConversationHistory = vi.fn()
			mockCline.handleWebviewAskResponse = vi.fn()
	...
	describe("Edit Permissions and Authorization", () => {
		...
		test("handles authorization failures during edit", async () => {
			...
			mockCline.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockCline.overwriteAfxMessages = vi.fn().mockRejectedValue(new Error("Unauthorized"))
			mockCline.overwriteApiConversationHistory = vi.fn()
			mockCline.handleWebviewAskResponse = vi.fn()
		...
		describe("Operations on Deleted or Non-existent Messages", () => {
			...
			test("handles edit operations on deleted messages", async () => {
				...
				expect(mockCline.overwriteAfxMessages).not.toHaveBeenCalled()
		...
		describe("Resource Cleanup During Failed Operations", () => {
			...
			test("validates proper cleanup during failed edit operations", async () => {
				...
				mockCline.overwriteAfxMessages = vi.fn().mockImplementation(() => {
					cleanupSpy()
		...
		describe("Large Message Payloads", () => {
			...
			test("handles deleting messages with large payloads", async () => {
				...
				expect(mockCline.overwriteAfxMessages).toHaveBeenCalledWith([mockMessages[0], mockMessages[1]])
