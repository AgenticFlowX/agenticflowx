...
vi.mock("../../../api/providers/fetchers/model-cache")
...
vi.mock("../diagnostics-handler", () => ({
	generateErrorDiagnostics: vi.fn().mockResolvedValue({ success: true, filePath: "/tmp/diagnostics.json" }),
}))

import type { ModelRecord } from "@agenticflowx/types"

import { webviewMessageHandler } from "../webview-message-handler"
import type { AfxProvider } from "../afx-provider"
import { getModels } from "../../../api/providers/fetchers/model-cache"
...
vi.mock("../../../utils/fs")
vi.mock("../../../utils/path")
vi.mock("../../../utils/global-context")

vi.mock("../../mentions/resolve-image-mentions", () => ({
...
describe("webviewMessageHandler - requestLmStudioModels", () => {
	beforeEach(() => {
		vi.clearAllMocks()
		mockAfxProvider.getState = vi.fn().mockResolvedValue({
			apiConfiguration: {
...
describe("webviewMessageHandler - image mentions", () => {
	...
	it("should resolve image mentions for askResponse payloads", async () => {
		...
		vi.mocked(mockAfxProvider.getCurrentTask).mockReturnValue({
			cwd: "/mock/workspace",
			afxIgnoreController: undefined,
...
describe("webviewMessageHandler - requestOpenAiCodexRateLimits", () => {
	...
	it("posts error when not authenticated", async () => {
		await webviewMessageHandler(mockAfxProvider, { type: "requestOpenAiCodexRateLimits" } as any)
...
describe("webviewMessageHandler - deleteCustomMode", () => {
	beforeEach(() => {
		...
		vi.mocked(ensureSettingsDirectoryExists).mockResolvedValue("/mock/global/storage/.afx")
	})

	it("should delete a project mode and its rules folder", async () => {
		const slug = "test-project-mode"
		const rulesFolderPath = path.join("/mock/workspace", ".afx", `rules-${slug}`)

		vi.mocked(mockAfxProvider.customModesManager.getCustomModes).mockResolvedValue([
			{
		...
		vi.mocked(mockAfxProvider.customModesManager.deleteCustomMode).mockResolvedValue(undefined)

		await webviewMessageHandler(mockAfxProvider, { type: "deleteCustomMode", slug })

		// The confirmation dialog is now handled in the webview, so we don't expect showInformationMessage to be called
		expect(vscode.window.showInformationMessage).not.toHaveBeenCalled()
		expect(mockAfxProvider.customModesManager.deleteCustomMode).toHaveBeenCalledWith(slug)
	...
	it("should delete a global mode and its rules folder", async () => {
		...
		const rulesFolderPath = path.join(homeDir, ".afx", `rules-${slug}`)
	...
	it("should handle errors when deleting rules folder", async () => {
		...
		expect(mockAfxProvider.postMessageToWebview).not.toHaveBeenCalled()
	})
})
...
describe("webviewMessageHandler - message dialog preferences", () => {
	beforeEach(() => {
		...
		// Reset getValue mock
		vi.mocked(mockAfxProvider.contextProxy.getValue).mockReturnValue(false)
	})
...
describe("webviewMessageHandler - mcpEnabled", () => {
	...
	beforeEach(() => {
		...
		;(mockAfxProvider as any).getMcpHub = vi.fn().mockReturnValue(mockMcpHub)
	})
	...
	it("delegates enable=true to McpHub and posts updated state", async () => {
		...
		expect((mockAfxProvider as any).getMcpHub).toHaveBeenCalledTimes(1)
		...
		expect(mockAfxProvider.postStateToWebview).toHaveBeenCalledTimes(1)
	})
	...
	it("handles missing McpHub instance gracefully and still posts state", async () => {
		;(mockAfxProvider as any).getMcpHub = vi.fn().mockReturnValue(undefined)
...
describe("webviewMessageHandler - requestCommands", () => {
	...
	it("includes skill slug commands and dedupes duplicate skill names while preserving first skill entry", async () => {
		...
		vi.mocked(mockAfxProvider.getCurrentTask).mockReturnValue({
			cwd: "/mock/workspace",
			getTaskMode,
		} as unknown as ReturnType<AfxProvider["getCurrentTask"]>)

		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				name: "skill-slug-entry",
				description: "Primary skill slug",
				path: "/mock/.afx/skills/skill-slug-entry/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.afx/skills/duplicate-skill/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.afx/skills/another-skill-slug/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
		])

		vi.mocked(mockAfxProvider.getSkillsManager).mockReturnValue({
			getSkillsForMode,
		} as unknown as ReturnType<AfxProvider["getSkillsManager"]>)

		await webviewMessageHandler(mockAfxProvider, { type: "requestCommands" })

		const commandMessageCall = vi
			.mocked(mockAfxProvider.postMessageToWebview)
			...
			expect.arrayContaining([
				{
					name: "skill-slug-entry",
					source: "project",
					filePath: "/mock/.afx/skills/skill-slug-entry/SKILL.md",
					description: "Primary skill slug",
				},
				{
					name: "another-skill-slug",
					source: "global",
					filePath: "/mock/.afx/skills/another-skill-slug/SKILL.md",
					description: "Another skill-generated command",
				},
			]),
		)
		...
		mockGetCommands.mockResolvedValue([
			{
				...
				source: "project",
				filePath: "/mock/workspace/.afx/commands/deploy.md",
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.afx/skills/deploy/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.afx/skills/skill-only/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
		])
		...
		expect(mockAfxProvider.postMessageToWebview).toHaveBeenCalledWith({
			...
			commands: expect.arrayContaining([
				...
				},
				{
					name: "skill-only",
					source: "project",
					filePath: "/mock/.afx/skills/skill-only/SKILL.md",
					description: "Skill-generated command",
				},
			]),
		})
	...
	it("preserves existing behavior when skills manager is unavailable", async () => {
		...
		vi.mocked(mockAfxProvider.getSkillsManager).mockReturnValue(undefined)
...
describe("webviewMessageHandler - downloadErrorDiagnostics", () => {
	beforeEach(() => {
		...
		;(mockAfxProvider as any).contextProxy.globalStorageUri = { fsPath: "/mock/global/storage" }
	...
	it("shows error when no active task", async () => {
		vi.mocked(mockAfxProvider.getCurrentTask).mockReturnValue(null as any)
