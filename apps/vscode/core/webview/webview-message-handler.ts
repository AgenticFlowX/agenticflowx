...
import { getAfxDirectoriesForCwd } from "../../services/afx-config/index.js"
...
import { customToolRegistry } from "@agenticflowx/core"
import { TelemetryService } from "@agenticflowx/telemetry"

import { type ApiMessage } from "../task-persistence/api-messages"
import { saveTaskMessages } from "../task-persistence"

import { AfxProvider } from "./afx-provider"
import { handleCheckpointRestoreOperation } from "./checkpoint-restore-handler"
import { generateErrorDiagnostics } from "./diagnostics-handler"
import {
	handleRequestSkills,
	handleCreateSkill,
	handleDeleteSkill,
	...
	handleOpenSkillFile,
} from "./skills-message-handler"
import { changeLanguage, t } from "../../i18n"
import { Package } from "../../shared/package"
import { type RouterName, toRouterName } from "../../shared/api"
import { MessageEnhancer } from "./message-enhancer"

import { CodeIndexManager } from "../../services/code-index/manager"
import { checkExistKey } from "../../shared/check-exist-api-config"
...
import { exportSettings, importSettingsWithFeedback } from "../config/import-export"
import { getOpenAiModels } from "../../api/providers/openai"
import { getVsCodeLmModels } from "../../api/providers/vscode-lm"
import { openMention } from "../mentions"
import { resolveImageMentions } from "../mentions/resolve-image-mentions"
import { AfxIgnoreController, createVscodeFileWatcher } from "../ignore/afx-ignore-controller"
import { getWorkspacePath } from "../../utils/path"
import { isPathOutsideWorkspace } from "../../utils/path-utils"
import { Mode, defaultModeSlug } from "../../shared/modes"
import { getModels, flushModels } from "../../api/providers/fetchers/model-cache"
...
import { setPendingTodoList } from "../tools/update-todo-list-tool"
import {
	handleListWorktrees,
...
export const webviewMessageHandler = async (
	...
	const getDiscoveredCommands = async (): Promise<SlashCommand[]> => {
		...
		for (const skill of availableSkills) {
			...
			commandList.push({
				...
				argumentHint: skill.argumentHint, // [AFX] Pass argumentHint for skill subcommand hints
			})
		}

		// [AFX-START] Inject hidden AFX commands with feature/section data for parameter autocomplete
		try {
			const { getGlobalAfxManager } = await import("../../agenticflowx")
			const afxManager = getGlobalAfxManager()
			if (afxManager) {
				const features = await (afxManager as any).specsData?.getFeatures()
				if (features?.length > 0) {
					// Feature names for Level 2 autocomplete
					commandList.push({
						name: "__afx-features",
						source: "project",
						description: "",
						argumentHint: features.map((f: any) => f.name).join(","),
					})

					// Section headings + task IDs for Level 3 autocomplete
					// Format: JSON { "feature": { "doc": ["heading1", "heading2", ...] } }
					const sectionsMap: Record<string, Record<string, string[]>> = {}
					for (const f of features) {
						sectionsMap[f.name] = {}
						for (const docType of ["spec", "design", "tasks", "journal"] as const) {
							const doc = f[docType]
							if (!doc) continue
							const items: string[] = []
							...
							// Add task IDs for tasks.md
							if (docType === "tasks" && f.taskStats?.phases) {
								for (const phase of f.taskStats.phases) {
									if (phase.items) {
										for (const item of phase.items) {
											const taskId = item.text.split(" ")[0]
											const status = item.completed ? "✓" : "○"
											items.push(`${taskId}|${status} ${item.text}`)
										}
									}
								}
							}
							...
							if (items.length > 0) {
								sectionsMap[f.name][docType] = items
							}
						}
					}
					...
					commandList.push({
						name: "__afx-sections",
						source: "project",
						description: JSON.stringify(sectionsMap),
					})
				}
			}
		} catch {}
	...
	const resolveIncomingImages = async (payload: { text?: string; images?: string[] }) => {
		...
		const resolved = await resolveImageMentions({
			...
			images,
			cwd: getCurrentCwd(),
			afxIgnoreController: currentTask?.afxIgnoreController,
	...
	const findMessageIndices = (messageTs: number, currentCline: any) => {
		...
		const messageIndex = currentCline.afxMessages.findIndex((msg: AfxMessage) => msg.ts === messageTs)
	...
	const handleDeleteOperation = async (messageTs: number): Promise<void> => {
		...
		if (messageIndex !== -1) {
			...
			const checkpoints = currentCline.afxMessages.filter(
	...
	const handleDeleteMessageConfirm = async (messageTs: number, restoreCheckpoint?: boolean): Promise<void> => {
		...
		const tsThreshold = currentCline.afxMessages[messageIndex]?.ts
		...
		try {
			const targetMessage = currentCline.afxMessages[messageIndex]
			...
			if (restoreCheckpoint) {
				...
				for (let i = 0; i < messageIndex; i++) {
					const msg = currentCline.afxMessages[i]
				...
				for (const [ts, checkpoint] of preservedCheckpoints) {
					const msgIndex = currentCline.afxMessages.findIndex((msg) => msg.ts === ts)
					if (msgIndex !== -1) {
						currentCline.afxMessages[msgIndex].checkpoint = checkpoint
					}
				}

				// Save the updated messages with restored checkpoints
				await saveTaskMessages({
					messages: currentCline.afxMessages,
	...
	const handleEditOperation = async (messageTs: number, editedContent: string, images?: string[]): Promise<void> => {
		...
		if (currentCline) {
			...
			if (messageIndex !== -1) {
				...
				console.log("[webviewMessageHandler] Edit - Message not found in afxMessages!")
			}
		} else {
	...
	const handleEditMessageConfirm = async (
		...
		try {
			...
			for (let i = messageIndex; i >= 0; i--) {
				const m = currentCline.afxMessages[i]
			...
			if (deleteFromApiIndex === -1) {
				const tsThresholdForEdit = currentCline.afxMessages[deleteFromMessageIndex]?.ts
			...
			const rewindTs = currentCline.afxMessages[deleteFromMessageIndex]?.ts
			if (rewindTs) {
	...
	switch (message.type) {
		...
		case "updateSettings":
			if (message.updatedSettings) {
				for (const [key, value] of Object.entries(message.updatedSettings)) {
					...
					await provider.contextProxy.setValue(key as keyof AfxSettings, newValue)
				}
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the AgenticFlowX commands
		...
		case "openProjectMcpSettings": {
			...
			const afxDir = path.join(workspaceFolder, ".afx")
			const mcpPath = path.join(afxDir, "mcp.json")

			try {
				await fs.mkdir(afxDir, { recursive: true })
		...
		case "taskSyncEnabled":
			// Cloud task sync is no longer supported; this is a no-op.
			break
			...
			if (message.text) {
				try {
					...
					const result = await MessageEnhancer.enhanceMessage({
						...
						enhancementApiConfigId,
						includeTaskHistoryInEnhance,
						currentAfxMessages: currentCline?.afxMessages,
						providerSettingsManager: provider.providerSettingsManager,
					})
		...
		case "searchFiles": {
			...
			try {
				...
				// Get the AfxIgnoreController from the current task, or create a new one
				const currentTask = provider.getCurrentTask()
				let afxIgnoreController = currentTask?.afxIgnoreController
				let tempController: AfxIgnoreController | undefined

				// If no current task or no controller, create a temporary one
				if (!afxIgnoreController) {
					tempController = new AfxIgnoreController(workspacePath, createVscodeFileWatcher)
					await tempController.initialize()
					afxIgnoreController = tempController
				}

				try {
					// Get showAfxIgnoredFiles setting from state
					const { showAfxIgnoredFiles = false } = (await provider.getState()) ?? {}

					// Filter results using AfxIgnoreController if showAfxIgnoredFiles is false
					let filteredResults = results
					if (!showAfxIgnoredFiles && afxIgnoreController) {
						const allowedPaths = afxIgnoreController.filterPaths(results.map((r) => r.path))
		...
		case "refreshCustomTools": {
			try {
				const toolDirs = getAfxDirectoriesForCwd(getCurrentCwd()).map((dir) => path.join(dir, "tools"))
		...
		case "deleteCustomMode":
			if (message.slug) {
				...
				if (scope === "project") {
					...
					if (workspacePath) {
						rulesFolderPath = path.join(workspacePath, ".afx", `rules-${message.slug}`)
					} else {
						rulesFolderPath = path.join(".afx", `rules-${message.slug}`)
					}
				} else {
					// Global scope - use OS home directory
					const homeDir = os.homedir()
					rulesFolderPath = path.join(homeDir, ".afx", `rules-${message.slug}`)
				}
		...
		case "createCommand": {
			try {
				...
				if (source === "global") {
					const globalConfigDir = path.join(os.homedir(), ".afx")
					...
					commandsDir = path.join(workspaceRoot, ".afx", "commands")
				}
		...
		case "openMarkdownPreview": {
			if (message.text) {
				try {
					...
					const tempFileName = `afx-preview-${timestamp}.md`
		...
		case "openDebugUiHistory": {
			...
			try {
				...
				const tmpDir = os.tmpdir()
				const timestamp = Date.now()
				const tempFileName = `afx-debug-${message.type === "openDebugApiHistory" ? "api" : "ui"}-${currentTask.taskId.slice(0, 8)}-${timestamp}.json`
				const tempFilePath = path.join(tmpDir, tempFileName)
