...
import { worktreeService, worktreeIncludeService, type CopyProgressCallback } from "@agenticflowx/core"
...
export async function handleListWorktrees(provider: AfxProvider): Promise<WorktreeListResponse> {
...
export async function handleSwitchWorktree(
	...
	try {
		...
		if (newWindow) {
			// Set the auto-open path so the new window opens AgenticFlowX sidebar.
...
export async function handleGetAvailableBranches(provider: AfxProvider): Promise<BranchInfo> {
	const cwd = provider.cwd
...
export async function handleGetWorktreeDefaults(provider: AfxProvider): Promise<WorktreeDefaultsResponse> {
	const suffix = generateRandomSuffix()
	const workspaceFolders = vscode.workspace.workspaceFolders
	const projectName = workspaceFolders?.[0]?.name || "project"

	const dotAfxPath = path.join(os.homedir(), ".afx")
	const suggestedPath = path.join(dotAfxPath, "worktrees", `${projectName}-${suffix}`)
...
export async function handleGetWorktreeIncludeStatus(provider: AfxProvider): Promise<WorktreeIncludeStatus> {
	const cwd = provider.cwd
	return worktreeIncludeService.getStatus(cwd)
}

export async function handleCheckBranchWorktreeInclude(provider: AfxProvider, branch: string): Promise<boolean> {
	const cwd = provider.cwd
	return worktreeIncludeService.branchHasWorktreeInclude(cwd, branch)
}

export async function handleCreateWorktreeInclude(provider: AfxProvider, content: string): Promise<WorktreeResult> {
	const cwd = provider.cwd
...
export async function handleCheckoutBranch(provider: AfxProvider, branch: string): Promise<WorktreeResult> {
	const cwd = provider.cwd
