...
import { aggregateTaskCostsRecursive, type AggregatedCosts } from "./aggregate-task-costs"
import { TelemetryService } from "@agenticflowx/telemetry"
...
import { EMBEDDING_MODEL_PROFILES } from "../../shared/embedding-models"
...
import { getTheme } from "../../integrations/theme/get-theme"
import WorkspaceTracker from "../../integrations/workspace/workspace-tracker"

import { McpHub } from "../../services/mcp/mcp-hub"
import { McpServerManager } from "../../services/mcp/mcp-server-manager"
import { MarketplaceManager } from "../../services/marketplace"
import { ShadowCheckpointService } from "../../services/checkpoints/shadow-checkpoint-service"
import { CodeIndexManager } from "../../services/code-index/manager"
import type { IndexProgressUpdate } from "../../services/code-index/interfaces/manager"
import { SkillsManager } from "../../services/skills/skills-manager"
...
import { setPanel } from "../../activate/register-commands"
...
import { ContextProxy } from "../config/context-proxy"
import { ProviderSettingsManager } from "../config/provider-settings-manager"
import { CustomModesManager } from "../config/custom-modes-manager"
import { Task } from "../task/task"

import { webviewMessageHandler } from "./webview-message-handler"
import type { AfxMessage, TodoItem } from "@agenticflowx/types"
import { readApiMessages, saveApiMessages, saveTaskMessages, TaskHistoryStore } from "../task-persistence"
import { readTaskMessages } from "../task-persistence/task-messages"
import { getNonce } from "./get-nonce"
import { getUri } from "./get-uri"
import { REQUESTY_BASE_URL } from "../../shared/utils/requesty"
import { validateAndFixToolResultIds } from "../task/validate-tool-result-ids"
...
export class AfxProvider
	extends EventEmitter<TaskProviderEvents>
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike, ITaskProvider
{
	...
	private static activeInstances: Set<AfxProvider> = new Set()
	...
	public afxManager?: import("../../agenticflowx").AfxManager // [AFX] AgenticFlowX manager instance
	...
	/**
	 * Monotonically increasing sequence number for afxMessages state pushes.
	 * Used by the frontend to reject stale state that arrives out-of-order.
	 */
	private afxMessagesSeq = 0

	public isViewLaunched = false
	public settingsImportedAt?: number
	public readonly latestAnnouncementId = "afx-v1.0.0" // AgenticFlowX v1.0.0 initial release
	...
	constructor(
		...
		AfxProvider.activeInstances.add(this)
		...
		this.customModesManager = new CustomModesManager(this.context, async () => {
			await this.postStateToWebviewWithoutAfxMessages()
		})
		...
		this.taskCreationCallback = (instance: Task) => {
			this.emit(AfxEventName.TaskCreated, instance)

			// Create named listener functions so we can remove them later.
			const onTaskStarted = () => this.emit(AfxEventName.TaskStarted, instance.taskId)
			const onTaskCompleted = (taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage) =>
				this.emit(AfxEventName.TaskCompleted, taskId, tokenUsage, toolUsage)
			const onTaskAborted = async () => {
				this.emit(AfxEventName.TaskAborted, instance.taskId)
			...
			const onTaskFocused = () => this.emit(AfxEventName.TaskFocused, instance.taskId)
			const onTaskUnfocused = () => this.emit(AfxEventName.TaskUnfocused, instance.taskId)
			const onTaskActive = (taskId: string) => this.emit(AfxEventName.TaskActive, taskId)
			const onTaskInteractive = (taskId: string) => this.emit(AfxEventName.TaskInteractive, taskId)
			const onTaskResumable = (taskId: string) => this.emit(AfxEventName.TaskResumable, taskId)
			const onTaskIdle = (taskId: string) => this.emit(AfxEventName.TaskIdle, taskId)
			const onTaskPaused = (taskId: string) => this.emit(AfxEventName.TaskPaused, taskId)
			const onTaskUnpaused = (taskId: string) => this.emit(AfxEventName.TaskUnpaused, taskId)
			const onTaskSpawned = (taskId: string) => this.emit(AfxEventName.TaskSpawned, taskId)
			const onTaskUserMessage = (taskId: string) => this.emit(AfxEventName.TaskUserMessage, taskId)
			const onTaskTokenUsageUpdated = (taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage) =>
				this.emit(AfxEventName.TaskTokenUsageUpdated, taskId, tokenUsage, toolUsage)

			// Attach the listeners.
			instance.on(AfxEventName.TaskStarted, onTaskStarted)
			instance.on(AfxEventName.TaskCompleted, onTaskCompleted)
			instance.on(AfxEventName.TaskAborted, onTaskAborted)
			instance.on(AfxEventName.TaskFocused, onTaskFocused)
			instance.on(AfxEventName.TaskUnfocused, onTaskUnfocused)
			instance.on(AfxEventName.TaskActive, onTaskActive)
			instance.on(AfxEventName.TaskInteractive, onTaskInteractive)
			instance.on(AfxEventName.TaskResumable, onTaskResumable)
			instance.on(AfxEventName.TaskIdle, onTaskIdle)
			instance.on(AfxEventName.TaskPaused, onTaskPaused)
			instance.on(AfxEventName.TaskUnpaused, onTaskUnpaused)
			instance.on(AfxEventName.TaskSpawned, onTaskSpawned)
			instance.on(AfxEventName.TaskUserMessage, onTaskUserMessage)
			instance.on(AfxEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated)

			// Store the cleanup functions for later removal.
			this.taskEventListeners.set(instance, [
				() => instance.off(AfxEventName.TaskStarted, onTaskStarted),
				() => instance.off(AfxEventName.TaskCompleted, onTaskCompleted),
				() => instance.off(AfxEventName.TaskAborted, onTaskAborted),
				() => instance.off(AfxEventName.TaskFocused, onTaskFocused),
				() => instance.off(AfxEventName.TaskUnfocused, onTaskUnfocused),
				() => instance.off(AfxEventName.TaskActive, onTaskActive),
				() => instance.off(AfxEventName.TaskInteractive, onTaskInteractive),
				() => instance.off(AfxEventName.TaskResumable, onTaskResumable),
				() => instance.off(AfxEventName.TaskIdle, onTaskIdle),
				() => instance.off(AfxEventName.TaskUserMessage, onTaskUserMessage),
				() => instance.off(AfxEventName.TaskPaused, onTaskPaused),
				() => instance.off(AfxEventName.TaskUnpaused, onTaskUnpaused),
				() => instance.off(AfxEventName.TaskSpawned, onTaskSpawned),
				() => instance.off(AfxEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated),
			])
		}
	}
	...
	async addClineToStack(task: Task) {
		...
		task.emit(AfxEventName.TaskFocused)
	...
	async removeClineFromStack(options?: { skipDelegationRepair?: boolean }) {
		...
		if (task) {
			...
			task.emit(AfxEventName.TaskUnfocused)
			...
			try {
				...
				this.log(
					`[AfxProvider#removeClineFromStack] abortTask() failed ${task.taskId}.${task.instanceId}: ${e.message}`,
				)
			}
			...
			if (parentTaskId && childTaskId && !options?.skipDelegationRepair) {
				try {
					...
					if (parentHistory.status === "delegated" && parentHistory.awaitingChildId === childTaskId) {
						...
						this.log(
							`[AfxProvider#removeClineFromStack] Repaired parent ${parentTaskId} metadata: delegated → active (child ${childTaskId} removed)`,
						)
					}
				} catch (err) {
					// Non-fatal: log but do not block the pop operation.
					this.log(
						`[AfxProvider#removeClineFromStack] Failed to repair parent metadata for ${parentTaskId} (non-fatal): ${
	...
	public setPendingEditOperation(
		...
		}, AfxProvider.PENDING_OPERATION_TIMEOUT_MS)
	...
	*/
	...
	async dispose() {
		...
		this._disposed = true
		this.log("Disposing AfxProvider...")
		...
		AfxProvider.activeInstances.delete(this)
	...
	public static getVisibleInstance(): AfxProvider | undefined {
		return findLast(Array.from(this.activeInstances), (instance) => instance.view?.visible === true)
	}

	public static async getInstance(): Promise<AfxProvider | undefined> {
		let visibleProvider = AfxProvider.getVisibleInstance()
		...
		if (!visibleProvider) {
			...
			await delay(100)
			visibleProvider = AfxProvider.getVisibleInstance()
		}
	...
	public static async isActiveTask(): Promise<boolean> {
		const visibleProvider = await AfxProvider.getInstance()
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		webviewView.onDidDispose(
			async () => {
				if (inTabMode) {
					this.log("Disposing AfxProvider instance for tab view")
					await this.dispose()
				} else {
	...
	public async createTaskWithHistoryItem(
		...
		const isCliRuntime = process.env.AFX_CLI_RUNTIME === "1"
			...
			setTimeout(async () => {
				try {
					...
					const { messageIndex, apiConversationHistoryIndex } = (() => {
						const messageIndex = task.afxMessages.findIndex((msg) => msg.ts === pendingEdit.messageTs)
					...
					if (messageIndex !== -1) {
						...
						await task.overwriteAfxMessages(task.afxMessages.slice(0, messageIndex))
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		const stylesUri = getUri(webview, this.contextProxy.extensionUri, ["webview", "build", "assets", "index.css"])
	...
	public async handleModeSwitch(newMode: Mode) {
		...
		if (task) {
			...
			task.emit(AfxEventName.TaskModeSwitched, task.taskId, newMode)
		...
		this.emit(AfxEventName.ModeChanged, newMode)
	...
	async activateProviderProfile(
		...
		if (providerSettings.apiProvider) {
			this.emit(AfxEventName.ProviderProfileChanged, { name, provider: providerSettings.apiProvider })
		}
	}
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\AgenticFlowX\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "AgenticFlowX", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Cline/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "Cline", "MCP")
		} else {
			// Linux: ~/.local/share/Cline/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "AgenticFlowX", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".agenticflowx", "mcp")
		}
	...
	/**
	 * Like postStateToWebview but intentionally omits both afxMessages and taskHistory.
	 *
	 * Rationale:
	 * - Cloud event handlers (auth, settings, user-info) and mode changes trigger state pushes
	 *   that have nothing to do with chat messages. Including afxMessages in these pushes
	 *   creates race conditions where a stale snapshot of afxMessages (captured during async
	 ...
	 */
	async postStateToWebviewWithoutAfxMessages(): Promise<void> {
		const state = await this.getStateToPostToWebview()
		const { afxMessages: _omitMessages, taskHistory: _omitHistory, ...rest } = state
		this.postMessageToWebview({ type: "state", state: rest })
	}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		return {
			...
			afxMessages: currentTask?.afxMessages || [],
			...
			disabledTools,
			telemetrySetting,
			showAfxIgnoredFiles: showAfxIgnoredFiles ?? false,
	...
	> {
		...
		const cloudUserInfo: CloudUserInfo | null = null
		const cloudIsAuthenticated: boolean = false
		const sharingEnabled: boolean = false
		const publicSharingEnabled: boolean = false
		const organizationSettingsVersion: number = -1
		...
		return {
			...
			showAfxIgnoredFiles: stateValues.showAfxIgnoredFiles ?? false,
	...
	private scheduleGlobalStateWriteThrough(): void {
		...
		}, AfxProvider.GLOBAL_STATE_WRITE_THROUGH_DEBOUNCE_MS)
	}
	...
	public getValue<K extends keyof AfxSettings>(key: K) {
		return this.contextProxy.getValue(key)
	}
	...
	public async setValues(values: AfxSettings) {
	...
	get messages() {
		return this.getCurrentTask()?.afxMessages || []
	}
	...
	public async createTask(
		...
		if (configuration) {
			...
			// when it merges .afxmodes + global settings on refresh.  Persisting
	...
	public async delegateParentAndOpenChild(params: {
		...
		// (and writing to globalState via saveAfxMessages → updateTaskHistory)
		...
		try {
			this.emit(AfxEventName.TaskDelegated, parentTaskId, child.taskId)
		} catch {
			// non-fatal
		}
	...
	public async reopenParentFromDelegation(params: {
		...
		let parentAfxMessages: AfxMessage[] = []
		try {
			parentAfxMessages = await readTaskMessages({
				taskId: parentTaskId,
				globalStoragePath,
			})
		} catch {
		...
		if (!Array.isArray(parentAfxMessages)) parentAfxMessages = []
		if (!Array.isArray(parentApiMessages)) parentApiMessages = []

		const subtaskUiMessage: AfxMessage = {
			type: "say",
			say: "subtask_result",
			text: completionResultSummary,
			ts,
		}
		parentAfxMessages.push(subtaskUiMessage)
		await saveTaskMessages({ messages: parentAfxMessages, taskId: parentTaskId, globalStoragePath })
		...
		//    removeClineFromStack() → abortTask(true) → saveAfxMessages() writes
		...
		//    that saveAfxMessages() may have written during step 3.
		try {
		...
		try {
			this.emit(AfxEventName.TaskDelegationCompleted, parentTaskId, childTaskId, completionResultSummary)
		} catch {
			// non-fatal
		}
		...
		if (parentInstance) {
			try {
				await parentInstance.overwriteAfxMessages(parentAfxMessages)
			} catch {
				// non-fatal
			}
			try {
		...
		try {
			this.emit(AfxEventName.TaskDelegationResumed, parentTaskId, childTaskId)
		} catch {
			// non-fatal
		}
	}
