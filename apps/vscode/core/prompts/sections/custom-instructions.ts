...
import { isLanguage } from "@agenticflowx/types"
...
export async function loadRuleFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	const afxDirectories = enableSubfolderRules ? await getAllAfxDirectoriesForCwd(cwd) : getAfxDirectoriesForCwd(cwd)

	// Check for .afx/rules/ directories in order (global, project-local, and optionally subfolders)
	for (const afxDir of afxDirectories) {
		const rulesDir = path.join(afxDir, "rules")
	...
	// If we found rules in .afx/rules/ directories, return them
	if (rules.length > 0) {
		return "\n# Rules from .afx directories:\n\n" + rules.join("\n\n")
	}

	// Fall back to .afxrules file
	const ruleFiles = [".afxrules"]
...
/**
 * Load all AGENTS.md files from project root and optionally subdirectories with .afx folders
...
async function loadAllAgentRulesFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	// When enabled, load from root and all subdirectories with .afx folders
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		// Use recursive discovery only if enableSubfolderRules is true
		const afxDirectories = enableSubfolderRules
			? await getAllAfxDirectoriesForCwd(cwd)
			: getAfxDirectoriesForCwd(cwd)

		// Check for .afx/rules-${mode}/ directories in order (global, project-local, and optionally subfolders)
		for (const afxDir of afxDirectories) {
			const modeRulesDir = path.join(afxDir, `rules-${mode}`)
		...
		// If we found mode-specific rules in .afx/rules-${mode}/ directories, use them
		if (modeRules.length > 0) {
			modeRuleContent = "\n" + modeRules.join("\n\n")
			usedRuleFile = `rules-${mode} directories`
		} else {
			// Fall back to .afxrules-${mode} file
			const afxModeRuleFile = `.afxrules-${mode}`
			modeRuleContent = await safeReadFile(path.join(cwd, afxModeRuleFile))
			if (modeRuleContent) {
				usedRuleFile = afxModeRuleFile
			}
		}
	}
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".afx", `rules-${mode}`))) {
	...
	if (options.afxIgnoreInstructions) {
		rules.push(options.afxIgnoreInstructions)
	}

	// Add AGENTS.md content if enabled (default: true)
	// Load from root and optionally subdirectories with .afx folders based on enableSubfolderRules setting
