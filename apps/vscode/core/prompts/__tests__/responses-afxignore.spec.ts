...
// npx vitest core/prompts/__tests__/responses-afxignore.spec.ts

import type { Mock } from "vitest"

import { formatResponse } from "../responses"
import { AfxIgnoreController, LOCK_TEXT_SYMBOL } from "../../ignore/afx-ignore-controller"
...
describe("AfxIgnore Response Formatting", () => {
	...
	describe("formatResponse.afxIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.afxIgnoreError("secrets/api-keys.json")
			...
			expect(parsed.suggestion).toContain("update the .afxignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.afxIgnoreError(testPath)
				const parsed = JSON.parse(errorMessage) as any
	...
	describe("formatResponse.formatFilesList with AFXIgnoreController", () => {
		/**
		 * Tests file listing with afxignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new AfxIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showAfxIgnoredFiles is set to false
		 */
		it("should hide ignored files when showAfxIgnoredFiles is false", async () => {
			// Create controller
			const controller = new AfxIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showAfxIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showAfxIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with AFXIgnoreController
		 */
		it("should handle truncation with AFXIgnoreController", async () => {
			// Create controller
			const controller = new AfxIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with AFXIgnoreController", async () => {
			// Create controller
			const controller = new AfxIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .afxignore instructions for the LLM", async () => {
			// Create controller
			const controller = new AfxIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .afxignore exists", async () => {
			// Set up no .afxignore — mock fs.access to throw (file not found)
			mockFileExists.mockResolvedValue(false)
			const mockAccess = fs.access as Mock<typeof fs.access>
			mockAccess.mockRejectedValue(new Error("ENOENT"))

			// Create controller without .afxignore
			const controller = new AfxIgnoreController(TEST_CWD)
			await controller.initialize()
