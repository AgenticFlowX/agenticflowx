...
import * as ProfileValidatorMod from "../shared/profile-validator"

// Mock Task class used by AfxProvider to avoid heavy startup
...
describe("Single-open-task invariant", () => {
	...
	it("User-initiated create: closes existing before opening new", async () => {
		...
		await (AfxProvider.prototype as any).createTask.call(provider, "New task")
	...
	it("History resume path always closes current before rehydration (non-rehydrating case)", async () => {
		...
		const task = await (AfxProvider.prototype as any).createTaskWithHistoryItem.call(provider, historyItem)
