...
describe("AfxProvider.delegateParentAndOpenChild()", () => {
	it("persists parent delegation metadata and emits TaskDelegated", async () => {
		...
		const child = await (AfxProvider.prototype as any).delegateParentAndOpenChild.call(provider, params)
		...
		expect(providerEmit).toHaveBeenCalledWith(AfxEventName.TaskDelegated, "parent-1", "child-1")
	...
	it("calls child.start() only after parent metadata is persisted (no race condition)", async () => {
		...
		await (AfxProvider.prototype as any).delegateParentAndOpenChild.call(provider, {
			parentTaskId: "parent-1",
			message: "Do something",
			initialTodos: [],
			mode: "code",
