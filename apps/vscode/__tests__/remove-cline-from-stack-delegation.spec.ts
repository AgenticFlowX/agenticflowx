...
describe("AfxProvider.removeClineFromStack() delegation awareness", () => {
	/**
	...
	it("repairs parent metadata (delegated → active) when a delegated child is removed", async () => {
		...
		await (AfxProvider.prototype as any).removeClineFromStack.call(provider)
	...
	it("skips delegation repair when skipDelegationRepair option is true", async () => {
		...
		await (AfxProvider.prototype as any).removeClineFromStack.call(provider, { skipDelegationRepair: true })
