...
vi.mock("../core/task-persistence/task-messages", () => ({
...
import { readTaskMessages } from "../core/task-persistence/task-messages"
...
describe("History resume delegation - parent metadata transitions", () => {
	...
	it("reopenParentFromDelegation persists parent metadata (delegated → active) before reopen", async () => {
		...
		await (AfxProvider.prototype as any).reopenParentFromDelegation.call(provider, {
	...
	it("reopenParentFromDelegation injects subtask_result into both UI and API histories", async () => {
		const provider = {
			...
			createTaskWithHistoryItem: vi.fn().mockResolvedValue({
				taskId: "p1",
				resumeAfterDelegation: vi.fn().mockResolvedValue(undefined),
				overwriteAfxMessages: vi.fn().mockResolvedValue(undefined),
	...
	it("reopenParentFromDelegation emits events in correct order: TaskDelegationCompleted → TaskDelegationResumed", async () => {
		...
		expect(eventNames).toContain(AfxEventName.TaskDelegationCompleted)
		expect(eventNames).toContain(AfxEventName.TaskDelegationResumed)

		// CRITICAL: verify ordering (TaskDelegationCompleted before TaskDelegationResumed)
		const completedIdx = emitSpy.mock.calls.findIndex((c) => c[0] === AfxEventName.TaskDelegationCompleted)
		const resumedIdx = emitSpy.mock.calls.findIndex((c) => c[0] === AfxEventName.TaskDelegationResumed)
	...
	it("reopenParentFromDelegation continues when overwrite operations fail and still resumes/emits (RPD-06)", async () => {
		...
		const parentInstance = {
			resumeAfterDelegation: vi.fn().mockResolvedValue(undefined),
			overwriteAfxMessages: vi.fn().mockRejectedValue(new Error("ui overwrite failed")),
		...
		await expect(
			(AfxProvider.prototype as any).reopenParentFromDelegation.call(provider, {
		...
		expect(parentInstance.overwriteAfxMessages).toHaveBeenCalledTimes(1)
		expect(parentInstance.overwriteApiConversationHistory).toHaveBeenCalledTimes(1)
		expect(parentInstance.resumeAfterDelegation).toHaveBeenCalledTimes(1)

		expect(emitSpy).toHaveBeenCalledWith(
			AfxEventName.TaskDelegationCompleted,
			"parent-rpd06",
			"child-rpd06",
			"Subtask finished despite overwrite failures",
		)
		expect(emitSpy).toHaveBeenCalledWith(AfxEventName.TaskDelegationResumed, "parent-rpd06", "child-rpd06")
	...
	it("reopenParentFromDelegation does NOT emit TaskPaused or TaskUnpaused (new flow only)", async () => {
		...
		const eventNames = emitSpy.mock.calls.map((c) => c[0])
		expect(eventNames).not.toContain(AfxEventName.TaskPaused)
		expect(eventNames).not.toContain(AfxEventName.TaskUnpaused)
		expect(eventNames).not.toContain(AfxEventName.TaskSpawned)
	})
	...
	it("reopenParentFromDelegation logs child status persistence failure and continues reopen flow (RPD-04)", async () => {
		...
		expect(parentInstance.resumeAfterDelegation).toHaveBeenCalledTimes(1)
		expect(emitSpy).toHaveBeenCalledWith(AfxEventName.TaskDelegationResumed, "parent-rpd04", "child-rpd04")
	})
