...
import { attemptCompletionTool } from "../core/tools/attempt-completion-tool"
	...
	it("C completes → reopens B; then B completes → reopens A; emits correct events; no resume_task asks", async () => {
		...
		const provider = {
			...
			reopenParentFromDelegation: vi.fn(async (params: any) => {
				return await (AfxProvider.prototype as any).reopenParentFromDelegation.call(provider, params)
			}),
		...
		expect(eventNamesAfterC).toContain(AfxEventName.TaskDelegationCompleted)
		expect(eventNamesAfterC).toContain(AfxEventName.TaskDelegationResumed)
		...
		const completedEvents = emitSpy.mock.calls.filter((c: any[]) => c[0] === AfxEventName.TaskDelegationCompleted)
		const resumedEvents = emitSpy.mock.calls.filter((c: any[]) => c[0] === AfxEventName.TaskDelegationResumed)
