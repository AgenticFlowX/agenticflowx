...
[remote "origin"]
 url = https://github.com/agenticflowx/agenticflowx.git
...
`
		...
		expect(result).toEqual({
			repositoryUrl: "https://github.com/agenticflowx/agenticflowx.git",
			repositoryName: "agenticflowx/agenticflowx",
			defaultBranch: "main",
		})
...
[core]
	...
	bare = false
[remote "origin"]
	url = git@github.com:agenticflowx/agenticflowx.git
	fetch = +refs/heads/*:refs/remotes/origin/*
[branch "main"]
	remote = origin
	merge = refs/heads/main
...
describe("convertGitUrlToHttps", () => {
	it("should leave HTTPS URLs unchanged", () => {
		const url = "https://github.com/agenticflowx/agenticflowx.git"
		const converted = convertGitUrlToHttps(url)

		expect(converted).toBe("https://github.com/agenticflowx/agenticflowx.git")
	})

	it("should convert SSH URLs to HTTPS format", () => {
		const url = "git@github.com:agenticflowx/agenticflowx.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should convert SSH URLs with ssh:// prefix to HTTPS format", () => {
		const url = "ssh://git@github.com/agenticflowx/agenticflowx.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should handle URLs without git@ prefix", () => {
		const url = "ssh://github.com/agenticflowx/agenticflowx.git"
		const converted = convertGitUrlToHttps(url)
...
describe("sanitizeGitUrl", () => {
	it("should sanitize HTTPS URLs with credentials", () => {
		const url = "https://username:password@github.com/agenticflowx/agenticflowx.git"
		const sanitized = sanitizeGitUrl(url)

		expect(sanitized).toBe("https://github.com/agenticflowx/agenticflowx.git")
	})
	...
	it("should leave SSH URLs unchanged", () => {
		...
		expect(sanitized).toBe("git@github.com:agenticflowx/agenticflowx.git")
	})
	...
	it("should leave SSH URLs with ssh:// prefix unchanged", () => {
		...
		expect(sanitized).toBe("ssh://git@github.com/agenticflowx/agenticflowx.git")
	})

	it("should remove tokens from other URL formats", () => {
		const url = "https://oauth2:ghp_abcdef1234567890abcdef1234567890abcdef@github.com/agenticflowx/agenticflowx.git"
		const sanitized = sanitizeGitUrl(url)
...
describe("extractRepositoryName", () => {
	it("should extract repository name from HTTPS URL", () => {
		...
		expect(repoName).toBe("agenticflowx/agenticflowx")
	})

	it("should extract repository name from HTTPS URL without .git suffix", () => {
		const url = "https://github.com/agenticflowx/agenticflowx"
		const repoName = extractRepositoryName(url)
