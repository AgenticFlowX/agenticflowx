...
import { AfxTerminal, AfxTerminalProvider } from "./types"
import { TerminalProcess } from "./terminal-process"
import { Terminal } from "./terminal"
import { ExecaTerminal } from "./execa-terminal"
import { ShellIntegrationManager } from "./shell-integration-manager"
...
export class TerminalRegistry {
	private static terminals: AfxTerminal[] = []
		...
		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					...
					if (terminal) {
						...
						console.error(
							"[onDidStartTerminalShellExecution] Shell execution started, but not from a AFX-registered terminal:",
							e,
						)
					}
				},
			...
			const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionEndEvent) => {
					...
					if (!terminal) {
						console.error(
							"[onDidEndTerminalShellExecution] Shell execution ended, but not from a AFX-registered terminal:",
							e,
						)
	...
	public static createTerminal(cwd: string, provider: AfxTerminalProvider): AfxTerminal {
		let newTerminal
	...
	public static async getOrCreateTerminal(
		cwd: string,
		taskId?: string,
		provider: AfxTerminalProvider = "vscode",
	): Promise<AfxTerminal> {
		const terminals = this.getAllTerminals()
		let terminal: AfxTerminal | undefined
	 ...
	 */
	public static getTerminals(busy: boolean, taskId?: string): AfxTerminal[] {
	 ...
	 * @returns Array of Terminal objects
	 */
	public static getBackgroundTerminals(busy?: boolean): AfxTerminal[] {
		return this.getAllTerminals().filter((t) => {
	...
	private static getAllTerminals(): AfxTerminal[] {
		this.terminals = this.terminals.filter((t) => !t.isClosed())
		return this.terminals
	}

	private static getTerminalById(id: number): AfxTerminal | undefined {
	 ...
	 */
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): AfxTerminal | undefined {
