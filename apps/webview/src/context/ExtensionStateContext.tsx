...
import { findLastIndex } from "@afx/array"

import { checkExistKey } from "@afx/check-exist-api-config"
import { Mode, defaultModeSlug, defaultPrompts } from "@afx/modes"
import { CustomSupportPrompts } from "@afx/support-prompt"
import { experimentDefault } from "@afx/experiments"
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	setShowAfxIgnoredFiles: (value: boolean) => void
...
export const mergeExtensionState = (prevState: ExtensionState, newState: Partial<ExtensionState>) => {
	...
	// Protect afxMessages from stale state pushes using sequence numbering.
	// Multiple async event sources (cloud auth, settings, task streaming) can trigger
	// concurrent state pushes. If a stale push arrives after a newer one, its afxMessages
	// would overwrite the newer messages. The sequence number prevents this by only applying
	// afxMessages when the incoming seq is strictly greater than the last applied seq.
	if (
		newState.afxMessagesSeq !== undefined &&
		prevState.afxMessagesSeq !== undefined &&
		newState.afxMessagesSeq <= prevState.afxMessagesSeq &&
		newState.afxMessages !== undefined
	) {
		rest.afxMessages = prevState.afxMessages
		rest.afxMessagesSeq = prevState.afxMessagesSeq
	}
...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	const [state, setState] = useState<ExtensionState>({
		...
		showAfxIgnoredFiles: true, // Default to showing .afxignore'd files with lock symbol (current behavior).
			...
			switch (message.type) {
				...
				case "messageUpdated": {
					...
					setState((prevState) => {
						...
						const lastIndex = findLastIndex(prevState.afxMessages, (msg) => msg.ts === clineMessage.ts)
						if (lastIndex !== -1) {
							const newAfxMessages = [...prevState.afxMessages]
							newAfxMessages[lastIndex] = clineMessage
							return { ...prevState, afxMessages: newAfxMessages }
						}
						// Log a warning if messageUpdated arrives for a timestamp not in the
						// frontend's afxMessages. With the seq guard and cloud event isolation
						...
						console.warn(
							`[messageUpdated] Received update for unknown message ts=${clineMessage.ts}, dropping. ` +
								`Frontend has ${prevState.afxMessages.length} messages.`,
						)
						return prevState
					})
					break
	...
	const contextValue: ExtensionStateContextType = {
		...
		setShowAfxIgnoredFiles: (value) => setState((prevState) => ({ ...prevState, showAfxIgnoredFiles: value })),
