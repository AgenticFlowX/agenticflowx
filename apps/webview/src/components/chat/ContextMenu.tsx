...
import { removeLeadingNonAlphanumeric } from "@src/utils/remove-leading-non-alphanumeric"
import { vscode } from "@src/utils/vscode"

import { buildDocLink } from "@/utils/doc-links"
...
const ContextMenu: React.FC<ContextMenuProps> = ({
	...
	const renderOptionContent = (option: ContextMenuQueryItem) => {
		switch (option.type) {
			...
			// [AFX] Parameter autocomplete item (subcommand or feature name)
			case ContextMenuOptionType.Parameter:
				return (
					<div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
						<div style={{ lineHeight: "1.2", display: "flex", alignItems: "center", gap: "6px" }}>
							<span style={{ fontWeight: 500 }}>{option.label ?? option.value}</span>
						</div>
						{option.description && (
							<span
								style={{
	...
	const getIconForOption = (option: ContextMenuQueryItem): string => {
		switch (option.type) {
			...
			case ContextMenuOptionType.NoResults:
				return "info"
			case ContextMenuOptionType.Parameter: // [AFX]
				...
				{searchQuery === "/" && (
					<div className="p-2 flex items-start gap-4 justify-between">
						...
						<button
							...
							<span className="codicon codicon-settings-gear" style={{ fontSize: 16 }} />
						</button>
					</div>
				)}
