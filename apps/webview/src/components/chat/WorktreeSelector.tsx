...
import type { Worktree, WorktreeListResponse } from "@agenticflowx/types"
		...
		<Popover open={open} onOpenChange={setOpen} data-testid="worktree-selector-root">
			<StandardTooltip content={title}>
				<PopoverTrigger
					...
					<span className="codicon codicon-git-branch w-3 h-3" />
					<span className="truncate">{currentWorktree?.branch || t("worktrees:noBranch")}</span>
					<span className="codicon codicon-chevron-down size-3" />
				</PopoverTrigger>
			</StandardTooltip>
			<PopoverContent
				align="start"
				...
				<div className="flex flex-col w-full">
								...
								<div
									...
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2">
											<span className="codicon codicon-git-branch w-3 h-3 shrink-0" />
									...
									{isSelected && <span className="codicon codicon-check ml-auto size-4 p-0.5" />}
								</div>
							)
						})}
					</div>
					...
					<div className="px-3 py-2 border-t border-vscode-panel-border">
						<Button
							...
							<span className="codicon codicon-add w-3 h-3 mr-2" />
