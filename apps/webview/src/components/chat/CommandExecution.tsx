...
import { type ExtensionMessage, type CommandExecutionStatus, commandExecutionStatusSchema } from "@agenticflowx/types"

import { safeJsonParse } from "@afx/core"
import { COMMAND_OUTPUT_STRING } from "@afx/combine-command-sequences"
import { parseCommand } from "@afx/parse-command"
					...
					<div className="flex flex-row items-center gap-1">
							...
							<div className="flex flex-row items-center gap-2 font-mono text-xs">
								...
								<StandardTooltip content={t("chat:commandExecution.abort")}>
									<Button
										...
										<span className="codicon codicon-error size-4" />
									</Button>
								</StandardTooltip>
							</div>
						)}
						{output.length > 0 && (
							<Button variant="ghost" size="icon" onClick={() => setIsExpanded(!isExpanded)}>
								<span
									className={cn(
										"codicon codicon-chevron-down size-4 transition-transform duration-300",
