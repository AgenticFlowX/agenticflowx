...
import type { HistoryItem, AfxMessage } from "@agenticflowx/types"
	...
	useTranslation: () => ({
		t: (key: string) => {
			const map: Record<string, string> = {
				"chat:subtasks.wantsToCreate": "AFX wants to create a new subtask",
...
let mockAfxMessages: AfxMessage[] = []
...
vi.mock("@src/components/ui/hooks/use-selected-model", () => ({
	useSelectedModel: () => ({ info: { supportsImages: true } }),
}))

const queryClient = new QueryClient()

function renderChatRow(message: any, currentTaskItem?: Partial<HistoryItem>, afxMessages?: AfxMessage[]) {
	mockCurrentTaskItem = currentTaskItem
	mockAfxMessages = afxMessages || [message]
...
describe("ChatRow - subtask links", () => {
	...
	describe("newTask tool", () => {
		...
		it("should not display 'Go to subtask' link when directly followed by subtask_result", () => {
			...
			// Pass both messages in the afxMessages array
