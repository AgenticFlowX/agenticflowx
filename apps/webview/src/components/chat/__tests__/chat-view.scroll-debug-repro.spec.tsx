...
vi.mock("@src/components/welcome/AfxHero", nullDefaultModule)
vi.mock("@src/components/welcome/AfxTips", nullDefaultModule)
vi.mock("@src/components/welcome/AfxQuickStart", nullDefaultModule)
...
vi.mock("./FileChangesPanel", () => ({ FileChangesPanel: () => null }))
...
vi.mock("../ChatRow", () => ({
	default: ({ message }: { message: AfxMessage }) => <div data-testid="chat-row">{message.ts}</div>,
}))
...
const buildMessages = (baseTs: number): AfxMessage[] => [
...
const postState = (afxMessages: AfxMessage[]) => {
...
/**
 * Find the scroll-to-bottom chevron icon. The scroll-to-bottom button's chevron
 * has no opacity classes, unlike the TaskHeader's chevron which has opacity-0.
 */
const findScrollToBottomIcon = (): Element | null => {
	const icons = document.querySelectorAll(".codicon-chevron-down")
...
const getScrollToBottomButton = (): HTMLButtonElement => {
	const icon = findScrollToBottomIcon()
...
describe("ChatView scroll behavior regression coverage", () => {
	...
	it("rehydration uses bounded bottom pinning", async () => {
		...
		expect(findScrollToBottomIcon()).toBeNull()
	})
	...
	it("user escape hatch during hydration prevents repinning", async () => {
		...
		await waitFor(() => expect(findScrollToBottomIcon()).toBeTruthy(), {
			timeout: 1_200,
		})
	})
	...
	it("scroll-to-bottom CTA re-anchors with one interaction", async () => {
		...
		await waitFor(() => expect(findScrollToBottomIcon()).toBeNull(), { timeout: 1_200 })
	})
})
