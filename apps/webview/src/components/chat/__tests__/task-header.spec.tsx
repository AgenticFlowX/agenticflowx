...
// Mock findLastIndex from @afx/array
vi.mock("@afx/array", () => ({
...
vi.mock("@/components/ui/hooks/use-selected-model", () => ({
	useSelectedModel: () => ({
		provider: "anthropic",
		id: "test-model",
		info: mockModelInfo,
...
// Mock getModelMaxOutputTokens from @afx/api
...
describe("TaskHeader", () => {
	...
	it("should render the condense context button when expanded", () => {
		...
		const condenseButton = buttons.find((button) => button.querySelector(".codicon-fold"))
		expect(condenseButton).toBeDefined()
		expect(condenseButton?.querySelector(".codicon-fold")).toBeInTheDocument()
	})
	...
	describe("DismissibleUpsell behavior", () => {
		...
		it("should not show DismissibleUpsell when task has completion_result in afxMessages", async () => {
	...
	describe("Back to parent task button", () => {
		...
		it("should show back button with ArrowLeft icon", () => {
			...
			expect(backButton?.querySelector(".codicon-arrow-left")).toBeInTheDocument()
		})
	})
