...
vi.mock("@/components/ui/hooks/use-afx-portal", () => ({
	useAfxPortal: () => document.body,
}))
...
describe("WorktreeSelector", () => {
	...
	test("shows check mark on current worktree", () => {
		...
		// The current worktree should have a check codicon
		const items = screen.getAllByTestId("worktree-selector-item")
		const currentItem = items[0] // main is current
		const checkIcon = currentItem.querySelector(".codicon-check")
