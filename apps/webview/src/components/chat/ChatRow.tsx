...
import { Mode } from "@afx/modes"

import { COMMAND_OUTPUT_STRING } from "@afx/combine-command-sequences"
import { safeJsonParse } from "@afx/core"

import { useExtensionState } from "@src/context/ExtensionStateContext"
import { findMatchingResourceOrTemplate } from "@src/utils/mcp"
import { vscode } from "@src/utils/vscode"
import { formatPathTooltip } from "@src/utils/format-path-tooltip"
...
import { appendImages } from "@src/utils/image-utils"
import { McpExecution } from "./McpExecution"
import { ChatTextArea } from "./ChatTextArea"
import { MAX_IMAGES_PER_MESSAGE } from "./ChatView"
import { useSelectedModel } from "../ui/hooks/use-selected-model"
...
function getPreviousTodos(messages: AfxMessage[], currentMessageTs: number): any[] {
...
interface ChatRowProps {
	message: AfxMessage
	lastModifiedMessage?: AfxMessage
	isExpanded: boolean
	isLast: boolean
	isStreaming: boolean
...
export const ChatRowContent = ({
	...
	const { mcpServers, alwaysAllowMcp, currentCheckpoint, mode, apiConfiguration, afxMessages, currentTaskItem } =
		useExtensionState()
	...
	const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
		if (message.text !== null && message.text !== undefined && message.say === "api_req_started") {
			const info = safeJsonParse<AfxApiReqInfo>(message.text)
	...
	const [icon, title] = useMemo(() => {
		switch (type) {
			...
			case "command":
				return [
					isCommandExecuting ? (
						...
						<span className="codicon codicon-terminal size-4" aria-label="Terminal icon" />
					),
			...
			case "use_mcp_server":
				const mcpServerUse = safeJsonParse<AfxAskUseMcpServer>(message.text)
			...
			case "followup":
				return [
					<span className="codicon codicon-question w-4 shrink-0" aria-label="Question icon" />,
	...
	const tool = useMemo(
		() => (message.ask === "tool" ? safeJsonParse<AfxSayTool>(message.text) : null),
	...
	if (tool) {
		...
		switch (tool.tool as string) {
					...
					return (
						<>
							<div style={headerStyle}>
								<span className="codicon codicon-diff w-4 shrink-0" aria-label="Batch diff icon" />
			...
			case "updateTodoList" as any: {
				...
				const previousTodos = getPreviousTodos(afxMessages, message.ts)
			...
			case "readFile":
				...
				if (isBatchRequest) {
					return (
						<>
							<div style={headerStyle}>
								<span className="codicon codicon-eye w-4 shrink-0" aria-label="View files icon" />
								<span style={{ fontWeight: "bold" }}>
				...
				return (
					<>
						<div style={headerStyle}>
							<span className="codicon codicon-file-code w-4 shrink-0" aria-label="Read file icon" />
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? tool.isOutsideWorkspace
						...
						<div className="pl-6">
							<ToolUseBlock>
								<ToolUseBlockHeader
									...
									<span
										className="codicon codicon-link-external w-4 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
			...
			case "listFilesTopLevel":
				return (
					<>
						<div style={headerStyle}>
							<span className="codicon codicon-list-tree w-4 shrink-0" aria-label="List files icon" />
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? tool.isOutsideWorkspace
			...
			case "listFilesRecursive":
				return (
					<>
						<div style={headerStyle}>
							<span className="codicon codicon-folder w-4 shrink-0" aria-label="Folder tree icon" />
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? tool.isOutsideWorkspace
			...
			case "switchMode":
				return (
					<>
						<div style={headerStyle}>
							<span className="codicon codicon-tools w-4 shrink-0" aria-label="Switch mode icon" />
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask" ? (
									<>
										{tool.reason ? (
			...
			case "newTask":
				...
				const newTaskMessages = afxMessages.filter((msg) => {
					if (msg.type === "ask" && msg.ask === "tool") {
						const t = safeJsonParse<AfxSayTool>(msg.text)
				...
				const currentMessageIndex = afxMessages.findIndex((msg) => msg.ts === message.ts)
				const nextMessage = currentMessageIndex >= 0 ? afxMessages[currentMessageIndex + 1] : undefined
				const isFollowedBySubtaskResult = nextMessage?.type === "say" && nextMessage?.say === "subtask_result"

				return (
					<>
						<div style={headerStyle}>
							<span className="codicon codicon-split-horizontal size-4" />
							<span style={{ fontWeight: "bold" }}>
								<Trans
							...
							<div>
								{childTaskId && !isFollowedBySubtaskResult && (
									<button
										...
										<span className="codicon codicon-arrow-right size-3" />
									</button>
								)}
							</div>
						</div>
			...
			switch (message.say) {
					...
					return (
						<div className="border-l border-muted-foreground/80 ml-2 pl-4 pt-2 pb-1 -mt-5">
							<div style={headerStyle}>
								...
								<span className="codicon codicon-check size-3" />
							</div>
					...
					if (message.text !== undefined) {
						...
						if (!isNaN(potentialCode) && potentialCode >= 400) {
							...
							if (i18n.exists(stringForError)) {
								...
								docsURL = "https://github.com/AgenticFlowX/agenticflowx/issues/new"
							}
						}
						...
						retryInfo = retryTimer > 0 && (
							<p
								...
								<span className="codicon codicon-sync size-3" />
				...
				case "text":
					return (
						<div className="group">
							<div style={headerStyle}>
								<span
									className="codicon codicon-feedback w-4 shrink-0"
									aria-label="Speech bubble icon"
								/>
								<span style={{ fontWeight: "bold" }}>{t("chat:text.afxSaid")}</span>
				...
				case "user_feedback":
					return (
						<div className="group">
							<div style={headerStyle}>
								<span className="codicon codicon-person w-4 shrink-0" aria-label="User icon" />
									...
									<div className="flex justify-between">
										...
										<div className="flex gap-2 pr-1">
											<div
												...
												<span
													className="codicon codicon-edit w-4 shrink-0"
											...
											<div
												...
												<span
													className="codicon codicon-trash w-4 shrink-0"
				...
				case "user_feedback_diff":
					const tool = safeJsonParse<AfxSayTool>(message.text)
					return (
				...
				case "tool" as any:
					...
					const sayTool = safeJsonParse<AfxSayTool>(message.text)
					...
					switch (sayTool.tool) {
						...
						case "readCommandOutput": {
							...
							return (
								<div style={headerStyle}>
									<span
										className="codicon codicon-file-code w-4 shrink-0"
										aria-label="Read command output icon"
									/>
		...
		case "ask":
			switch (message.ask) {
				...
				case "use_mcp_server":
					...
					const useMcpServer: AfxAskUseMcpServer = {
						...mcpServerRequest,
						response,
					}
