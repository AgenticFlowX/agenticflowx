...
import { getModelMaxOutputTokens } from "@afx/api"
...
import { useSelectedModel } from "@/components/ui/hooks/use-selected-model"
	...
	const condenseButton = (
		<StandardTooltip content={t("chat:task.condenseContext")}>
			<Button
				...
				className="relative inline-flex items-center justify-center bg-transparent border-none p-1.5 rounded-full text-vscode-foreground opacity-85 transition-all duration-150 focus:outline-none focus-visible:ring-1 focus-visible:ring-vscode-focusBorder active:bg-[rgba(255,255,255,0.1)] cursor-pointer hover:opacity-100 hover:bg-[rgba(255,255,255,0.03)] disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:active:bg-transparent"
				disabled={buttonsDisabled}
				onClick={() => !buttonsDisabled && currentTaskItem && handleCondenseContext(currentTaskItem.id)}
				style={{ fontSize: 16.5 }}>
				<span className="codicon codicon-fold size-2.5" />
			</Button>
		</StandardTooltip>
	)
			...
			{isSubtask && (
				<div className="mb-2" onClick={(e) => e.stopPropagation()}>
					<Button
						...
						<span className="codicon codicon-arrow-left size-3" />
						...
						<div className="flex items-center shrink-0 ml-2" onClick={(e) => e.stopPropagation()}>
							<StandardTooltip content={isTaskExpanded ? t("chat:task.collapse") : t("chat:task.expand")}>
								<button
									...
									{isTaskExpanded ? (
										<span className="codicon codicon-chevron-up" style={{ fontSize: 16 }} />
									) : (
										<span
											className="codicon codicon-chevron-down opacity-0 group-hover:opacity-100"
											style={{ fontSize: 16 }}
										/>
									)}
								</button>
										...
										<tr>
											...
											<td className="font-light align-top">
												<div className="flex items-center gap-1 flex-wrap">
													{typeof cacheWrites === "number" && cacheWrites > 0 && (
														<>
															<span className="codicon codicon-desktop-download size-2.5" />
															<span>{formatLargeNumber(cacheWrites)}</span>
														</>
													)}
													{typeof cacheReads === "number" && cacheReads > 0 && (
														<>
															<span className="codicon codicon-export size-2.5" />
