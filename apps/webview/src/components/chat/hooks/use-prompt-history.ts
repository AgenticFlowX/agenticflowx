...
import { AfxMessage, HistoryItem } from "@agenticflowx/types"
...
export const usePromptHistory = ({
	...
	const filteredPromptHistory = useMemo(() => {
		// First try to get conversation messages (user_feedback from afxMessages)
		const conversationPrompts = afxMessages
		...
		// If we have afxMessages array (meaning we're in an active task), don't fall back to task history
		// Only use task history when starting fresh (no active conversation)
		if (afxMessages?.length) {
			return []
		}
