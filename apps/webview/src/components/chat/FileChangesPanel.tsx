...
import type { AfxMessage, ExtensionMessage } from "@agenticflowx/types"
...
interface FileChangesPanelProps {
	afxMessages: AfxMessage[] | undefined
	className?: string
}

const FileChangesPanel = memo(({ afxMessages, className }: FileChangesPanelProps) => {
	...
	const fileChanges = useMemo(() => fileChangesFromMessages(afxMessages), [afxMessages])
	...
	return (
		<Collapsible open={panelExpanded} onOpenChange={setPanelExpanded} className={cn("px-3", className)}>
			<CollapsibleTrigger
				...
				{panelExpanded ? (
					<span className="codicon codicon-chevron-down size-4 shrink-0" aria-hidden />
				) : (
					<span className="codicon codicon-chevron-right size-4 shrink-0" aria-hidden />
				)}
				<span className="codicon codicon-diff size-4 shrink-0" aria-hidden />
