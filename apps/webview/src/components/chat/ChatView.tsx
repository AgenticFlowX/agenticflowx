...
import { useDebounceEffect } from "@src/utils/use-debounce-effect"
import { appendImages } from "@src/utils/image-utils"
import { getCostBreakdownIfNeeded } from "@src/utils/cost-formatting"
import { batchConsecutive } from "@src/utils/batch-consecutive"

import type { AfxAsk, AfxSayTool, AfxMessage, ExtensionMessage } from "@agenticflowx/types"
import { isRetiredProvider } from "@agenticflowx/types"

import { findLast } from "@afx/array"
import { SuggestionItem } from "@agenticflowx/types"
import { combineApiRequests } from "@afx/combine-api-requests"
import { combineCommandSequences } from "@afx/combine-command-sequences"
import { getApiMetrics } from "@afx/get-api-metrics"
import { getAllModes } from "@afx/modes"
import { ProfileValidator } from "@afx/profile-validator"
import { getLatestTodo } from "@afx/todo"

import { vscode } from "@src/utils/vscode"
import { useAppTranslation } from "@src/i18n/TranslationContext"
import { useExtensionState } from "@src/context/ExtensionStateContext"
import { useSelectedModel } from "@src/components/ui/hooks/use-selected-model"
...
import { useScrollLifecycle } from "@src/hooks/use-scroll-lifecycle"
...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	const [clineAsk, setAfxAsk] = useState<AfxAsk | undefined>(undefined)
				...
				case "ask":
					...
					switch (lastMessage.ask) {
						...
						case "mistake_limit_reached":
							...
							setAfxAsk("mistake_limit_reached")
							setEnableButtons(true)
						...
						case "tool":
							...
							const tool = JSON.parse(lastMessage.text || "{}") as AfxSayTool
							switch (tool.tool) {
						...
						case "resume_completed_task":
							setSendingDisabled(false)
							setAfxAsk("resume_completed_task")
							setEnableButtons(true)
							setPrimaryButtonText(t("chat:startNewTask.title"))
							setSecondaryButtonText(undefined)
	...
	const isStreaming = useMemo(() => {
		...
		if (isLastMessagePartial) {
			...
			const lastApiReqStarted = findLast(
				modifiedMessages,
				(message: AfxMessage) => message.say === "api_req_started",
			)
	...
	const markFollowUpAsAnswered = useCallback(() => {
		const lastFollowUpMessage = messagesRef.current.findLast((msg: AfxMessage) => msg.ask === "followup")
	...
	const visibleMessages = useMemo(() => {
		...
		const newVisibleMessages = modifiedMessages.filter((message) => {
			...
			if (everVisibleMessagesTsRef.current.has(message.ts)) {
				const alwaysHiddenOnceProcessedAsk: AfxAsk[] = [
					"api_req_failed",
					"resume_task",
		...
		newVisibleMessages
			.slice(viewportStart)
			.forEach((msg: AfxMessage) => everVisibleMessagesTsRef.current.set(msg.ts, true))
	...
	useEffect(() => {
		const cleanupInterval = setInterval(() => {
			const cache = everVisibleMessagesTsRef.current
			const currentMessageIds = new Set(modifiedMessages.map((m: AfxMessage) => m.ts))
			const viewportMessages = visibleMessages.slice(Math.max(0, visibleMessages.length - 100))
			const viewportMessageIds = new Set(viewportMessages.map((m: AfxMessage) => m.ts))
	...
	const groupedMessages = useMemo(() => {
		const filtered: AfxMessage[] = visibleMessages

		// Helper to check if a message is a read_file ask that should be batched
		const isReadFileAsk = (msg: AfxMessage): boolean => {
		...
		const isListFilesAsk = (msg: AfxMessage): boolean => {
			if (msg.type !== "ask" || msg.ask !== "tool") return false
			try {
				const tool = JSON.parse(msg.text || "{}")
				return (
		...
		const isEditFileAsk = (msg: AfxMessage): boolean => {
			if (msg.type !== "ask" || msg.ask !== "tool") return false
			try {
				const tool = JSON.parse(msg.text || "{}")
		...
		const synthesizeReadFileBatch = (batch: AfxMessage[]): AfxMessage => {
		...
		const synthesizeListFilesBatch = (batch: AfxMessage[]): AfxMessage => {
		...
		const synthesizeEditFileBatch = (batch: AfxMessage[]): AfxMessage => {
	...
	const itemContent = useCallback(
		(index: number, messageOrGroup: AfxMessage) => {
		...
		<div
			...
			{task && (
				<>
					...
					<FileChangesPanel afxMessages={messages} />
					{areButtonsVisible && (
						<div
