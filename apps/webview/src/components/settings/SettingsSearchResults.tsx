...
import { SearchResult } from "./use-settings-search"

export interface SettingsSearchResultsProps {
	results: SearchResult[]
	query: string
	onSelectResult: (result: SearchResult) => void
	sections: { id: SectionName; iconClass: string }[]
...
export function SettingsSearchResults({
	...
	// Create a map of section id to iconClass for quick lookup
	const sectionIconMap = useMemo(() => {
		return new Map(sections.map((section) => [section.id, section.iconClass]))
	}, [sections])
	...
	return (
		<div className="max-h-80 overflow-y-auto" role="listbox">
			{Object.entries(groupedResults).map(([section, sectionResults]) => {
				const iconClass = sectionIconMap.get(section as SectionName)
				...
				return (
					<div key={section}>
						...
						<div className="flex items-center gap-2 px-3 py-1.5 mt-4 first:mt-0 text-xs text-vscode-descriptionForeground bg-vscode-sideBar-background border-b-vscode-panel-border sticky top-0">
							{iconClass && <span className={`codicon ${iconClass} h-3.5 w-3.5`} />}
