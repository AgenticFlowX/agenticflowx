...
function createFileEditMessage(path: string, diff: string, diffStats?: { added: number; removed: number }): AfxMessage {
	return {
		type: "ask",
		ask: "tool",
		ts: Date.now(),
...
function renderPanel(messages: AfxMessage[] | undefined) {
	return render(
		<TranslationProvider>
...
describe("FileChangesPanel", () => {
	...
	it("renders nothing when afxMessages is undefined", () => {
		const { container } = renderPanel(undefined)
		expect(container.firstChild).toBeNull()
	})

	it("renders nothing when afxMessages is empty", () => {
