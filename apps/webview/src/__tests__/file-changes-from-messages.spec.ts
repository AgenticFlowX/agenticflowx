...
import { fileChangesFromMessages } from "../components/chat/utils/file-changes-from-messages"

function msg(overrides: Partial<AfxMessage> & { text: string }): AfxMessage {
	return {
		type: "say",
		say: "tool",
		ts: Date.now(),
...
describe("fileChangesFromMessages", () => {
	...
	it("recognizes all AfxSayTool file-edit tool names (editedExistingFile, appliedDiff, newFileCreated)", () => {
