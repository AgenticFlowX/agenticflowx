...
import { sourcemapPlugin } from "./src/vite-plugins/sourcemap-plugin"
...
export default defineConfig(({ mode }) => {
	let outDir = "../vscode/webview/build"

	const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "vscode", "package.json"), "utf8"))
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@afx": resolve(__dirname, "../vscode/shared"),
			},
		},
		build: {
			outDir,
			...
			rollupOptions: {
				...
				// dynamically imported by afx-config/index.ts, but should never be bundled
