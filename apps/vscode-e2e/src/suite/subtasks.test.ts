...
suite.skip("AgenticFlowX Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, AfxMessage[]> = {}

		api.on(AfxEventName.Message, ({ taskId, message }) => {
		...
		api.on(AfxEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
