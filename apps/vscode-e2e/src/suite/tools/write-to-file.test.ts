...
suite.skip("AgenticFlowX write_to_file Tool", function () {
	...
	suiteSetup(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afx-test-"))
	})
	...
	test("Should create a new file with content", async function () {
		...
		try {
			...
			const possibleLocations = [
				...
				path.join("/tmp/afx-test-workspace-" + "*", baseFileName), // In workspace created by runTest.ts
			]
			...
			const workspaceDirs = await fs
				.readdir("/tmp")
				.then((files) => files.filter((f) => f.startsWith("afx-test-workspace-")))
				.catch(() => [])
			...
			if (!fileFound) {
				...
				try {
					...
					console.log(
						"Test files in /tmp:",
						tmpFiles.filter((f) => f.includes("test-file") || f.includes("afx-test")),
					)
				} catch (e) {
