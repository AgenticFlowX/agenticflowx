...
suite.skip("AgenticFlowX use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afx-test-mcp-"))
		...
		testFiles = {
			...
			mcpConfig: path.join(workspaceDir, ".afx", "mcp.json"),
		}
		...
		// Create .afx directory and MCP configuration file
		const afxDir = path.join(workspaceDir, ".afx")
	...
	suiteTeardown(async () => {
		...
		// Clean up .afx directory
		const workspaceDir = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || tempDir
		const afxDir = path.join(workspaceDir, ".afx")
		try {
			await fs.rm(afxDir, { recursive: true, force: true })
		} catch {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		// Listen for messages
		const messageHandler = ({ message }: { message: AfxMessage }) => {
		...
		api.on(AfxEventName.Message, messageHandler)
		...
		api.on(AfxEventName.TaskStarted, taskStartedHandler)
		...
		api.on(AfxEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for AgenticFlowX to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this AgenticFlowX won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(AfxEventName.Message, messageHandler)
			api.off(AfxEventName.TaskStarted, taskStartedHandler)
			api.off(AfxEventName.TaskCompleted, taskCompletedHandler)
		}
	})
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasAfxDir = responseText.includes(".afx")

			// At least one of our test files or the .afx directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasAfxDir,
				`MCP server response should contain our test files or .afx directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.afx'. Got: ${responseText.substring(0, 200)}...`,
			)
	...
	test.skip("Should validate MCP request message format and complete successfully", async function () {
		...
		const messageHandler = ({ message }: { message: AfxMessage }) => {
			...
			if (message.type === "ask" && message.ask === "use_mcp_server") {
				...
				// Validate the message format matches AfxAskUseMcpServer interface
				if (message.text) {
					try {
						const mcpRequest = JSON.parse(message.text)
						mcpToolName = mcpRequest.toolName
