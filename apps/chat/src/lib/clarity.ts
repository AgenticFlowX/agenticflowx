...
const log = createLogger({ scope: "chat:clarity", level: "info", sinks: [consoleSink()] });
...
function tagSession(): void {
  ...
  try {
    ...
    // Helpful dimensions for filtering sessions inside Clarity.
    window.clarity("set", "afx_app", "chat");
    window.clarity("set", "afx_surface", "sidebar");
  } catch (err) {
...
function stopClarity(): void {
  ...
  try {
    // Best-effort opt-out: stop analytics storage and clear any existing cookies.
