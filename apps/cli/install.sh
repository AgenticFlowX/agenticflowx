...
# AgenticFlowX CLI Installer
#
# Based on Roo Code (https://github.com/RooCodeInc/Roo-Code)
# which is based on Cline (https://github.com/cline/cline)
# originally created by Saoud Rizwan as "Claude Dev"
# All upstream code licensed under the Apache License, Version 2.0.
#
# Usage: curl -fsSL https://raw.githubusercontent.com/AgenticFlowX/agenticflowx/main/apps/cli/install.sh | sh
#
# Environment variables:
#   AFX_INSTALL_DIR   - Installation directory (default: ~/.afx/cli)
#   AFX_BIN_DIR       - Binary symlink directory (default: ~/.local/bin)
#   AFX_VERSION       - Specific version to install (default: latest)
#   AFX_LOCAL_TARBALL - Path to local tarball to install (skips download)

set -e

# Configuration
INSTALL_DIR="${AFX_INSTALL_DIR:-$HOME/.afx/cli}"
BIN_DIR="${AFX_BIN_DIR:-$HOME/.local/bin}"
REPO="AgenticFlowX/agenticflowx"
MIN_NODE_VERSION=20
...
get_version() {
    ...
    if [ -n "$AFX_LOCAL_TARBALL" ]; then
        VERSION="${AFX_VERSION:-local}"
        info "Using local tarball (version: $VERSION)"
        return
    fi

    if [ -n "$AFX_VERSION" ]; then
        VERSION="$AFX_VERSION"
...
download_and_install() {
    TARBALL="afx-cli-${PLATFORM}.tar.gz"
    ...
    if [ -n "$AFX_LOCAL_TARBALL" ]; then
        if [ ! -f "$AFX_LOCAL_TARBALL" ]; then
            error "Local tarball not found: $AFX_LOCAL_TARBALL"
        fi
        info "Using local tarball: $AFX_LOCAL_TARBALL"
        cp "$AFX_LOCAL_TARBALL" "$TMP_DIR/$TARBALL"
    else
...
setup_bin() {
    ...
    if [ -L "$BIN_DIR/afx" ] || [ -f "$BIN_DIR/afx" ]; then
        rm -f "$BIN_DIR/afx"
    fi

    ln -sf "$INSTALL_DIR/bin/afx" "$BIN_DIR/afx"
    info "Created symlink: $BIN_DIR/afx"
}
...
# Verify installation
verify_install() {
    if [ -x "$BIN_DIR/afx" ]; then
        info "Verifying installation..."
        # Just check if it runs without error
        "$BIN_DIR/afx" --version >/dev/null 2>&1 || true
    fi
}
...
print_success() {
    echo ""
    printf "${GREEN}${BOLD}✓ AgenticFlowX CLI installed successfully!${NC}\n"
    echo ""
    echo "  Installation: $INSTALL_DIR"
    echo "  Binary: $BIN_DIR/afx"
    ...
    echo "    cd ~/my-project && afx \"What is this project?\""
    echo ""
}
...
main() {
    ...
    printf "${BLUE}${BOLD}"
    echo "  ╭─────────────────────────────────╮"
    echo "  │   AgenticFlowX CLI Installer    │"
    echo "  ╰─────────────────────────────────╯"
    printf "${NC}"
    echo ""
