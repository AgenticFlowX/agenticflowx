...
program
	.name("afx")
	.description(
		"AgenticFlowX CLI - starts an interactive session by default, use -p/--print for non-interactive output",
	)
	.version(VERSION)
...
program
	...
	.option("--provider <provider>", "API provider (anthropic, openai-native, openrouter, gemini, vercel-ai-gateway)")
...
applyListOptions(listCommand.command("models").description("List available AFX models")).action(
...
program
	.command("upgrade")
	.description("Upgrade AgenticFlowX CLI to the latest version")
	.action(async () => {
