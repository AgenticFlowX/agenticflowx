...
import { setLogger } from "@agenticflowx/vscode-shim"
...
export async function run(promptArg: string | undefined, flagOptions: FlagOptions) {
	...
	if (isResumeRequested && prompt) {
		...
		console.error("[CLI] Usage: afx [--session-id <session-id> | --continue] [options]")
		process.exit(1)
	}
	...
	if (outputFormat !== "text" && !flagOptions.print && isTuiSupported) {
		...
		console.error("[CLI] Usage: afx --print --output-format json")
		process.exit(1)
	}

	if (flagOptions.stdinPromptStream && !flagOptions.print) {
		console.error("[CLI] Error: --stdin-prompt-stream requires --print mode")
		console.error("[CLI] Usage: afx --print --output-format stream-json --stdin-prompt-stream [options]")
		process.exit(1)
	}
	...
	if (flagOptions.signalOnlyExit && !flagOptions.stdinPromptStream) {
		...
		console.error("[CLI] Usage: afx --print --output-format stream-json --stdin-prompt-stream --signal-only-exit")
		process.exit(1)
	}
	...
	if (flagOptions.stdinPromptStream && process.stdin.isTTY) {
		...
		console.error(
			'[CLI] Example: printf \'{"command":"start","requestId":"1","prompt":"1+1=?"}\\n\' | afx --print --output-format stream-json --stdin-prompt-stream [options]',
		)
		process.exit(1)
	}
	...
	if (!isTuiEnabled) {
		if (!prompt && !useStdinPromptStream && !isResumeRequested) {
			if (flagOptions.print) {
				...
				console.error("[CLI] Usage: afx --print [options] <prompt>")
				console.error(
					"[CLI] For stdin control mode: afx --print --output-format stream-json --stdin-prompt-stream [options]",
				)
			} else {
				console.error("[CLI] Error: prompt is required in non-interactive mode")
				console.error("[CLI] Usage: afx <prompt> [options]")
