...
const RELEASES_URL = "https://api.github.com/repos/agenticflowx/agenticflowx/releases?per_page=100"
...
export function runUpgradeInstaller(version?: string, spawnImpl: typeof spawn = spawn): Promise<void> {
	return new Promise((resolve, reject) => {
		const env = version ? { ...process.env, AFX_VERSION: version } : process.env
...
export async function upgrade(options: UpgradeOptions = {}): Promise<void> {
	...
	if (compareVersions(latestVersion, currentVersion) <= 0) {
		console.log("AFX CLI is already up to date.")
		return
	}

	console.log(`Upgrading AFX CLI from ${currentVersion} to ${latestVersion}...`)
	if (runInstaller) {
		await runInstaller()
	} else {
