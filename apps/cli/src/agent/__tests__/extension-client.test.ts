...
function createMessage(overrides: Partial<AfxMessage>): AfxMessage {
	return { ts: Date.now() + Math.random() * 1000, type: "say", ...overrides }
}

function createStateMessage(messages: AfxMessage[], mode?: string): ExtensionMessage {
	return { type: "state", state: { afxMessages: messages, mode } } as ExtensionMessage
}
...
describe("detectAgentState", () => {
	describe("NO_TASK state", () => {
		...
		it("should return NO_TASK for undefined messages", () => {
			const state = detectAgentState(undefined as unknown as AfxMessage[])
			expect(state.state).toBe(AgentLoopState.NO_TASK)
		})
	})
...
describe("Edge Cases", () => {
	...
	describe("State message edge cases", () => {
		it("should handle state message with empty afxMessages", () => {
			const { client } = createMockClient()
			client.handleMessage({ type: "state", state: { afxMessages: [] } } as unknown as ExtensionMessage)
			expect(client.getCurrentState()).toBe(AgentLoopState.NO_TASK)
			expect(client.isInitialized()).toBe(true)
		})

		it("should handle state message with missing afxMessages", () => {
			const { client } = createMockClient()
