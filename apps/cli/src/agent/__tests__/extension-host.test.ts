...
// pnpm --filter @agenticflowx/cli test src/agent/__tests__/extension-host.test.ts

import { EventEmitter } from "events"
import fs from "fs"

import type { ExtensionMessage, WebviewMessage } from "@agenticflowx/types"
...
vi.mock("@agenticflowx/vscode-shim", () => ({
...
vi.mock("@/lib/storage/index.js", () => ({
	createEphemeralStorageDir: vi.fn(() => Promise.resolve("/tmp/afx-cli-test-ephemeral")),
}))
...
describe("ExtensionHost", () => {
	const initialAfxCliRuntimeEnv = process.env.AFX_CLI_RUNTIME

	beforeEach(() => {
		vi.resetAllMocks()
		if (initialAfxCliRuntimeEnv === undefined) {
			delete process.env.AFX_CLI_RUNTIME
		} else {
			process.env.AFX_CLI_RUNTIME = initialAfxCliRuntimeEnv
		}
		// Clean up globals
	...
	describe("constructor", () => {
		...
		it("should mark process as CLI runtime", () => {
			delete process.env.AFX_CLI_RUNTIME
			createTestHost()
			expect(process.env.AFX_CLI_RUNTIME).toBe("1")
		})
	...
	describe("dispose", () => {
		...
		it("should clear AFX_CLI_RUNTIME on dispose when it was previously unset", async () => {
			delete process.env.AFX_CLI_RUNTIME
			host = createTestHost()
			expect(process.env.AFX_CLI_RUNTIME).toBe("1")
			...
			expect(process.env.AFX_CLI_RUNTIME).toBeUndefined()
		})

		it("should restore prior AFX_CLI_RUNTIME value on dispose", async () => {
			process.env.AFX_CLI_RUNTIME = "preexisting-value"
			host = createTestHost()
			expect(process.env.AFX_CLI_RUNTIME).toBe("1")

			await host.dispose()

			expect(process.env.AFX_CLI_RUNTIME).toBe("preexisting-value")
		})
	})
	...
	describe("ephemeral mode", () => {
		...
		it("should clean up ephemeral storage directory on dispose", async () => {
			...
			const mockEphemeralDir = "/tmp/afx-cli-test-ephemeral-cleanup"
		...
		it("should handle ephemeral storage cleanup errors gracefully", async () => {
			...
			// Set up a mock ephemeral storage directory
			setPrivate(host, "ephemeralStorageDir", "/tmp/afx-cli-test-ephemeral-error")
