...
function emitMessage(emitter: JsonEventEmitter, message: AfxMessage): void {
	;(emitter as unknown as { handleMessage: (msg: AfxMessage, isUpdate: boolean) => void }).handleMessage(
		message,
		false,
	)
}
...
function createAskCompletionMessage(ts: number, text = ""): AfxMessage {
	return {
		ts,
		type: "ask",
...
function createCompletedStateInfo(message: AfxMessage): AgentStateInfo {
	return {
