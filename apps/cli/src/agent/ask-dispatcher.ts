...
import { debugLog } from "@agenticflowx/core/cli"
...
export class AskDispatcher {
	 ...
	 *
	 * @param message - The AfxMessage with type="ask"
	 * @returns Promise<AskHandleResult>
	 */
	async handleAsk(message: AfxMessage): Promise<AskHandleResult> {
	 ...
	 */
	private async handleNonBlockingAsk(_ts: number, _ask: AfxAsk, _text: string): Promise<AskHandleResult> {
	 ...
	 */
	private async handleIdleAsk(ts: number, ask: AfxAsk, text: string): Promise<AskHandleResult> {
		switch (ask) {
	 ...
	 */
	private async handleResumableAsk(ts: number, ask: AfxAsk, text: string): Promise<AskHandleResult> {
		return await this.handleResumeTask(ts, ask, text)
	}
	 ...
	 */
	private async handleInteractiveAsk(ts: number, ask: AfxAsk, text: string): Promise<AskHandleResult> {
		switch (ask) {
			case "followup":
	 ...
	 */
	private async handleUnknownAsk(ts: number, ask: AfxAsk, text: string): Promise<AskHandleResult> {
	...
	private async handleToolApproval(ts: number, text: string): Promise<AskHandleResult> {
		...
		if (isProtected) {
			...
			this.outputManager.output(
				`    Protected files include .afxignore, .afx/*, and other sensitive config files.`,
			)
		} else {
	 ...
	 */
	private async handleResumeTask(ts: number, ask: AfxAsk, text: string): Promise<AskHandleResult> {
	 ...
	 */
	private async handleGenericApproval(ts: number, ask: AfxAsk, text: string): Promise<AskHandleResult> {
		this.outputManager.output(`\n[${ask}]`)
		if (text) {
			this.outputManager.output(`  ${text}`)
		}
