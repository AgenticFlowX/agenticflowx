 ...
 * AgenticFlowX agent loop. The state is determined by analyzing the afxMessages
...
import { AfxMessage, AfxAsk, isIdleAsk, isResumableAsk, isInteractiveAsk, isNonBlockingAsk } from "@agenticflowx/types"
 ...
 */
function isApiRequestInProgress(messages: AfxMessage[]): boolean {
 ...
 */
function getRequiredAction(ask: AfxAsk): RequiredAction {
	switch (ask) {
		case "followup":
			return "answer"
		case "command":
 ...
 */
function getStateDescription(state: AgentLoopState, ask?: AfxAsk): string {
	switch (state) {
...
/**
 * Detect the current state of the agent loop from the afxMessages array.
 *
 ...
 *
 * @param messages - The afxMessages array from extension state
 * @returns Detailed state information
 */
export function detectAgentState(messages: AfxMessage[]): AgentStateInfo {
 ...
 */
export function isAgentWaitingForInput(messages: AfxMessage[]): boolean {
 ...
 */
export function isAgentRunning(messages: AfxMessage[]): boolean {
 ...
 */
export function isContentStreaming(messages: AfxMessage[]): boolean {
