...
/**
 * ExtensionHost - Loads and runs the AgenticFlowX extension in CLI mode
 *
...
} from "@agenticflowx/vscode-shim"
import { DebugLogger, setDebugLogEnabled } from "@agenticflowx/core/cli"
...
// When running from a release tarball, AFX_CLI_ROOT is set by the wrapper script.
...
const CLI_PACKAGE_ROOT = process.env.AFX_CLI_ROOT || findCliPackageRoot()
...
export interface ExtensionHostInterface extends IExtensionHost<ExtensionHostEventMap> {
	...
	runTask(prompt: string, taskId?: string, configuration?: AfxSettings, images?: string[]): Promise<void>
...
export class ExtensionHost extends EventEmitter implements ExtensionHostInterface {
	...
	private initialSettings: AfxSettings
	...
	constructor(options: ExtensionHostOptions) {
		...
		this.previousCliRuntimeEnv = process.env.AFX_CLI_RUNTIME
		process.env.AFX_CLI_RUNTIME = "1"
		...
		const baseSettings: AfxSettings = {
	...
	private logMessageDebug(msg: AfxMessage, type: "new" | "updated"): void {
		if (msg.partial) {
	...
	public markWebviewReady(): void {
		...
		setRuntimeConfigValues("agenticflowx", this.initialSettings as Record<string, unknown>)
	...
	private waitForTaskCompletion(): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			let messageHandler: ((msg: AfxMessage) => void) | null = null

			if (this.options.exitOnError) {
				messageHandler = (msg: AfxMessage) => {
	...
	async dispose(): Promise<void> {
		...
		if (this.previousCliRuntimeEnv === undefined) {
			delete process.env.AFX_CLI_RUNTIME
		} else {
			process.env.AFX_CLI_RUNTIME = this.previousCliRuntimeEnv
		}
	}
}
