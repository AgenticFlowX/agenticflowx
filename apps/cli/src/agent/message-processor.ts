...
import { ExtensionMessage, AfxMessage } from "@agenticflowx/types"
...
export class MessageProcessor {
	 ...
	 * It contains the complete afxMessages array which is the source of truth.
	 */
	private handleStateMessage(message: ExtensionMessage): void {
		...
		const { afxMessages, mode } = message.state
		...
		if (!afxMessages) {
			if (this.options.debug) {
				debugLog("[MessageProcessor] State message missing afxMessages")
			}
			return
		}
		...
		this.store.setMessages(afxMessages)
		...
		if (this.options.debug) {
			const lastMsg = afxMessages[afxMessages.length - 1]
			...
			debugLog("[MessageProcessor] State update", {
				messageCount: afxMessages.length,
		...
		this.emitNewMessageEvents(previousState, currentState, afxMessages)
	}
...
/**
 * Check if a message is a valid AfxMessage.
 * Useful for validating messages before processing.
 */
export function isValidAfxMessage(message: unknown): message is AfxMessage {
 ...
 */
export function parseApiReqStartedText(message: AfxMessage): { cost?: number } | undefined {
