 ...
 *
 * This class transforms internal CLI events (AfxMessage, state changes, etc.)
...
/**
 * Parse tool information from a AfxMessage text field.
...
export class JsonEventEmitter {
	...
	constructor(options: JsonEventEmitterOptions) {
		...
		this.protocol = options.protocol ?? "afx-cli-stream"
	...
	/**
	 * Handle a AfxMessage and emit the appropriate JSON event.
	 */
	private handleMessage(msg: AfxMessage, _isUpdate: boolean): void {
	 ...
	 */
	private handleSayMessage(msg: AfxMessage, contentToSend: string | null, isDone: boolean): void {
		switch (msg.say) {
			case "text":
	 ...
	 */
	private handleReasoningMessage(msg: AfxMessage, isDone: boolean): void {
	 ...
	 */
	private handleAskMessage(msg: AfxMessage, isDone: boolean): void {
		switch (msg.ask) {
			case "tool":
	...
	private handleToolUseAsk(msg: AfxMessage, subtype: "tool" | "command" | "mcp", isDone: boolean): void {
	...
	private handleCommandOutputMessage(msg: AfxMessage, isDone: boolean): void {
