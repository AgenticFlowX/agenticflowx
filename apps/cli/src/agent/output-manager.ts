...
import { AfxMessage, AfxSay } from "@agenticflowx/types"
...
export class OutputManager {
	...
	/**
	 * Output a AfxMessage based on its type.
	 ...
	 */
	outputMessage(msg: AfxMessage, skipFirstUserMessage = true): void {
		const ts = msg.ts
		const text = msg.text || ""
	...
	private outputSayMessage(
		...
		switch (say) {
			...
			// case "thinking": - not a valid AfxSay type
			case "reasoning":
