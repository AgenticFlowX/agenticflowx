...
import type { ExtensionMessage, AfxMessage, AfxAsk, AfxSay, TodoItem } from "@agenticflowx/types"
import { consolidateTokenUsage, consolidateApiRequests, consolidateCommands } from "@agenticflowx/core/cli"
 ...
 *
 * Transforms AfxMessage format to TUIMessage format and updates the store.
 */
export function useMessageHandlers({ nonInteractive }: UseMessageHandlersOptions): UseMessageHandlersReturn {
	...
	const handleSayMessage = useCallback(
		(ts: number, say: AfxSay, text: string, partial: boolean) => {
	...
	const handleAskMessage = useCallback(
		(ts: number, ask: AfxAsk, text: string, partial: boolean) => {
			const messageId = ts.toString()
	...
	const handleExtensionMessage = useCallback(
		(msg: ExtensionMessage) => {
			if (msg.type === "state") {
				...
				const afxMessages = state.afxMessages

				if (afxMessages) {
					for (const afxMsg of afxMessages) {
						const ts = afxMsg.ts
						const type = afxMsg.type
						const say = afxMsg.say
						const ask = afxMsg.ask
						const text = afxMsg.text || ""
						const partial = afxMsg.partial || false
					...
					// Compute token usage metrics from afxMessages
					// Skip first message (task prompt) as per webview UI pattern
					if (afxMessages.length > 1) {
						const processed = consolidateApiRequests(
							consolidateCommands(afxMessages.slice(1) as AfxMessage[]),
						)
