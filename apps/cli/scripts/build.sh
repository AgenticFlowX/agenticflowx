...
# AgenticFlowX CLI Local Build Script
#
# Based on Roo Code (https://github.com/RooCodeInc/Roo-Code)
...
create_tarball() {
    ...
    RELEASE_DIR="$REPO_ROOT/afx-cli-${PLATFORM}"
    ...
    cat > "$RELEASE_DIR/bin/afx" << 'WRAPPER_EOF'
#!/usr/bin/env node
...
process.env.AFX_CLI_ROOT = join(__dirname, '..');
process.env.AFX_EXTENSION_PATH = join(__dirname, '..', 'extension');
const ripgrepPath = join(__dirname, 'rg');
if (existsSync(ripgrepPath)) {
  process.env.AFX_RIPGREP_PATH = ripgrepPath;
}
...
verify_local_install() {
    ...
    AFX_LOCAL_TARBALL="$TARBALL_PATH" \
    AFX_INSTALL_DIR="$VERIFY_INSTALL_DIR" \
    AFX_BIN_DIR="$VERIFY_BIN_DIR" \
    AFX_VERSION="$VERSION" \
    "$CLI_DIR/install.sh" || {
        rm -rf "$VERIFY_DIR"
    ...
    # Test --help
    if ! "$VERIFY_BIN_DIR/afx" --help > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
    ...
    # Test --version
    if ! "$VERIFY_BIN_DIR/afx" --version > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
...
print_summary() {
    ...
    if [ "$LOCAL_INSTALL" = true ]; then
        echo "  Installed to: ~/.afx/cli"
        echo "  Binary: ~/.local/bin/afx"
        echo ""
        echo "  Test it out:"
        echo "    afx --version"
        echo "    afx --help"
        ...
        echo "    AFX_LOCAL_TARBALL=$REPO_ROOT/$TARBALL ./apps/cli/install.sh"
        echo ""
...
main() {
    ...
    printf "${BLUE}${BOLD}"
    echo "  ╭─────────────────────────────────╮"
    echo "  │  AgenticFlowX CLI Local Build   │"
    echo "  ╰─────────────────────────────────╯"
    printf "${NC}"
    echo ""
