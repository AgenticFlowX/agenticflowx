...
This document explains how the AgenticFlowX CLI detects and tracks the agent loop state.
...
All agent activity is communicated through **AfxMessages** - a stream of timestamped messages that represent everything the agent does.
...
interface AfxMessage {
	...
	ask?: AfxAsk // Specific ask type (when type="ask")
	say?: AfxSay // Specific say type (when type="say")
...
const client = new ExtensionClient({
	...
	debug: true, // Writes to ~/.afx/cli-debug.log
})
...
Holds the `afxMessages` array, computed state, and current mode:

```typescript
interface StoreState {
	messages: AfxMessage[] // The raw message array
...
- `"state"` messages → Update `afxMessages` array and track mode
...
The CLI uses type guards from `@agenticflowx/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@agenticflowx/types"
...
Enable with `-d` flag. Logs go to `~/.afx/cli-debug.log`:

```bash
afx -d --print "Build something"
```

View logs:

```bash
tail -f ~/.afx/cli-debug.log
```
...
1. **Agent communicates via `AfxMessage` stream**
