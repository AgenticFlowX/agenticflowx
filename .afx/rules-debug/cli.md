...
1. **Write logs to a temporary file instead of console**:
    ...
    - Create a log file at a known location, e.g., `/tmp/afx-cli-debug.log`
    - Use `fs.appendFileSync()` to write timestamped log entries
    - Example logging utility:

        ```typescript
        import fs from "fs"
        const DEBUG_LOG = "/tmp/afx-cli-debug.log"
...
2. **Clear the log file before each debugging session**:
    - Run `echo "" > /tmp/afx-cli-debug.log` or use `fs.writeFileSync(DEBUG_LOG, "")` at app startup during debugging
...
3. **Read the log file** after the user completes testing:
    - Run `cat /tmp/afx-cli-debug.log` to retrieve the captured output
