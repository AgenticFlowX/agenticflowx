...
| FR-6 | `@earendil-works/pi-coding-agent` does not appear in the `apps/vscode` bundle (it is a devDependency of `@afx/agent-pi`, used only for type imports, and never reached from the extension entry; explicit esbuild externalization is therefore unnecessary) | Must Have |
...
| NFR-2 | Extension host bundle excludes `@earendil-works/pi-coding-agent` runtime | Verified via bundle inspection (devDep + type-only import path) |
