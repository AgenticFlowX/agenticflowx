...
updated_at: "2026-07-18T17:37:43.000Z"
...
- `2026-07-18T17:37:43.000Z` `/afx-task complete`: remediated the Modified-files and SDD presentation for the v2.4.0 candidate. Modified now represents the newest assistant edit batch, keeps at most two non-SDD chips in one compact row, moves the complete grouped inventory into a bounded `All N` overlay, disambiguates basename collisions, and presents SDD documents once in a subtle summary row. File controls preserve direct source navigation and add keyboard-, pointer-, and touch-reachable Markdown AFX Preview and native Git changes actions; the host resolves multi-root paths and explains Git unavailable, clean, outside-repository, missing-file, and command-failure cases. Automatic SDD Preview is batch-scoped to the newest successful SDD document. Visual regression work also caught and fixed a streaming hover race, an over-stretched desktop chip target, and a stale legacy SDD-preview expectation. Verification covered focused derivation/component/controller suites, shared/transport/host protocol suites, responsive 34/21 and 50-file Playwright scenarios, full workspace/E2E gates, a real `vscode.git` / `git.openChange` extension-host smoke, and four dedicated review captures. `[FR-10] [NFR-5] [NFR-7] [DES-COMPOSER-FILES-STRIP] [DES-TEST]`
