...
| FR-16 | Surface low-noise AFX workflow affordances from existing composer anchors: shared Memory dropdown, More menu with command-context presets/focus targets, and parsed `Next:` result actions under completed assistant messages; composer/menu mutating or open-argument commands remain draft-first, while completed-message `Next:` chips run supported catalog commands directly and leave unsupported commands unavailable instead of silently drafting                                                       | Should Have |
...
- [ ] Completed assistant messages keep raw `Next:` text visible, parse supported `/afx-*` follow-up commands, and surface them as subtle inline button chips that send directly without inserting into the composer draft
