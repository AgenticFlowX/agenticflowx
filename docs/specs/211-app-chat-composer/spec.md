...
| FR-15 | When `workspaceMode === "spec"` and an AFX document (sprint single-file or standard 4-file `spec.md` / `design.md` / `tasks.md`) is the active editor, render a doc-actions strip above the composer that surfaces SDD intent buttons routed by detected format; render a separate mode-suggest strip when a sprint file is active and the user is not yet in Spec mode | Must Have   |
