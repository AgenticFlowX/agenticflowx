...
updated_at: "2026-06-01T21:01:05.000Z"
...
| -------- | --------- | --------------------------------------------------------------------------------------------------------------------- | ------ |
| 1.1      | Phase 1.1 | Move panel FRs/DES content from `200-app-vscode/{spec,design}.md` into this spec.                                     | [x]    |
| 1.2      | Phase 1.2 | Retarget `apps/vscode/src/panels/*.ts` `@see` headers from `200-app-vscode` to `201-app-vscode-panels`.               | [x]    |
...
| Priority | Phase     | Description                                                               | Status |
| -------- | --------- | ------------------------------------------------------------------------- | ------ |
| 2.1      | Phase 2.1 | Add automated test that nonces differ across two `loadWebviewHtml` calls. | [x]    |
| 2.2      | Phase 2.2 | Add visual smoke check that confirms no FOUC on theme/style switch.       | [x]    |

## Phase 3: Appearance Bridge

| Priority | Phase     | Description                                                                                                  | Status |
| -------- | --------- | ------------------------------------------------------------------------------------------------------------ | ------ |
| 3.1      | Phase 3.1 | Validate `appearance/update` payloads against `AFX_THEME_IDS` / `AFX_STYLE_IDS` before applying.             | [x]    |
...
| ------------------------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- | ----- |
| 2026-06-01T21:01:05.000Z | -    | Cleared the Knip unused-export finding by removing the retired shell syntax boolean wrapper after Explore switched to structured syntax reasons.                                            | `apps/vscode/src/utils/shell-command-parser.ts`, `docs/specs/201-app-vscode-panels/tasks.md`                                                                                         | yes   | [x]   |
| 2026-06-01T13:01:16.000Z | -    | Reworked Explore runtime guardrails with reference-agent-informed browser action and MCP read-only delegation, plus host and VS Code e2e coverage.                                          | `apps/vscode/src/panels/{explore-guardrail,sidebar-panel}.ts`, `apps/vscode-e2e/src/extension.test.ts`, related tests, `docs/specs/201-app-vscode-panels/{design,tasks}.md`          | yes   | [x]   |
| 2026-06-01T12:41:07.000Z | -    | Relaxed Explore read-only guardrails for code and web investigation while preserving mutation blocks and adding actionable denial details.                                                  | `apps/vscode/src/utils/*shell*`, `apps/vscode/src/panels/{explore-guardrail,sidebar-panel}.ts`, related tests, `docs/specs/201-app-vscode-panels/{design,tasks}.md`                  | yes   | [x]   |
| 2026-05-21               | -    | Clarified that all four Explore intent slots inherit the same read-only inspection surface and added coverage for each slot.                                                                | `packages/shared/src/intent-prompts.ts`, `apps/vscode/src/panels/sidebar-panel.test.ts`, `docs/specs/201-app-vscode-panels/*.md`                                                     | yes   | [x]   |
| 2026-05-21               | -    | Tightened Explore shell classification: single `&`, shell grouping, and mutating curl request forms are blocked before spawn.                                                               | `apps/vscode/src/panels/sidebar-panel.ts`, `apps/vscode/src/panels/sidebar-panel.test.ts`, `docs/specs/201-app-vscode-panels/*.md`                                                   | yes   | [x]   |
