...
### Editor-Area Preview Protocol (FR-16)

- [x] Host-to-webview `afxPreviewShow { filePath, content, isAfxHint? }` pushes full preview content
- [x] Workbench `afxOpenFile.mode` accepts `"editor" | "preview" | "afxPreview"`
- [x] Chat `chat/openFile.mode` accepts `"editor" | "afxPreview"` and defaults to `"editor"`
