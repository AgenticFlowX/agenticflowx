...
| FR-16 | Editor-area preview protocol: host-to-webview `afxPreviewShow { filePath, content, isAfxHint? }` renders the editor-area preview; `afxOpenFile.mode` gains an `afxPreview` value so Workbench document actions can route to that panel; webview-to-host `afxCopyMarkdown { content; label? }` copies raw markdown through the host clipboard; `afxToggleSession` adds an optional `line?` and tightens `column` to the `"agent" \| "human"` literal union; `afxToggleAllSessions` and `afxApproveSessions` carry the bulk Work Sessions signoff mutations from the preview signoff toolbar | Must Have   |
