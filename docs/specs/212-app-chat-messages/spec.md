...
| FR-9  | Own floating user-turn context in the conversation timeline: the sticky prompt preview appears only after its user row scrolls above the actual transcript viewport, hides at the top of the thread, stays visually distinct from the sticky day header, recomputes on scroll/resize so stale observer state cannot cover content, and acts as an anchor button that scrolls back to the original user message | Must Have   |
...
- [ ] Floating user-turn context follows the conversation scroll root, appears only after the prompt row leaves view, hides again at scroll top, remains readable against surrounding history, and scrolls back to the original prompt when clicked
