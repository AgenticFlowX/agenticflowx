...
updated_at: "2026-07-19T00:53:07.000Z"
...
| FR-15 | Open a selected active-leaf session in the shared Chat conversation-timeline presentation without changing the active live Chat session; render each matched tool call/result once, retain unmatched tool results and bash executions, and suppress result/workflow actions plus live announcements in read-only History | Must Have   |
...
| FR-21 | At narrow sidebar widths (down to ~230px), labels and metadata stay compact, timeline content wraps without horizontal overflow, and sticky transcript controls remain reachable through container-aware rules                                                                                                           | Should Have |
...
- [x] Selecting a session opens the active leaf branch without modifying the live Chat timeline and presents supported messages, compaction, and tool activity through the same conversation timeline component used by Chat
- [x] A tool call and its matching result render as one execution row; unmatched tool results and bash executions remain visible once in chronological order
- [x] Read-only History suppresses result actions, SDD workflow suggestions, and live-region announcements while retaining message text and tool status/output
...
- [x] At ~230–320px container width the sub-tab labels and metadata stay compact, transcript content wraps within the timeline column without horizontal overflow, and the sticky header/footer remain reachable, proven by narrow-width e2e _(FR-21)_
...
| OQ-2 | Should transcript-to-Chat rehydration reuse the live normalizer or use a dedicated persisted mapper? | Resolved | Yes      | Share one pure persisted mapper between History preview and reopen; keep the host file as a compatibility re-export.                                                                                                                       |
| OQ-3 | What exact transcript depth appears in read-only mode: messages only, or messages plus tools/usage?  | Resolved | No       | Render Chat-parity messages, compaction, paired tools, unmatched results, and bash. Read-only hides actions and announcements.                                                                                                             |
...
| Owned files     | Live log: `apps/chat/src/views/history.tsx`, `apps/chat/src/lib/history-events.ts`. Persistent: `apps/chat/src/views/session-browser.tsx`, read-only renderer `apps/chat/src/components/chat/conversation-timeline.tsx`, History-tab shell in `apps/chat/src/app.tsx`; canonical mapper `packages/shared/src/transcript-to-timeline.ts`; host `apps/vscode/src/services/history/{history-service,transcript-to-timeline}.ts`; reader `packages/agent/pi/src/session-store.ts` |
| Local anchors   | Live: `History`, `ContextPreviewCard`, `HistorySetupState`, `HistorySection`, `HistoryEventRow`, `deriveHistoryEvents`, `toolEvent`, `classifyTool`. Persistent: `SessionBrowser`, `SessionRow`, `TranscriptView`, `ConversationTimeline` (`readOnly`), `transcriptToTimeline`, `StatusBlock`, `projectName`, `piSessionRoots`, `listSessionsFromDisk`, `parseTranscript`                                                                                                     |
