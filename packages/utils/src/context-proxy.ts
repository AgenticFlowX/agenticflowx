...
type AfxSettingsKey = keyof AfxSettings
...
export class ContextProxy {
	 ...
	 */
	private sanitizeProviderValues(values: AfxSettings): AfxSettings {
		...
		for (const key of legacyKeys) {
			if (key in sanitizedValues) {
				...
				sanitizedValues = copy as AfxSettings
			}
		}
		...
		if (values.apiProvider !== undefined && !isKnownProvider) {
			...
			return restValues as AfxSettings
		}
	...
	public async setValue<K extends AfxSettingsKey>(key: K, value: AfxSettings[K]) {
	...
	public getValue<K extends AfxSettingsKey>(key: K): AfxSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as AfxSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as AfxSettings[K])
	}

	public getValues(): AfxSettings {
	...
	public async setValues(values: AfxSettings) {
		const entries = Object.entries(values) as [AfxSettingsKey, unknown][]
	...
	public async export(): Promise<GlobalSettings | undefined> {
		try {
			...
			// Exports should only contain global settings, so this skips project custom modes (those exist in the .afxmode folder)
