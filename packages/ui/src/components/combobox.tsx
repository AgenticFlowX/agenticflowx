...
import {
  ...
  InputGroupButton,
  InputGroupInput,
} from "@afx/ui/components/input-group";
...
function ComboboxTrigger({ className, children, ...props }: ComboboxPrimitive.Trigger.Props) {
  return (
    <ComboboxPrimitive.Trigger
      ...
      className={cn("cn-combobox-trigger", "[&_svg:not([class*='size-'])]:size-4", className)}
      {...props}
    >
      {children}
      ...
      <InputGroupAddon align="inline-end">
        {showTrigger && (
          <InputGroupButton
            ...
            className="cn-input-group-button group-has-data-[slot=combobox-clear]/input-group:hidden data-pressed:bg-transparent"
            disabled={disabled}
          >
            <ComboboxTrigger />
          </InputGroupButton>
...
function ComboboxLabel({ className, ...props }: ComboboxPrimitive.GroupLabel.Props) {
  return (
    <ComboboxPrimitive.GroupLabel
      ...
      className={cn("cn-combobox-label", "px-2 py-2 text-xs text-muted-foreground", className)}
      {...props}
    />
  );
}
...
function ComboboxSeparator({ className, ...props }: ComboboxPrimitive.Separator.Props) {
  return (
    <ComboboxPrimitive.Separator
      ...
      className={cn("cn-combobox-separator", "-mx-1 h-px bg-border", className)}
      {...props}
    />
  );
}
