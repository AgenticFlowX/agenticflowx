...
import { type ModelInfo, type ModelRecord, unboundDefaultModelId, unboundDefaultModelInfo } from "@agenticflowx/types"
...
import { getModels } from "./fetchers/model-cache"
...
export class UnboundHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			defaultHeaders: {
				...DEFAULT_HEADERS,
				"X-Unbound-Metadata": JSON.stringify({ labels: [{ key: "app", value: "agenticflowx" }] }),
			},
		})
	}
	...
	override async *createMessage(
		...
		const completionParams: UnboundChatCompletionParamsStreaming = {
			...
			unbound_metadata: { originApp: "agenticflowx", taskId: metadata?.taskId, mode: metadata?.mode },
