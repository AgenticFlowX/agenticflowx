...
import { consolidateApiRequests } from "../consolidate-api-requests.js"

describe("consolidateApiRequests", () => {
	// Helper function to create a basic api_req_started message
	const createApiReqStarted = (ts: number, data: Record<string, unknown> = {}): AfxMessage => ({
		ts,
		type: "say",
		say: "api_req_started",
	...
	const createApiReqFinished = (ts: number, data: Record<string, unknown> = {}): AfxMessage => ({
		ts,
		type: "say",
	...
	const createTextMessage = (ts: number, text: string): AfxMessage => ({
		ts,
		type: "say",
		say: "text",
		text,
	...
	it("should handle messages with no api_req pairs", () => {
		const messages: AfxMessage[] = [createTextMessage(1000, "Hello"), createTextMessage(1001, "World")]
	...
	it("should handle single message array", () => {
		const messages: AfxMessage[] = [createTextMessage(1000, "Hello")]
		const result = consolidateApiRequests(messages)
		expect(result).toEqual(messages)
	})
