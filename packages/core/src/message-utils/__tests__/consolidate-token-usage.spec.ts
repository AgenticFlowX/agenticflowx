...
import { consolidateTokenUsage, hasTokenUsageChanged, hasToolUsageChanged } from "../consolidate-token-usage.js"
...
describe("consolidateTokenUsage", () => {
	...
	describe("basic token accumulation", () => {
		it("should accumulate tokens from a single message", () => {
			const messages: AfxMessage[] = [createApiReqMessage(1000, { tokensIn: 100, tokensOut: 50, cost: 0.01 })]
	...
	describe("invalid data handling", () => {
		it("should handle messages with invalid JSON", () => {
			const messages: AfxMessage[] = [{ ts: 1000, type: "say", say: "api_req_started", text: "invalid json" }]
		...
		it("should handle missing token values", () => {
			const messages: AfxMessage[] = [createApiReqMessage(1000, { cost: 0.01 })]
