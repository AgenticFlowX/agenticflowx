...
import { AfxIgnoreController } from "@agenticflowx/ignore"
...
export async function parseSourceCodeDefinitionsForFile(
	...
	if (ext === ".md" || ext === ".markdown") {
		...
		if (afxIgnoreController && !afxIgnoreController.validateAccess(filePath)) {
			return undefined
		}
	...
	const definitions = await parseFile(filePath, languageParsers, afxIgnoreController)
	if (definitions) {
		return `# ${path.basename(filePath)}\n${definitions}`
	}
 ...
 * @param afxIgnoreController - Optional controller to check file access permissions
