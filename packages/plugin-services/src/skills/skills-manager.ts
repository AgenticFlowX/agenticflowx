...
import type { ITaskProvider } from "@agenticflowx/types"
import { getGlobalAfxDirectory, getGlobalAgentsDirectory, getProjectAgentsDirectoryForCwd } from "../afx-config"
import { directoryExists, fileExists } from "../afx-config"
import { SkillMetadata, SkillContent } from "@agenticflowx/utils"
import { modes, getAllModes } from "@agenticflowx/utils"
import {
	validateSkillName as validateSkillNameShared,
	SkillNameValidationError,
	SKILL_NAME_MAX_LENGTH,
} from "@agenticflowx/types"
import { t } from "@agenticflowx/utils"
...
export class SkillsManager {
	...
	private providerRef: WeakRef<ITaskProvider>
	private disposables: vscode.Disposable[] = []
	private isDisposed = false

	constructor(provider: ITaskProvider) {
	 ...
	 * - .afx/skills can be a symlink to a directory containing skill subdirectories
	 * - .afx/skills/[dirname] can be a symlink to a skill directory
	 */
	...
	private async loadSkillMetadata(
		...
		try {
			...
			if (Array.isArray(metadataRaw?.["modeSlugs"])) {
				// agentskills.io standard: modeSlugs under metadata
				modeSlugs = (metadataRaw["modeSlugs"] as unknown[]).filter(
			...
			// [AFX-START] Extract argumentHint from metadata — @see docs/research/res-afx-plugin-architecture-plan.md
			const metadata = frontmatter.metadata as Record<string, unknown> | undefined
			const argumentHint =
				typeof metadata?.["afx-argument-hint"] === "string" && metadata["afx-argument-hint"].trim()
					? metadata["afx-argument-hint"].trim()
			...
			this.skills.set(skillKey, {
				...
				argumentHint, // [AFX] Pass parsed argumentHint to skill metadata
			})
		} catch (error) {
	...
	async createSkill(
		...
		if (source === "global") {
			...
			baseDir = path.join(provider.cwd, ".afx")
		}
...
`
	...
	> {
		...
		const provider = this.providerRef.deref()
		const projectAfxDir = provider?.cwd ? path.join(provider.cwd, ".afx") : null
		...
		// - Global: .agents/skills first, then .afx/skills (so .afx wins)
		// - Project: .agents/skills first, then .afx/skills (so .afx wins)
		...
		// Global .afx directories (AFX-specific, higher priority than .agents)
		dirs.push({ dir: path.join(globalAfxDir, "skills"), source: "global" })
		for (const mode of modesList) {
			dirs.push({ dir: path.join(globalAfxDir, `skills-${mode}`), source: "global", mode })
		}

		// Project .afx directories (highest priority)
		if (projectAfxDir) {
			dirs.push({ dir: path.join(projectAfxDir, "skills"), source: "project" })
			for (const mode of modesList) {
				dirs.push({ dir: path.join(projectAfxDir, `skills-${mode}`), source: "project", mode })
			}
		}
	...
	private async setupFileWatchers(): Promise<void> {
		...
		const globalAfxDir = getGlobalAfxDirectory()
		const globalAgentsDir = getGlobalAgentsDirectory()
		const projectAfxDir = path.join(provider.cwd, ".afx")
		const projectAgentsDir = getProjectAgentsDirectoryForCwd(provider.cwd)

		// Watch global .afx skills directory
		this.watchDirectory(path.join(globalAfxDir, "skills"))

		// Watch global .agents skills directory
		this.watchDirectory(path.join(globalAgentsDir, "skills"))

		// Watch project .afx skills directory
		this.watchDirectory(path.join(projectAfxDir, "skills"))
		...
		for (const mode of modesList) {
			// .afx mode-specific
			this.watchDirectory(path.join(globalAfxDir, `skills-${mode}`))
			this.watchDirectory(path.join(projectAfxDir, `skills-${mode}`))
