...
import { GlobalFileNames } from "@agenticflowx/utils"

import { fileExistsAtPath } from "@agenticflowx/utils"
import { arePathsEqual, getWorkspacePath } from "@agenticflowx/utils"
import { injectVariables } from "@agenticflowx/utils"
import { safeWriteJson } from "@agenticflowx/utils"
import { sanitizeMcpName, toolNamesMatch } from "@agenticflowx/utils"
...
export class McpHub {
	...
	/**
	 * Registers a client (e.g., ITaskProvider) using this hub.
	...
	private async watchProjectMcpFile(): Promise<void> {
		...
		const projectMcpPattern = new vscode.RelativePattern(workspaceFolder, ".afx/mcp.json")
  ...
  "mcpServers": {
	...
	private async getProjectMcpPath(): Promise<string | null> {
		...
		const projectMcpDir = path.join(workspacePath, ".afx")
	...
	private async notifyWebviewOfServerChanges(): Promise<void> {
		...
		const targetProvider: ITaskProvider | undefined = this.providerRef.deref()
