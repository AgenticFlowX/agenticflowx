...
export class SimpleInstaller {
	...
	private async installMode(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".afxmodes" : "custom-modes.yaml"
				throw new Error(
	...
	private async installMcp(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".afx/mcp.json" : "mcp-settings.json"
				throw new Error(
	...
	private async getModeFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".afxmodes")
		} else {
	...
	private async getMcpFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".afx", "mcp.json")
		} else {
			const globalSettingsPath = await ensureSettingsDirectoryExists(this.context)
