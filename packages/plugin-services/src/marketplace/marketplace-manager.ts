...
import type { MarketplaceItem, MarketplaceItemType } from "@agenticflowx/types"
import { TelemetryService } from "@agenticflowx/telemetry"

import { GlobalFileNames } from "@agenticflowx/utils"
import { ensureSettingsDirectoryExists } from "@agenticflowx/utils"
import { t } from "@agenticflowx/utils"
import type { CustomModesManager } from "@agenticflowx/utils"

import { RemoteConfigLoader } from "./remote-config-loader"
...
export class MarketplaceManager {
	...
	private async checkProjectInstallations(metadata: Record<string, { type: string }>): Promise<void> {
		try {
			...
			// Check modes in .afxmodes
			const projectModesPath = path.join(workspaceFolder.uri.fsPath, ".afxmodes")
			try {
			...
			// Check MCPs in .afx/mcp.json
			const projectMcpPath = path.join(workspaceFolder.uri.fsPath, ".afx", "mcp.json")
			try {
