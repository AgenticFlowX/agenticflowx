...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("agenticflowx", context)
			...
			setRuntimeConfig("agenticflowx", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("agenticflowx", "testSetting", "testValue")

			expect(getRuntimeConfig("agenticflowx.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("agenticflowx", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("agenticflowx.setting1")).toBe("value1")
			expect(getRuntimeConfig("agenticflowx.setting2")).toBe(42)
			expect(getRuntimeConfig("agenticflowx.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("agenticflowx.defined")).toBe("value")
			expect(getRuntimeConfig("agenticflowx.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("agenticflowx", "setting1", "value1")
			setRuntimeConfig("agenticflowx", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("agenticflowx.setting1")).toBeUndefined()
			expect(getRuntimeConfig("agenticflowx.setting2")).toBeUndefined()
		})
