...
build_extension() {
  echo "🔨 Building the AgenticFlowX extension..."
  pnpm -w vsix -- --out ../bin/agenticflowx-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/agenticflowx-$(git rev-parse --short HEAD).vsix || exit 1
}
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  ...
  echo "The AgenticFlowX evals system can also be run with Docker on any platform."
  echo "See https://github.com/AgenticFlowX/agenticflowx/blob/main/packages/evals/README.md for instructions."
  exit 1
fi
...
if ! code --list-extensions 2>/dev/null | grep -q "agenticflowx.agenticflowx"; then
  code --install-extension agenticflowx.agenticflowx &>/dev/null || exit 1
fi
...
if [[ ! -d "../../../evals" ]]; then
  ...
  git clone https://github.com/AgenticFlowX/agenticflowx-evals.git ../../../evals || exit 1
  echo "✅ Done"
else
...
echo -n "🗄️ Syncing AgenticFlowX evals database... "
pnpm --filter @agenticflowx/evals db:push --force &>/dev/null || exit 1
echo "✅ Done"
...
current_version=$(code --list-extensions --show-versions 2>/dev/null | grep agenticflowx)
read -p "💻 Do you want to build a new version of the AgenticFlowX extension? [currently $current_version] (y/N): " build_extension
...
if ! nc -z localhost 3446; then
  ...
  if [[ "$start_evals" =~ ^[Yy]|^$ ]]; then
    pnpm --filter @agenticflowx/evals cli
  else
    echo "💡 You can start it anytime with 'pnpm --filter @agenticflowx/evals cli'."
  fi
else
