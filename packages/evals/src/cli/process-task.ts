...
import { type TaskEvent, AfxEventName } from "@agenticflowx/types"
...
import { runUnitTest } from "./run-unit-test"
import { runTaskWithCli } from "./run-task-in-cli"
import { runTaskInVscode } from "./run-task-in-vscode"
...
export const processTask = async ({
	...
	try {
		...
		await publish({
			eventName: passed ? AfxEventName.EvalPass : AfxEventName.EvalFail,
			taskId: task.id,
		})
	} finally {
...
export const processTaskInContainer = async ({
	...
	if (jobToken) {
		baseArgs.push(`-e AFX_CLOUD_TOKEN=${jobToken}`)
	}
	...
	const command = `pnpm --filter @agenticflowx/evals cli --taskId ${taskId}`
	logger.info(command)
