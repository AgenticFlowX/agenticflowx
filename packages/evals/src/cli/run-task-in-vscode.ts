...
import { IpcClient } from "@agenticflowx/ipc"
...
import { MessageLogDeduper } from "./message-log-deduper"
...
export const runTaskInVscode = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const env = { AFX_IPC_SOCKET_PATH: ipcSocketPath }
	...
	let codeCommand = containerized
		? `xvfb-run --auto-servernum --server-num=1 code --wait --log trace --disable-workspace-trust --disable-gpu --disable-lcd-text --no-sandbox --user-data-dir /afx/.vscode --password-store="basic" -n ${workspacePath}`
		: `code --disable-workspace-trust -n ${workspacePath}`

	if (jobToken) {
		codeCommand = `AFX_CLOUD_TOKEN=${jobToken} ${codeCommand}`
	}
	...
	let afxTaskId: string | undefined
	...
	const ignoreEvents: Record<"broadcast" | "log", AfxEventName[]> = {
		broadcast: [AfxEventName.Message],
		log: [AfxEventName.TaskTokenUsageUpdated, AfxEventName.TaskAskResponded],
	}

	const loggableSays: AfxSay[] = [
		"error",
		"command_output",
		"afxignore_error",
		"diff_error",
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		if (
			eventName === AfxEventName.Message &&
		...
		if (
			...
			if (eventName === AfxEventName.Message) {
			...
			if (eventName === AfxEventName.Message && payload[0]?.message?.ask === "tool") {
				try {
			...
			} else if (eventName === AfxEventName.Message && payload[0]?.message?.ask === "command") {
				logEventName = `${eventName} (command)`
			} else if (eventName === AfxEventName.Message && payload[0]?.message?.ask === "completion_result") {
				logEventName = `${eventName} (completion_result)`
			}
			logger.info(`${logEventName} ->`, payload)
		}

		if (eventName === AfxEventName.TaskStarted) {
		...
		if (eventName === AfxEventName.TaskToolFailed) {
			const [_taskId, toolName, error] = payload
			await createToolError({ taskId: task.id, toolName, error })
		}

		if (eventName === AfxEventName.TaskTokenUsageUpdated || eventName === AfxEventName.TaskCompleted) {
		...
		if (eventName === AfxEventName.TaskAborted) {
			taskAbortedAt = Date.now()
		}

		if (eventName === AfxEventName.TaskCompleted) {
	...
	try {
		...
		if (afxTaskId && !isClientDisconnected) {
