...
export const resetEvalsRepo = async ({ run, cwd }: { run: Run; cwd: string }) => {
	await execa({ cwd })`git config user.name "AgenticFlowX"`
	await execa({ cwd })`git config user.email "support@agenticflowx.com"`
...
export async function copyConversationHistory({
	...
	const extensionStoragePath = "/afx/.vscode/User/globalStorage/agenticflowx.agenticflowx"
	const taskStoragePath = path.join(extensionStoragePath, "tasks", afxTaskId)
