...
import { type ToolUsage, TaskCommandName, AfxEventName, IpcMessageType } from "@agenticflowx/types"
...
/**
 * Run a task using the AgenticFlowX CLI (headless mode).
...
export const runTaskWithCli = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const env: Record<string, string> = {
		...
		AFX_IPC_SOCKET_PATH: ipcSocketPath,
	}

	if (jobToken) {
		env.AFX_CLOUD_TOKEN = jobToken
	}
	...
	const ignoreEventsForBroadcast = [AfxEventName.Message]
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		// but we still want to capture the afxTaskId from TaskStarted if we receive it.
		if (eventName === AfxEventName.TaskStarted) {
			taskStartedAt = Date.now()
			afxTaskId = payload[0]
			logger.info(`received TaskStarted event, afxTaskId: ${afxTaskId}`)
		}
