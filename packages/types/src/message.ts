...
export const afxAskSchema = z.enum(afxAsks)

export type AfxAsk = z.infer<typeof afxAskSchema>
/**
 * IdleAsk
 *
...
export const idleAsks = [
	...
	"mistake_limit_reached",
	"auto_approval_max_req_reached",
] as const satisfies readonly AfxAsk[]

export type IdleAsk = (typeof idleAsks)[number]

export function isIdleAsk(ask: AfxAsk): ask is IdleAsk {
	return (idleAsks as readonly AfxAsk[]).includes(ask)
}
...
export const resumableAsks = ["resume_task"] as const satisfies readonly AfxAsk[]

export type ResumableAsk = (typeof resumableAsks)[number]

export function isResumableAsk(ask: AfxAsk): ask is ResumableAsk {
	return (resumableAsks as readonly AfxAsk[]).includes(ask)
}
...
export const interactiveAsks = ["followup", "command", "tool", "use_mcp_server"] as const satisfies readonly AfxAsk[]

export type InteractiveAsk = (typeof interactiveAsks)[number]

export function isInteractiveAsk(ask: AfxAsk): ask is InteractiveAsk {
	return (interactiveAsks as readonly AfxAsk[]).includes(ask)
}
...
export const nonBlockingAsks = ["command_output"] as const satisfies readonly AfxAsk[]

export type NonBlockingAsk = (typeof nonBlockingAsks)[number]

export function isNonBlockingAsk(ask: AfxAsk): ask is NonBlockingAsk {
	return (nonBlockingAsks as readonly AfxAsk[]).includes(ask)
}
 ...
 * - `afxignore_error`: Error related to .afxignore file processing
...
export const afxSaySchema = z.enum(afxSays)

export type AfxSay = z.infer<typeof afxSaySchema>
 ...
 */
export const afxMessageSchema = z.object({
	ts: z.number(),
	type: z.union([z.literal("ask"), z.literal("say")]),
	ask: afxAskSchema.optional(),
	say: afxSaySchema.optional(),
	text: z.string().optional(),
...
export type AfxMessage = z.infer<typeof afxMessageSchema>
