...
export class AfxIgnoreController {
	...
	constructor(cwd: string) {
		...
		// Set up file watcher for .afxignore
	...
	private setupFileWatcher(): void {
		const afxignorePattern = new vscode.RelativePattern(this.cwd, ".afxignore")
		const fileWatcher = vscode.workspace.createFileSystemWatcher(afxignorePattern)
	...
	private async loadAfxIgnore(): Promise<void> {
		try {
			...
			console.error("Unexpected error loading .afxignore:", error)
		}
	}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .afxignore does not exist
		...
		try {
			...
			// Convert real path to relative for .afxignore checking
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .afxignore exists
		if (!this.afxIgnoreContent) {
			return undefined
		}
	 ...
	 * @returns Formatted instructions or undefined if .afxignore doesn't exist
	 */
