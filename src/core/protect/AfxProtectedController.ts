...
/**
 * Controls write access to AFX configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive AFX configuration files.
 */
export class AfxProtectedController {
	private cwd: string
	private ignoreInstance: Ignore

	// Predefined list of protected AFX configuration patterns
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(AfxProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a AFX configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = AfxProtectedController.PROTECTED_PATTERNS.join(", ")
		return `# Protected Files\n\n(The following AFX configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return AfxProtectedController.PROTECTED_PATTERNS
	}
}
