...
const {
	...
	mockReaddir,
	mockLstat,
	mockGetAfxDirectoriesForCwd,
	mockGetAllAfxDirectoriesForCwd,
	mockGetAgentsDirectoriesForCwd,
	mockGetGlobalAfxDirectory,
} = vi.hoisted(() => ({
	mockHomedir: vi.fn(),
	mockStat: vi.fn(),
	mockReadFile: vi.fn(),
	mockReaddir: vi.fn(),
	mockLstat: vi.fn(),
	mockGetAfxDirectoriesForCwd: vi.fn(),
	mockGetAllAfxDirectoriesForCwd: vi.fn(),
	mockGetAgentsDirectoriesForCwd: vi.fn(),
	mockGetGlobalAfxDirectory: vi.fn(),
}))
...
// Mock the afx-config service
vi.mock("../../../../services/afx-config", () => ({
	getAfxDirectoriesForCwd: mockGetAfxDirectoriesForCwd,
	getAllAfxDirectoriesForCwd: mockGetAllAfxDirectoriesForCwd,
	getAgentsDirectoriesForCwd: mockGetAgentsDirectoriesForCwd,
	getGlobalAfxDirectory: mockGetGlobalAfxDirectory,
}))

import { loadRuleFiles, addCustomInstructions } from "../custom-instructions"

describe("custom-instructions global .afx support", () => {
	const mockCwd = "/mock/project"
	const mockHomeDir = "/mock/home"
	const globalAfxDir = path.join(mockHomeDir, ".afx")
	const projectAfxDir = path.join(mockCwd, ".afx")

	beforeEach(() => {
		vi.clearAllMocks()
		mockHomedir.mockReturnValue(mockHomeDir)
		mockGetAfxDirectoriesForCwd.mockReturnValue([globalAfxDir, projectAfxDir])
		// getAllAfxDirectoriesForCwd is now async and returns the same directories by default
		mockGetAllAfxDirectoriesForCwd.mockResolvedValue([globalAfxDir, projectAfxDir])
		// getAgentsDirectoriesForCwd returns parent directories (without .afx)
		mockGetAgentsDirectoriesForCwd.mockResolvedValue([mockCwd])
		mockGetGlobalAfxDirectory.mockReturnValue(globalAfxDir)
	...
	describe("loadRuleFiles", () => {
		...
		it("should fall back to legacy .afxrules file when no .afx/rules directories exist", async () => {
		...
		it("should return empty string when no rules exist anywhere", async () => {
			...
			mockReadFile
				.mockResolvedValueOnce("") // .afxrules returns empty (simulating ENOENT caught by safeReadFile)
	...
	describe("addCustomInstructions mode-specific rules", () => {
		it("should load global and project mode-specific rules", async () => {
			...
			mockReadFile
				...
				.mockResolvedValueOnce("") // .afxrules legacy file (empty)
		...
		it("should fall back to legacy mode-specific files when no mode directories exist", async () => {
			...
			mockReadFile
				.mockResolvedValueOnce("legacy mode rule content") // .afxrules-code
				.mockResolvedValueOnce("") // AGENTS.md file (empty)
				.mockResolvedValueOnce("") // generic .afxrules (empty)
				.mockResolvedValueOnce("") // generic .clinerules (empty)

			const result = await addCustomInstructions("", "", mockCwd, mode)

			expect(result).toContain("# Rules from .afxrules-code:")
