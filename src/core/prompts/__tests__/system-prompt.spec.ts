...
import { ModeConfig } from "@agenticflowx/types"
...
__setMockImplementation(
	async (
		...
		cwd: string,
		mode: string,
		options?: { language?: string; afxIgnoreInstructions?: string; settings?: Record<string, any> },
	) => {
		const sections = []
...
describe("SYSTEM_PROMPT", () => {
	...
	it("should maintain consistent system prompt", async () => {
		const prompt = await SYSTEM_PROMPT(
			...
			experiments,
			undefined, // language
			undefined, // afxIgnoreInstructions
		)
