...
describe("webviewMessageHandler delete functionality", () => {
	...
	beforeEach(() => {
		...
		getCurrentTaskMock = {
			...
			overwriteAfxMessages: vi.fn(async () => {}),
	...
	describe("handleDeleteMessageConfirm", () => {
		it("should handle deletion when apiConversationHistoryIndex is -1 (message not in API history)", async () => {
			...
			getCurrentTaskMock.afxMessages = [
			...
			// Verify that afxMessages was truncated at the correct index
			expect(getCurrentTaskMock.overwriteAfxMessages).toHaveBeenCalledWith([])
		...
		it("should handle deletion when message not found in afxMessages", async () => {
			getCurrentTaskMock.afxMessages = [{ ts: 1000, say: "user", text: "Some message" }]
			...
			expect(getCurrentTaskMock.overwriteAfxMessages).not.toHaveBeenCalled()
		...
		it("should handle deletion with attempt_completion in API history", async () => {
			...
			// Verify that afxMessages was truncated
			expect(getCurrentTaskMock.overwriteAfxMessages).toHaveBeenCalledWith([])
