...
import type {
	AfxTerminalProvider,
	AfxTerminal,
	AfxTerminalCallbacks,
	AfxTerminalProcess,
	AfxTerminalProcessResultPromise,
	ExitCodeDetails,
} from "./types"

export abstract class BaseTerminal implements AfxTerminal {
	public readonly provider: AfxTerminalProvider
	...
	public process?: AfxTerminalProcess
	public completedProcesses: AfxTerminalProcess[] = []

	constructor(provider: AfxTerminalProvider, id: number, cwd: string) {
	...
	abstract runCommand(command: string, callbacks: AfxTerminalCallbacks): AfxTerminalProcessResultPromise
	...
	public setActiveStream(stream: AsyncIterable<string> | undefined, pid?: number): void {
		if (stream) {
			if (!this.process) {
				...
				console.warn(
					`[Terminal ${this.provider}/${this.id}] process is undefined, so cannot set terminal stream (probably user-initiated non-AFX command)`,
				)
	 ...
	 */
	public getProcessesWithOutput(): AfxTerminalProcess[] {
