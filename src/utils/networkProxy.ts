 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the AgenticFlowX extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure AgenticFlowX's `fetch()` calls are proxied, patch global fetch in debug mode.
