...
function asObjectSafe(value: any): object {
	...
	try {
		...
		console.warn("AgenticFlowX <Language Model API>: Failed to parse object:", error)
		return {}
	}
}
			...
			if (item instanceof vscode.LanguageModelToolCallPart) {
				...
				if (item.input && Object.keys(item.input).length > 0) {
					try {
						...
						console.error("AgenticFlowX <Language Model API>: Failed to stringify tool call input:", error)
					}
				}
			}
		}
